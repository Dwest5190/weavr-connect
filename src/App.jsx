  ...
  return <Modal title={"Contact "+person.firstName} onClose={p.onClose}>
    {person.phone&&<div style={{marginBottom:16}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:8,background:"var(--inp)",borderRadius:12,padding:"12px 16px"}}>
        ...
        <span style={{flex:1,fontSize:16,fontWeight:600,color:"var(--text)",letterSpacing:"0.02em"}}>{fmtPhone(person.phone)}</span>
...
function Btn(p){var v=p.v||"default";var base={display:"inline-flex",alignItems:"center",gap:7,padding:"10px 20px",borderRadius:14,fontSize:13,fontWeight:600,cursor:"pointer",whiteSpace:"nowrap",border:"none",transition:"all 0.2s"};var styles=v==="ghost"?{background:"var(--inp)",color:"var(--text-sub)",border:"1px solid var(--inp-border)"}:v==="red"?{background:"#FEF2F2",color:"#EF4444",border:"1px solid #FECACA"}:v==="green"?{background:"#ECFDF5",color:"#059669",border:"1px solid #D1FAE5"}:v==="teal"?{background:"#ECFEFF",color:"#0891B2",border:"1px solid #CFFAFE"}:{background:"var(--primary)",color:"#fff",boxShadow:"0 2px 8px var(--primary)30"};return <button style={{...base,...styles,...p.sx}} onClick={p.onClick} onMouseEnter={function(e){e.currentTarget.style.transform="translateY(-1px)"}} onMouseLeave={function(e){e.currentTarget.style.transform="translateY(0)"}}>{p.icon && <I n={p.icon} sz={14}/>}{p.label}</button>}

function Field(p){return <div><div style={{fontSize:10,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.08em",color:"var(--text-muted)",marginBottom:8}}>{p.label}</div><input style={{width:"100%",padding:"12px 16px",borderRadius:12,border:"1px solid var(--inp-border)",background:"var(--inp)",color:"var(--text)",fontSize:13,boxSizing:"border-box",outline:"none",transition:"border-color 0.2s"}} value={p.value} onChange={function(e){p.onChange(e.target.value)}} autoFocus={p.autoFocus} placeholder={p.placeholder}/></div>}
...
function SortBar(p){return <div className="weavr-sort-wrap" style={{display:"flex",gap:6,flexWrap:"wrap"}}>{(p.opts||SORT_OPTS).map(function(o){var active=p.value===o.key;return <button key={o.key} onClick={function(){p.onChange(active?"newest":o.key)}} style={{padding:"6px 14px",borderRadius:10,border:active?"1px solid var(--primary)":"1px solid transparent",background:active?"var(--primary)08":"var(--inp)",fontSize:11,fontWeight:active?700:500,color:active?"var(--primary)":"var(--text-muted)",cursor:"pointer",transition:"all 0.15s"}} onMouseEnter={function(e){if(!active)e.currentTarget.style.background="var(--divider)"}} onMouseLeave={function(e){if(!active)e.currentTarget.style.background="var(--inp)"}}>{o.label}</button>})}</div>}

function Modal(p){return <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,0.5)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",animation:"gentleFade 0.3s ease"}} onClick={p.onClose}><div className="weavr-modal-inner" style={{background:"var(--card-solid)",borderRadius:24,padding:32,width:p.wide?680:480,maxWidth:"92vw",maxHeight:"85vh",overflowY:"auto",boxShadow:"0 32px 80px rgba(0,0,0,0.15)",animation:"gentleFade 0.35s cubic-bezier(0.22,1,0.36,1)"}} onClick={function(e){e.stopPropagation()}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:28}}><h3 style={{fontSize:20,fontWeight:700,color:"var(--text)"}}>{p.title}</h3><button style={{background:"var(--inp)",border:"1px solid var(--inp-border)",borderRadius:12,width:36,height:36,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",transition:"all 0.15s"}} onClick={p.onClose}><I n="x" sz={16} c="var(--text-muted)"/></button></div>{p.children}</div></div>}
...
function MiniCal(p){
  ...
  return <div style={{background:"var(--card)",borderRadius:20,padding:16,border:"1px solid var(--inp-border)"}}>
...
function WeeklyStats(p){
  ...
  return <div style={{background:"var(--card)",borderRadius:20,overflow:"hidden",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
    <div style={{background:"var(--hero)",padding:"24px 28px"}}>
    ...
    <Reveal open={showAdd}><div style={{padding:"16px 24px",background:"var(--inp)",borderBottom:"1px solid var(--divider)"}}>
      <div className="weavr-ws-input" style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10}}>
    ...
    {tab==="week"&&<div>
      {recent.length>0?<div className="weavr-ws-grid" style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10}}>
        {cats.map(function(c){var val=latest[c.key]||0;var delta=getDelta(c.key);var avg=getAvg(c.key);return <div key={c.key} style={{background:c.color+"08",borderRadius:16,padding:"18px 16px",position:"relative",overflow:"hidden",border:"1px solid "+c.color+"12"}}>
    ...
    {tab==="trends"&&<div>
      <div className="weavr-stats-compare" style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
...
function Overview(p){
  ...
  if(!total)return <div style={{display:"flex",alignItems:"center",justifyContent:"center",minHeight:"70vh",textAlign:"center",padding:"20px"}}><div><div style={{width:88,height:88,borderRadius:24,background:"var(--primary)",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 28px",boxShadow:"0 8px 32px var(--primary)25"}}><WeavrLogo sz={44}/></div><h2 style={{fontSize:28,fontWeight:800,color:"var(--text)",marginBottom:10,letterSpacing:"-0.01em"}}>Welcome to Weavr</h2><p style={{fontSize:15,color:"var(--text-muted)",maxWidth:420,margin:"0 auto 36px",lineHeight:1.8}}>Start weaving people into your church community. Add your first person to get started.</p><div style={{display:"flex",gap:12,justifyContent:"center"}}><Btn icon="plus" label="Add Person" onClick={p.onAdd}/><Btn icon="upload" label="Import CSV" onClick={p.onImport} v="ghost"/></div></div></div>;

  var renderWidget=function(key){
    if(key==="next-up"){var top1=mml[0];if(!top1)return null;var stg=STAGES.find(function(s){return s.key===top1.currentStage});var d=ago(top1.lastContactDate);var tm=teams.find(function(t){return t.id===top1.assignedTo});return <div className="weavr-next-up" style={{display:"flex",gap:16,flexWrap:"wrap"}}>
      <div style={{flex:1,minWidth:280,background:"var(--card)",borderRadius:22,padding:"24px 28px",boxShadow:"var(--card-shadow)",display:"flex",alignItems:"center",gap:18,cursor:"pointer",transition:"all 0.2s",border:"1px solid var(--divider)"}} onClick={function(){p.onPerson(top1)}} onMouseEnter={function(e){e.currentTarget.style.transform="translateY(-2px)";e.currentTarget.style.boxShadow="0 4px 20px rgba(0,0,0,0.08)"}} onMouseLeave={function(e){e.currentTarget.style.transform="translateY(0)";e.currentTarget.style.boxShadow="var(--card-shadow)"}}>
        <div style={{width:52,height:52,borderRadius:16,background:stg?stg.grad:"var(--divider)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><I n="zap" sz={22} c="#fff"/></div>
        <div style={{flex:1,minWidth:0}}>
          <div style={{fontSize:9,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.1em",color:"var(--primary)",marginBottom:4}}>Next Up</div>
          <div style={{fontSize:17,fontWeight:700,color:"var(--text)",marginBottom:2}}>{top1.firstName} {top1.lastName}</div>
          <div style={{fontSize:12,color:"var(--text-muted)",display:"flex",alignItems:"center",gap:8}}><span style={{color:stg?stg.color:"#999",fontWeight:600}}>{stg?stg.label:"?"}</span><span>{d===null?"Never contacted":d+"d ago"}</span>{tm&&<span style={{color:tm.color}}>{tm.name}</span>}</div>
        </div>
        <div style={{display:"flex",gap:8,flexShrink:0}}>
          {top1.phone&&<button onClick={function(e){e.stopPropagation();p.onContact(top1)}} style={{width:40,height:40,borderRadius:12,background:"#06B6D408",border:"1px solid #06B6D415",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}><I n="msg" sz={16} c="#06B6D4"/></button>}
          {top1.phone&&<button onClick={function(e){e.stopPropagation();p.onContact(top1)}} style={{width:40,height:40,borderRadius:12,background:"var(--primary)08",border:"1px solid var(--primary)15",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}><I n="phone" sz={16} c="var(--primary)"/></button>}
          {top1.email&&<button onClick={function(e){e.stopPropagation();p.onContact(top1)}} style={{width:40,height:40,borderRadius:12,background:"#10B98108",border:"1px solid #10B98115",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}><I n="mail" sz={16} c="#10B981"/></button>}
        </div>
      </div>
      <div style={{display:"flex",flexDirection:"column",gap:8}}>
        {[{l:"New This Week",v:people.filter(function(x){return ago(x.createdAt)<=7}).length,c:"#10B981"},{l:"Need Follow-Up",v:people.filter(function(x){return!x.fullyConnected&&(ago(x.lastContactDate)===null||ago(x.lastContactDate)>3)}).length,c:"#EF4444"},{l:"Fully Connected",v:people.filter(function(x){return x.fullyConnected}).length,c:"#F59E0B"}].map(function(k){return <div key={k.l} style={{background:k.c+"08",borderRadius:14,padding:"12px 18px",display:"flex",alignItems:"center",gap:10,border:"1px solid "+k.c+"12",minWidth:170}}><div style={{fontSize:22,fontWeight:800,color:k.c}}>{k.v}</div><div style={{fontSize:10,fontWeight:600,color:"var(--text-muted)",lineHeight:1.3}}>{k.l}</div></div>})}
      </div>
    </div>}

    if(key==="mml"&&mml.length>0)return <div style={{background:"var(--card)",borderRadius:22,padding:"16px 24px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
      <button onClick={function(){setMmlOpen(!mmlOpen)}} style={{width:"100%",display:"flex",alignItems:"center",gap:8,background:"none",border:"none",padding:"4px 0",cursor:"pointer",textAlign:"left"}}>
        ...
        <h3 style={{fontSize:16,fontWeight:700,color:"var(--text)",flex:1}}>Monday Morning List</h3>
      ...
      <Reveal open={mmlOpen}><div style={{marginTop:12}}>
        {mml.map(function(x,i){var stg=STAGES.find(function(s){return s.key===x.currentStage});var d=ago(x.lastContactDate);var tm=teams.find(function(t){return t.id===x.assignedTo});return <div key={x.id} draggable onDragStart={function(){onDragStart(i)}} onDragOver={function(e){onDragOver(e,i)}} onDragEnd={onDragEnd} style={{display:"flex",alignItems:"center",gap:14,padding:"14px 8px",borderTop:i>0?"1px solid var(--divider)":"none",cursor:"grab",opacity:dragIdx===i?0.5:1,transition:"all 0.15s",borderRadius:12,margin:"0 -8px"}} onClick={function(){p.onPerson(x)}} onMouseEnter={function(e){e.currentTarget.style.background="var(--hover)"}} onMouseLeave={function(e){e.currentTarget.style.background="transparent"}}><div style={{display:"flex",flexDirection:"column",gap:2,opacity:0.25,cursor:"grab",flexShrink:0,padding:"0 2px"}}><div style={{width:12,height:2,background:"var(--text-muted)",borderRadius:1}}/><div style={{width:12,height:2,background:"var(--text-muted)",borderRadius:1}}/><div style={{width:12,height:2,background:"var(--text-muted)",borderRadius:1}}/></div><div style={{width:32,height:32,borderRadius:10,background:stg?stg.color+"12":"var(--inp)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:700,color:stg?stg.color:"var(--text-muted)",flexShrink:0}}>{(x.firstName||"?").charAt(0)}{(x.lastName||"").charAt(0)}</div><div style={{flex:1,minWidth:0}}><div style={{fontSize:14,fontWeight:600,color:"var(--text)"}}>{x.firstName} {x.lastName}</div><div style={{fontSize:11,color:"var(--text-muted)",display:"flex",alignItems:"center",gap:8,marginTop:2}}><span style={{color:stg?stg.color:"#999",fontWeight:600}}>{stg?stg.label:"?"}</span><span>{d===null?"Never":d+"d ago"}</span>{tm&&<span style={{color:tm.color}}>{tm.name}</span>}</div></div><ScoreRing score={x.engScore} sz={34}/></div>})}
      </div></Reveal>
    </div>;

    if(key==="funnel")return <div style={{background:"var(--card)",borderRadius:22,padding:"26px 28px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
      <h3 style={{fontSize:16,fontWeight:700,color:"var(--text)",marginBottom:24}}>Engagement Funnel</h3>
      {STAGES.map(function(s){var ct=s.key==="bgroup"?people.filter(function(x){return x.stages&&x.stages.bgroup&&x.stages.bgroup.completed}).length:s.key==="ateam"?people.filter(function(x){return x.stages&&x.stages.ateam&&x.stages.ateam.completed}).length:people.filter(function(x){return x.currentStage===s.key}).length;return <div key={s.key} style={{display:"flex",alignItems:"center",gap:16,marginBottom:14,cursor:"pointer",transition:"all 0.15s"}} onClick={function(){p.navTo("people",s.key)}} onMouseEnter={function(e){e.currentTarget.style.opacity="0.8"}} onMouseLeave={function(e){e.currentTarget.style.opacity="1"}}><div style={{width:90,fontSize:12,fontWeight:600,color:"var(--text-sub)",textAlign:"right",flexShrink:0}}>{s.label}</div><div style={{flex:1,height:10,background:"var(--divider)",borderRadius:5,overflow:"hidden"}}><div style={{height:"100%",width:Math.max(ct/mx*100,4)+"%",background:s.color,borderRadius:5,transition:"width 1s ease"}}/></div><div style={{width:36,fontSize:15,fontWeight:800,color:s.color,textAlign:"right"}}>{ct}</div></div>})}
      {(function(){var fc=people.filter(function(x){return x.fullyConnected}).length;if(fc===0)return null;return <div style={{display:"flex",alignItems:"center",gap:16,marginTop:8,cursor:"pointer",padding:"10px 0 0",borderTop:"1px solid var(--divider)"}} onClick={function(){p.navTo("connected")}}><div style={{width:90,fontSize:12,fontWeight:700,color:"#F59E0B",textAlign:"right",flexShrink:0}}>Connected</div><div style={{flex:1,height:10,background:"var(--divider)",borderRadius:5,overflow:"hidden"}}><div style={{height:"100%",width:Math.max(fc/mx*100,4)+"%",background:"#F59E0B",borderRadius:5}}/></div><div style={{width:36,fontSize:15,fontWeight:800,color:"#F59E0B",textAlign:"right"}}>{fc}</div></div>})()}
    </div>;

    if(key==="weekly-stats")return <WeeklyStats config={p.config} setConfig={p.setConfig}/>;

    if(key==="kpis"){var added7=people.filter(function(x){return ago(x.createdAt)<=7}).length;var needFollow=people.filter(function(x){return!x.fullyConnected&&(ago(x.lastContactDate)===null||ago(x.lastContactDate)>3)}).length;var avgScore=total>0?Math.round(people.map(calcScore).reduce(function(a,b){return a+b},0)/total):0;return <div className="weavr-kpi-grid" style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:16}}>{[{l:"Total People",v:total,i:"users",c:"var(--primary)",bg:"var(--primary)"},{l:"Added (7d)",v:added7,i:"plus",c:"#10B981",bg:"#10B981"},{l:"Need Follow-Up",v:needFollow,i:"flag",c:"#EF4444",bg:"#EF4444"},{l:"Avg Score",v:avgScore,i:"target",c:"#06B6D4",bg:"#06B6D4"}].map(function(k){return <div key={k.l} style={{background:k.bg+"0A",borderRadius:20,padding:"24px",position:"relative",overflow:"hidden",border:"1px solid "+k.bg+"15"}}><div style={{position:"absolute",top:16,right:16,width:40,height:40,borderRadius:12,background:k.bg+"12",display:"flex",alignItems:"center",justifyContent:"center"}}><I n={k.i} sz={18} c={k.c}/></div><div style={{fontSize:9,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.08em",color:k.c,marginBottom:10}}>{k.l}</div><div style={{fontSize:32,fontWeight:800,color:"var(--text)",lineHeight:1}}>{k.v}</div></div>})}</div>}

    if(key==="stale"){var stale=people.filter(function(x){return!x.fullyConnected}).filter(function(x){var d=ago(x.lastContactDate);return d===null||d>=14}).sort(function(a,b){var da=ago(a.lastContactDate),db=ago(b.lastContactDate);return(db===null?999:db)-(da===null?999:da)});return <div style={{background:"var(--card)",borderRadius:22,padding:"26px 28px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}><h3 style={{fontSize:16,fontWeight:700,color:"var(--text)",marginBottom:16}}>Stale Contacts (14d+)</h3>{stale.length===0?<div style={{color:"var(--text-muted)",padding:"20px 0",textAlign:"center",fontSize:13}}>No stale contacts</div>:<div style={{maxHeight:300,overflowY:"auto"}}>{stale.slice(0,15).map(function(x,i){var d=ago(x.lastContactDate);var stg=STAGES.find(function(s){return s.key===x.currentStage});return <div key={x.id} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 0",borderTop:i>0?"1px solid var(--divider)":"none",cursor:"pointer"}} onClick={function(){p.onPerson(x)}}><Dot color={stg?stg.color:"#ccc"} sz={8}/><div style={{flex:1,fontSize:12,color:"var(--text)"}}>{x.firstName} {x.lastName}</div><span style={{fontSize:11,fontWeight:600,color:"#EF4444"}}>{d===null?"Never":d+"d"}</span></div>})}</div>}</div>}

    if(key==="recent"){var recentCI=[];people.forEach(function(x){(x.checkIns||[]).forEach(function(c){if(ago(c.date)<=7)recentCI.push({...c,person:x})})});recentCI.sort(function(a,b){return new Date(b.date)-new Date(a.date)});var ciTypes=p.config.checkInTypes||DEFAULT_CI;return <div style={{background:"var(--card)",borderRadius:22,padding:"26px 28px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}><h3 style={{fontSize:16,fontWeight:700,color:"var(--text)",marginBottom:16}}>Recent Check-ins (7d)</h3>{recentCI.length===0?<div style={{color:"var(--text-muted)",padding:"20px 0",textAlign:"center",fontSize:13}}>No recent check-ins</div>:<div style={{maxHeight:300,overflowY:"auto"}}>{recentCI.slice(0,15).map(function(c,i){var ct=ciTypes.find(function(t){return t.key===c.type});return <div key={i} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 0",borderTop:i>0?"1px solid var(--divider)":"none"}}><I n={ct?ct.icon:"msg"} sz={12} c={ct?ct.color:"var(--text-muted)"}/><div style={{flex:1,fontSize:12,color:"var(--text-sub)"}}>{c.person.firstName} {c.person.lastName} - {ct?ct.label:c.type}</div><div style={{fontSize:10,color:"var(--text-muted)"}}>{fmtS(c.date)}</div></div>})}</div>}</div>}

    if(key==="weekly-summary"){var w7=people.filter(function(x){return ago(x.createdAt)<=7}).length;var fu7=0;var adv7=0;var fc7=people.filter(function(x){return x.fullyConnected&&ago(x.fullyConnectedDate)<=7}).length;people.forEach(function(x){(x.checkIns||[]).forEach(function(c){if(ago(c.date)<=7)fu7++});STAGES.forEach(function(s){if(s.key!=="first-visit"&&x.stages&&x.stages[s.key]&&ago(x.stages[s.key].date)<=7)adv7++})});return <div style={{background:"var(--card)",borderRadius:22,padding:"26px 28px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}><h3 style={{fontSize:16,fontWeight:700,color:"var(--text)",marginBottom:16}}>This Week</h3><div className="weavr-weekly-summary-grid" style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:12}}>{[{l:"New People",v:w7,c:"#10B981",i:"plus"},{l:"Follow-ups Done",v:fu7,c:"var(--primary)",i:"check"},{l:"Stage Advances",v:adv7,c:"#06B6D4",i:"up"},{l:"Fully Connected",v:fc7,c:"#F59E0B",i:"target"}].map(function(k){return <div key={k.l} style={{background:k.c+"08",borderRadius:14,padding:"16px 18px",display:"flex",alignItems:"center",gap:12,border:"1px solid "+k.c+"12"}}><div style={{width:40,height:40,borderRadius:12,background:k.c+"15",display:"flex",alignItems:"center",justifyContent:"center"}}><I n={k.i} sz={17} c={k.c}/></div><div><div style={{fontSize:24,fontWeight:800,color:"var(--text)"}}>{k.v}</div><div style={{fontSize:10,color:"var(--text-muted)",fontWeight:600}}>{k.l}</div></div></div>})}</div></div>}

    if(key==="distribution"){var scoreDist=[{label:"0-25 (Cold)",color:"#EF4444",count:people.filter(function(x){var s=calcScore(x);return s>=0&&s<=25}).length},{label:"26-50 (Warming)",color:"#F59E0B",count:people.filter(function(x){var s=calcScore(x);return s>25&&s<=50}).length},{label:"51-75 (Engaged)",color:"#06B6D4",count:people.filter(function(x){var s=calcScore(x);return s>50&&s<=75}).length},{label:"76-100 (Connected)",color:"#10B981",count:people.filter(function(x){var s=calcScore(x);return s>75}).length}];var maxB=Math.max.apply(null,scoreDist.map(function(b){return b.count}).concat([1]));return <div style={{background:"var(--card)",borderRadius:22,padding:"26px 28px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}><h3 style={{fontSize:16,fontWeight:700,color:"var(--text)",marginBottom:16}}>Engagement Distribution</h3>{scoreDist.map(function(b){return <div key={b.label} style={{display:"flex",alignItems:"center",gap:12,marginBottom:10}}><div style={{width:50,fontSize:20,fontWeight:700,color:b.color,textAlign:"right"}}>{b.count}</div><div style={{flex:1}}><div style={{height:28,background:"var(--divider)",borderRadius:8,overflow:"hidden"}}><div style={{height:"100%",width:Math.max(b.count/maxB*100,3)+"%",background:b.color,borderRadius:8}}/></div><div style={{fontSize:10,color:"var(--text-muted)",marginTop:3}}>{b.label}</div></div></div>})}</div>}

    if(key==="velocity"){var stageStats=STAGES.filter(function(s){return s.key!=="bgroup"&&s.key!=="ateam"}).map(function(s){var inStg=people.filter(function(x){return x.currentStage===s.key});var avgD=0;if(inStg.length>0){avgD=Math.round(inStg.reduce(function(a,x){var sd=x.stages&&x.stages[s.key]?x.stages[s.key].date:x.createdAt;return a+(ago(sd)||0)},0)/inStg.length)}return{...s,count:inStg.length,avgDays:avgD}});return <div style={{background:"var(--card)",borderRadius:22,padding:"26px 28px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}><h3 style={{fontSize:16,fontWeight:700,color:"var(--text)",marginBottom:16}}>Stage Velocity</h3>{stageStats.map(function(s){return <div key={s.key} style={{display:"flex",alignItems:"center",gap:12,marginBottom:12}}><Dot color={s.color} sz={8}/><div style={{flex:1,fontSize:13,fontWeight:500,color:"var(--text-sub)"}}>{s.label}</div><div style={{fontSize:11,color:"var(--text-muted)"}}>{s.count}</div><div style={{width:60,textAlign:"right"}}><span style={{fontSize:16,fontWeight:700,color:"var(--text)"}}>{s.avgDays}</span><span style={{fontSize:11,color:"var(--text-muted)"}}>d</span></div></div>})}</div>}

    if(key==="team"){var teamStats=teams.map(function(t){var assigned=people.filter(function(x){return x.assignedTo===t.id});var followedUp=assigned.filter(function(x){return ago(x.lastContactDate)!==null&&ago(x.lastContactDate)<=7}).length;var asc=assigned.length>0?Math.round(assigned.map(calcScore).reduce(function(a,b){return a+b},0)/assigned.length):0;return{...t,assigned:assigned.length,followedUp:followedUp,avgScore:asc}}).filter(function(t){return t.assigned>0});if(teamStats.length===0)return null;return <div style={{background:"var(--card)",borderRadius:22,padding:"26px 28px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}><h3 style={{fontSize:16,fontWeight:700,color:"var(--text)",marginBottom:16}}>Team Performance</h3>{teamStats.map(function(t){var rate=t.assigned>0?Math.round(t.followedUp/t.assigned*100):0;return <div key={t.id} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 14px",background:"var(--inp)",borderRadius:12,marginBottom:6}}><div style={{width:36,height:36,borderRadius:10,background:t.color+"20",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,fontWeight:700,color:t.color}}>{t.name.charAt(0)}</div><div style={{flex:1}}><div style={{fontSize:13,fontWeight:600,color:"var(--text)"}}>{t.name}</div><div style={{fontSize:11,color:"var(--text-muted)"}}>{t.assigned} assigned - {rate}% followed up</div></div><ScoreRing score={t.avgScore} sz={36}/></div>})}</div>}

    return null;
  };

  return <div>
    <div className="weavr-pipeline-hero" style={{background:"var(--hero)",borderRadius:24,padding:"32px 36px",marginBottom:28,position:"relative",overflow:"hidden"}}>
      <div style={{position:"relative",zIndex:1}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:28}}>
          <div><h2 style={{fontSize:20,fontWeight:800,color:"#fff",marginBottom:4,letterSpacing:"-0.01em"}}>Engagement Pipeline</h2><p style={{fontSize:12,color:"rgba(255,255,255,0.45)"}}>{total} people tracked</p></div>
          <button onClick={function(){setShowWidgetPicker(!showWidgetPicker)}} style={{padding:"8px 14px",borderRadius:10,background:"rgba(255,255,255,0.08)",border:"1px solid rgba(255,255,255,0.12)",cursor:"pointer",display:"flex",alignItems:"center",gap:5,fontSize:11,fontWeight:600,color:"rgba(255,255,255,0.6)",backdropFilter:"blur(4px)"}}><I n="gear" sz={12} c="rgba(255,255,255,0.6)"/>Customize</button>
        </div>
        <div className="weavr-pipeline-grid" style={{display:"grid",gridTemplateColumns:"repeat(6,1fr)",gap:12}}>{STAGES.map(function(s,i){var ct=s.key==="bgroup"?people.filter(function(x){return x.stages&&x.stages.bgroup&&x.stages.bgroup.completed}).length:s.key==="ateam"?people.filter(function(x){return x.stages&&x.stages.ateam&&x.stages.ateam.completed}).length:people.filter(function(x){return x.currentStage===s.key}).length;var prev=i>0?people.filter(function(x){return x.currentStage===STAGES[i-1].key}).length:total;var conv=i>0&&prev>0?Math.round(ct/prev*100):null;return <div key={s.key} style={{background:"rgba(255,255,255,0.07)",borderRadius:16,padding:"20px 12px",cursor:"pointer",border:"1px solid rgba(255,255,255,0.06)",textAlign:"center",transition:"all 0.2s"}} onClick={function(){p.navTo("people",s.key)}} onMouseEnter={function(e){e.currentTarget.style.background="rgba(255,255,255,0.14)";e.currentTarget.style.transform="translateY(-2px)"}} onMouseLeave={function(e){e.currentTarget.style.background="rgba(255,255,255,0.07)";e.currentTarget.style.transform="translateY(0)"}}><div style={{fontSize:28,fontWeight:800,color:"#fff",marginBottom:6}}>{ct}</div><div style={{fontSize:11,fontWeight:600,color:"rgba(255,255,255,0.6)",marginBottom:4}}>{s.label}</div>{conv!==null&&<div style={{fontSize:9,fontWeight:700,color:s.color,background:s.color+"20",padding:"2px 8px",borderRadius:6,display:"inline-block"}}>{conv}%</div>}</div>})}</div>
      </div>
    </div>

    <Reveal open={showWidgetPicker}><div style={{background:"var(--card)",borderRadius:22,padding:"24px 28px",marginBottom:24,boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
        <div><div style={{fontSize:15,fontWeight:700,color:"var(--text)"}}>Add Widgets</div><div style={{fontSize:12,color:"var(--text-muted)",marginTop:2}}>Hover any widget to resize, reorder, or remove it.</div></div>
        <button onClick={function(){setShowWidgetPicker(false)}} style={{padding:"8px 16px",borderRadius:10,background:"var(--primary)",color:"#fff",border:"none",fontSize:12,fontWeight:600,cursor:"pointer"}}>Done</button>
      </div>
      <div className="weavr-widget-picker" style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8}}>
        {OV_WIDGETS.map(function(w){var on=ovWidgets.indexOf(w.key)>=0;return <button key={w.key} onClick={function(){toggleWidget(w.key)}} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:6,padding:"14px 10px",borderRadius:14,border:on?"1px solid var(--primary)":"1px solid var(--inp-border)",background:on?"var(--primary)06":"var(--inp)",cursor:"pointer",textAlign:"center",transition:"all 0.15s"}}><div style={{width:32,height:32,borderRadius:10,background:on?"var(--primary)12":"var(--divider)",display:"flex",alignItems:"center",justifyContent:"center"}}><I n={w.icon} sz={14} c={on?"var(--primary)":"var(--text-muted)"}/></div><span style={{fontSize:10,fontWeight:on?600:500,color:on?"var(--primary)":"var(--text-muted)",lineHeight:1.3}}>{w.label}</span></button>})}
      </div>
    </div></Reveal>

    <div className="weavr-ov-grid" style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:20}}>
      {ovWidgets.map(function(key,idx){var el=renderWidget(key);if(!el)return null;var sizes=p.config.widgetSizes||{};var wDef=OV_WIDGETS.find(function(o){return o.key===key});var sz=sizes[key]||(wDef?wDef.defaultSize:"full");var toggleSz=function(){var ns={...(p.config.widgetSizes||{})};ns[key]=sz==="full"?"half":"full";p.setConfig({...p.config,widgetSizes:ns})};return <div key={key} className="weavr-widget-wrap" style={{gridColumn:sz==="full"?"1 / -1":"auto",position:"relative"}} draggable onDragStart={function(){onWDragStart(idx)}} onDragOver={function(e){onWDragOver(e,idx)}} onDragEnd={onWDragEnd}>
        <div className="weavr-widget-ctrl" style={{position:"absolute",top:12,right:12,zIndex:5,display:"flex",gap:4,opacity:0,transition:"opacity 0.2s"}}>
          <button onClick={toggleSz} style={{width:28,height:28,borderRadius:8,background:"var(--card)",border:"1px solid var(--divider)",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",boxShadow:"0 2px 8px rgba(0,0,0,0.08)"}} title={sz==="full"?"Make compact":"Make full width"}><I n={sz==="full"?"grid":"expand"} sz={12} c="var(--text-muted)"/></button>
          <button onClick={function(){toggleWidget(key)}} style={{width:28,height:28,borderRadius:8,background:"var(--card)",border:"1px solid var(--divider)",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",boxShadow:"0 2px 8px rgba(0,0,0,0.08)"}} title="Remove widget"><I n="x" sz={12} c="var(--text-muted)"/></button>
          <div style={{width:28,height:28,borderRadius:8,background:"var(--card)",border:"1px solid var(--divider)",display:"flex",alignItems:"center",justifyContent:"center",cursor:"grab",boxShadow:"0 2px 8px rgba(0,0,0,0.08)"}} title="Drag to reorder"><I n="grip" sz={12} c="var(--text-muted)"/></div>
        </div>
        {el}
      </div>})}
    </div>
...
function PeopleView(p){
  ...
  return <div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
      <div><h2 style={{fontSize:22,fontWeight:800,color:"var(--text)",display:"flex",alignItems:"center",gap:10,letterSpacing:"-0.01em"}}>{stObj&&<Dot color={stObj.color} sz={10}/>}{stObj?stObj.label:"All People"}</h2><p style={{fontSize:13,color:"var(--text-muted)",marginTop:4}}>{fil.length} people</p></div>
    ...
    <div style={{display:"flex",gap:12,alignItems:"center",marginBottom:16}}>
      <div style={{flex:1,display:"flex",alignItems:"center",gap:12,background:"var(--card)",borderRadius:20,padding:"0 18px",border:"1px solid var(--inp-border)"}}><I n="search" sz={16} c="var(--text-muted)"/><input style={{flex:1,padding:"13px 0",border:"none",background:"transparent",color:"var(--text)",fontSize:14,outline:"none"}} placeholder="Search..." value={p.search} onChange={function(e){p.setSearch(e.target.value)}}/>{p.search&&<button style={{background:"none",border:"none",padding:4,cursor:"pointer"}} onClick={function(){p.setSearch("")}}><I n="x" sz={14} c="var(--text-muted)"/></button>}</div>
      <SortBar value={sortKey} onChange={setSortKey}/>
    </div>
    <div style={{background:"var(--card)",borderRadius:22,padding:0,overflow:"hidden",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
      <table style={{width:"100%",borderCollapse:"collapse"}}><thead><tr>{["Name","Stage","Score","Assigned","Status","Contact","Next",""].map(function(h){return <th key={h||"act"} style={{padding:"14px 18px",fontSize:10,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.08em",color:"var(--text-muted)",textAlign:"left",borderBottom:"1px solid var(--divider)",width:h===""?40:"auto"}}>{h}</th>})}</tr></thead>
      <tbody>{fil.map(function(x){var s=STAGES.find(function(st){return st.key===x.currentStage});var d=ago(x.lastContactDate);var urg=d===null||d>3;var sc=calcScore(x);var tm=p.teams.find(function(t){return t.id===x.assignedTo});return <tr key={x.id} style={{cursor:"pointer",transition:"background 0.15s"}} onMouseEnter={function(e){e.currentTarget.style.background="var(--hover)"}} onMouseLeave={function(e){e.currentTarget.style.background=""}}>
        <td style={{padding:"14px 18px",borderBottom:"1px solid var(--divider)"}} onClick={function(){p.onPerson(x)}}><div style={{display:"flex",alignItems:"center",gap:12}}><div style={{width:36,height:36,borderRadius:12,background:s?s.color+"10":"var(--inp)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:700,color:s?s.color:"var(--text-muted)",flexShrink:0}}>{(x.firstName||"?").charAt(0)}{(x.lastName||"").charAt(0)}</div><span style={{fontSize:14,fontWeight:600,color:"var(--text)"}}>{x.firstName} {x.lastName}</span></div></td>
        <td style={{padding:"14px 18px",borderBottom:"1px solid var(--divider)"}} onClick={function(){p.onPerson(x)}}><span style={{display:"inline-flex",alignItems:"center",gap:6,fontSize:12,fontWeight:600,color:s?s.color:"#999",background:s?s.color+"10":"var(--inp)",padding:"4px 10px",borderRadius:8}}><Dot color={s?s.color:"#ccc"} sz={6}/>{s?s.label:"?"}</span></td>
        <td style={{padding:"14px 18px",borderBottom:"1px solid var(--divider)"}} onClick={function(){p.onPerson(x)}}><ScoreRing score={sc} sz={34}/></td>
        <td style={{padding:"14px 18px",fontSize:12,borderBottom:"1px solid var(--divider)"}} onClick={function(){p.onPerson(x)}}>{tm?<span style={{color:tm.color,fontWeight:600}}>{tm.name}</span>:<span style={{color:"var(--text-muted)"}}>-</span>}</td>
        <td style={{padding:"14px 18px",borderBottom:"1px solid var(--divider)"}} onClick={function(){p.onPerson(x)}}>{urg?<span style={{fontSize:11,fontWeight:600,padding:"4px 12px",borderRadius:8,background:"#FEF2F2",color:"#EF4444",border:"1px solid #FECACA"}}>Follow Up</span>:<span style={{fontSize:11,fontWeight:600,padding:"4px 12px",borderRadius:8,background:"#ECFDF5",color:"#10B981",border:"1px solid #D1FAE5"}}>On Track</span>}</td>
        <td style={{padding:"14px 18px",fontSize:12,color:"var(--text-muted)",borderBottom:"1px solid var(--divider)"}} onClick={function(){p.onPerson(x)}}>{x.lastContactDate?fmtS(x.lastContactDate):"Never"}</td>
        <td style={{padding:"14px 18px",fontSize:12,color:"var(--text-sub)",borderBottom:"1px solid var(--divider)"}} onClick={function(){p.onPerson(x)}}>{NEXT_ACT[x.currentStage]}</td>
        <td style={{padding:"14px 18px",borderBottom:"1px solid var(--divider)",textAlign:"center"}}><button onClick={function(e){e.stopPropagation();if(confirm("Delete "+x.firstName+" "+x.lastName+"?"))p.onDelete(x.id)}} style={{background:"none",border:"none",padding:4,cursor:"pointer",opacity:0.3}} onMouseEnter={function(e){e.currentTarget.style.opacity="1"}} onMouseLeave={function(e){e.currentTarget.style.opacity="0.3"}}><I n="trash" sz={14} c="#EF4444"/></button></td>
      </tr>})}</tbody></table>
...
function AssignedCards(p){
  ...
  return <div>
    <div className="weavr-ac-header" style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
      <div><h2 style={{fontSize:22,fontWeight:800,color:"var(--text)",display:"flex",alignItems:"center",gap:10,letterSpacing:"-0.01em"}}><I n="card" sz={22} c="var(--primary)"/>Assigned Cards</h2><p style={{fontSize:13,color:"var(--text-muted)",marginTop:4}}>View and work contacts by team member</p></div>
      <div style={{display:"flex",gap:6}}>
    ...
    <div className="weavr-ac-teams" style={{display:"flex",gap:8,marginBottom:20,flexWrap:"wrap"}}>
    ...
    {p.teams.length===0?<div style={{background:"var(--card)",borderRadius:22,padding:40,textAlign:"center",color:"var(--text-muted)"}}><I n="users" sz={28} c="var(--text-muted)"/><div style={{fontWeight:600,marginTop:10}}>Add team members in Settings first</div></div>:
    workMode&&workPerson?<div style={{background:"var(--card)",borderRadius:22,overflow:"hidden",boxShadow:"0 4px 24px rgba(0,0,0,0.06)",animation:"gentleFade 0.4s cubic-bezier(0.22,1,0.36,1)"}}>
      ...
      <div style={{padding:"20px 32px"}}>
        ...
        {workPerson.currentStage==="next-steps"&&<div style={{background:"var(--inp)",borderRadius:12,padding:"14px 16px",marginBottom:14}}>
    ...
    workMode&&!workPerson?<div style={{background:"var(--card)",borderRadius:22,padding:48,textAlign:"center",color:"var(--text-muted)"}}><I n="check" sz={32} c="#10B981"/><div style={{fontWeight:600,marginTop:12,color:"#10B981"}}>All caught up!</div><div style={{fontSize:12,marginTop:4}}>No pending contacts for {tm?tm.name:"this team"}</div></div>:
    acView==="list"?<div style={{background:"var(--card)",borderRadius:22,overflow:"hidden",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(300px,1fr))",gap:18}}>
      {assigned.map(function(x){
        ...
        return <div key={x.id} style={{background:"var(--card)",borderRadius:20,padding:"24px",cursor:"pointer",border:"1px solid var(--divider)",boxShadow:"var(--card-shadow)",transition:"all 0.2s"}} onClick={function(){p.onPerson(x)}} onMouseEnter={function(e){e.currentTarget.style.transform="translateY(-3px)";e.currentTarget.style.boxShadow="0 8px 24px rgba(0,0,0,0.08)"}} onMouseLeave={function(e){e.currentTarget.style.transform="translateY(0)";e.currentTarget.style.boxShadow="var(--card-shadow)"}}>
          <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:16}}>
            <div style={{width:44,height:44,borderRadius:14,background:stg?stg.color+"12":"var(--inp)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,fontWeight:700,color:stg?stg.color:"var(--text-muted)",flexShrink:0}}>{(x.firstName||"?").charAt(0)}{(x.lastName||"").charAt(0)}</div>
            <div style={{flex:1,minWidth:0}}><div style={{fontSize:15,fontWeight:700,color:"var(--text)"}}>{x.firstName} {x.lastName}</div><div style={{display:"flex",alignItems:"center",gap:6,marginTop:3}}><span style={{fontSize:11,fontWeight:600,color:stg?stg.color:"var(--text-muted)"}}>{stg?stg.label:"?"}</span><span style={{fontSize:11,color:"var(--text-muted)"}}>{d===null?"Never contacted":d+"d ago"}</span></div></div>
            <ScoreRing score={sc} sz={38}/>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            <div style={{flex:1,fontSize:12,color:"var(--text-sub)",background:stg?stg.color+"08":"var(--inp)",border:"1px solid "+(stg?stg.color+"12":"var(--inp-border)"),borderRadius:10,padding:"8px 12px",display:"flex",alignItems:"center",gap:6}}><I n="zap" sz={12} c={stg?stg.color:"var(--text-muted)"}/>{NEXT_ACT[x.currentStage]}</div>
          </div>
          {x.phone&&<div style={{fontSize:12,color:"var(--text-muted)",marginTop:10,display:"flex",alignItems:"center",gap:6}}><I n="phone" sz={12} c="var(--text-muted)"/>{fmtPhone(x.phone)}</div>}
        </div>;
      })}
      {assigned.length===0&&<div style={{background:"var(--card)",borderRadius:20,padding:40,textAlign:"center",color:"var(--text-muted)",gridColumn:"1/-1"}}>{tm?"No pending contacts for "+tm.name:"Select a team member"}</div>}
    </div>}
  </div>;
}
...
function QuickEntry(p){
  ...
  return <div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:24}}>
      <div><h2 style={{fontSize:22,fontWeight:800,color:"var(--text)",display:"flex",alignItems:"center",gap:10}}><I n="card" sz={22} c="var(--primary)"/>Quick Entry</h2><p style={{fontSize:12,color:"var(--text-muted)",marginTop:3}}>Rapid connection card entry</p></div>
    ...
    <div className="weavr-qe-layout" style={{display:"flex",gap:20,alignItems:"flex-start"}}>
      <div style={{flex:1,background:"var(--card)",borderRadius:22,padding:"26px 28px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
          {enabled.map(function(fd,i){
            ...
            return <div key={fd.key}><div style={{fontSize:10,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.08em",color:"var(--text-muted)",marginBottom:6}}>{fd.label}{fd.required?" *":""}</div><input ref={i===0?firstRef:undefined} style={{width:"100%",padding:"10px 14px",borderRadius:10,border:"1px solid var(--inp-border)",background:"var(--inp)",color:"var(--text)",fontSize:13,boxSizing:"border-box",outline:"none"}} value={fd.key==="phone"?fmtPhone(f[fd.key]||""):(f[fd.key]||"")} onChange={function(e){var nf={...f};nf[fd.key]=fd.key==="phone"?e.target.value.replace(/\D/g,"").slice(0,10):e.target.value;setF(nf)}} placeholder={fd.label} onKeyDown={function(e){if(e.key==="Enter")sub()}}/></div>;
          })}
      ...
      {recent.length>0&&<div className="weavr-qe-recent" style={{width:320,flexShrink:0}}>
        ...
        <div style={{background:"var(--card)",borderRadius:14,padding:"8px",maxHeight:500,overflowY:"auto",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
          {recent.map(function(x,i){var stg=STAGES.find(function(s){return s.key===x.currentStage});return <div key={x.id} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",borderRadius:10,background:i===0?"var(--primary)06":"transparent",borderBottom:i<recent.length-1?"1px solid var(--divider)":"none"}}>
            ...
            <div style={{flex:1,minWidth:0}}>
              ...
              <div style={{fontSize:10,color:"var(--text-muted)"}}>{stg?stg.label:"?"}{x.phone?" \u00B7 "+fmtPhone(x.phone):""}</div>
            </div>
...
function BulkMessage(p){
  ...
  return <div>
    <h2 style={{fontSize:22,fontWeight:800,color:"var(--text)",display:"flex",alignItems:"center",gap:10,marginBottom:4}}><I n="send" sz={22} c="var(--primary)"/>Bulk Message</h2>
    ...
    <div style={{background:"var(--card)",borderRadius:22,padding:"26px 28px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
        <div><h3 style={{fontSize:16,fontWeight:700,color:"var(--text)"}}>{stg?stg.label:""} Messages</h3><p style={{fontSize:12,color:"var(--text-muted)"}}>{filtered.length} people with phone numbers</p></div>
      ...
      <div style={{maxHeight:500,overflowY:"auto"}}>{msgs.map(function(m,i){return <div key={m.person.id} style={{padding:"14px 0",borderTop:i>0?"1px solid var(--divider)":"none"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
          <div style={{fontSize:13,fontWeight:600,color:"var(--text)"}}>{m.person.firstName} {m.person.lastName} <span style={{fontWeight:400,color:"var(--text-muted)"}}>{fmtPhone(m.person.phone)}</span></div>
          <div style={{display:"flex",gap:6}}>
...
function Reports(p){
  ...
  return <div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
      <h2 style={{fontSize:22,fontWeight:800,color:"var(--text)",display:"flex",alignItems:"center",gap:10}}><I n="chart" sz={22} c="var(--primary)"/>Reports</h2>
    </div>
    ...
    {widgets.includes("kpis")&&<div className="weavr-kpi-grid" style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:16,marginBottom:20}}>{[{l:"Total People",v:total,i:"users",c:"var(--primary)",bg:"var(--primary)"},{l:"Added (7d)",v:added7,i:"plus",c:"#10B981",bg:"#10B981"},{l:"Need Follow-Up",v:needFollow,i:"flag",c:"#EF4444",bg:"#EF4444"},{l:"Avg Score",v:avgScore,i:"target",c:"#06B6D4",bg:"#06B6D4"}].map(function(k){return <div key={k.l} style={{background:k.bg+"0A",borderRadius:20,padding:"24px",position:"relative",overflow:"hidden",border:"1px solid "+k.bg+"15"}}><div style={{position:"absolute",top:16,right:16,width:40,height:40,borderRadius:12,background:k.bg+"12",display:"flex",alignItems:"center",justifyContent:"center"}}><I n={k.i} sz={18} c={k.c}/></div><div style={{fontSize:9,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.08em",color:k.c,marginBottom:10}}>{k.l}</div><div style={{fontSize:32,fontWeight:800,color:"var(--text)",lineHeight:1}}>{k.v}</div></div>})}</div>}
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20}}>
      ...
      {widgets.includes("team")&&teamStats.length>0&&<div style={{background:"var(--card)",borderRadius:22,padding:"26px 28px",boxShadow:"var(--card-shadow)",gridColumn:widgets.includes("recent")||widgets.includes("stale")?"auto":"span 2"}}><h3 style={{fontSize:16,fontWeight:700,color:"var(--text)",marginBottom:16}}>Team Performance</h3>{teamStats.map(function(t){var rate=t.assigned>0?Math.round(t.followedUp/t.assigned*100):0;return <div key={t.id} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 14px",background:"var(--inp)",borderRadius:12,marginBottom:6}}><div style={{width:36,height:36,borderRadius:10,background:t.color+"20",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,fontWeight:700,color:t.color}}>{t.name.charAt(0)}</div><div style={{flex:1}}><div style={{fontSize:13,fontWeight:600,color:"var(--text)"}}>{t.name}</div><div style={{fontSize:11,color:"var(--text-muted)"}}>{t.assigned} assigned - {rate}% followed up</div></div><ScoreRing score={t.avgScore} sz={36}/></div>})}</div>}
...
function FullyConnected(p){
  ...
  return <div>
    <div className="weavr-fc-header" style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
      <div style={{textAlign:"left"}}>
        <h2 style={{fontSize:22,fontWeight:800,color:"var(--text)",display:"flex",alignItems:"center",gap:10}}>{"⭐"} Fully Connected</h2>
    ...
    {connected.length===0?<div style={{background:"var(--card)",borderRadius:22,padding:48,textAlign:"center",color:"var(--text-muted)",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}><div style={{fontSize:15,fontWeight:600}}>No one fully connected yet</div><p style={{fontSize:12,marginTop:6}}>When someone joins both a BGroup and the ATeam, they'll appear here.</p></div>:
    fcView==="list"?<div style={{background:"var(--card)",borderRadius:22,overflow:"hidden",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(320px,1fr))",gap:18}}>
      {connected.map(function(x){
        ...
        return <div key={x.id} style={{background:"var(--card)",borderRadius:22,padding:"24px",cursor:"pointer",boxShadow:"var(--card-shadow)",border:"1px solid #F59E0B20",transition:"all 0.2s"}} onClick={function(){p.onPerson(x)}} onMouseEnter={function(e){e.currentTarget.style.transform="translateY(-2px)";e.currentTarget.style.borderColor="#F59E0B40"}} onMouseLeave={function(e){e.currentTarget.style.transform="translateY(0)";e.currentTarget.style.borderColor="#F59E0B20"}}>
          <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:16}}>
            <div style={{width:44,height:44,borderRadius:14,background:"#F59E0B10",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,fontWeight:700,color:"#F59E0B"}}>{(x.firstName||"?").charAt(0)}{(x.lastName||"").charAt(0)}</div>
            <div style={{flex:1}}><div style={{fontSize:16,fontWeight:700,color:"var(--text)"}}>{x.firstName} {x.lastName}</div><div style={{fontSize:11,color:"var(--text-muted)"}}>{fmt(x.fullyConnectedDate)}</div></div>
            <ScoreRing score={sc} sz={36}/>
          </div>
          <div style={{display:"flex",gap:10}}>
            <div style={{flex:1,background:"#EC489908",borderRadius:12,padding:"10px 14px",border:"1px solid #EC489912"}}><div style={{fontSize:9,fontWeight:700,color:"#EC4899",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:3}}>BGroup</div><div style={{fontSize:13,fontWeight:600,color:"var(--text)"}}>{ns.bGroupLeader||"Connected"}</div></div>
            <div style={{flex:1,background:"#34D39908",borderRadius:12,padding:"10px 14px",border:"1px solid #34D39912"}}><div style={{fontSize:9,fontWeight:700,color:"#34D399",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:3}}>ATeam</div><div style={{fontSize:13,fontWeight:600,color:"var(--text)"}}>{ns.aTeamArea||"Serving"}</div></div>
          </div>
        </div>;
      })}
    </div>}
...
function Settings(p){
  ...
  return <div>
    <h2 style={{fontSize:22,fontWeight:800,color:"var(--text)",marginBottom:20}}>Settings</h2>
    ...
    {tab==="teams"&&<div style={{background:"var(--card)",borderRadius:22,padding:"26px 28px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
      <h3 style={{fontSize:16,fontWeight:700,color:"var(--text)",marginBottom:16}}>Team Members</h3>
      ...
      {p.teams.map(function(t){var ct=p.people.filter(function(x){return x.assignedTo===t.id}).length;return <div key={t.id} style={{display:"flex",alignItems:"center",gap:12,padding:"14px 16px",background:"var(--inp)",borderRadius:12,marginBottom:6}}><div style={{width:36,height:36,borderRadius:10,background:t.color+"20",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,fontWeight:700,color:t.color}}>{t.name.charAt(0)}</div><div style={{flex:1}}><div style={{fontSize:13,fontWeight:600,color:"var(--text)"}}>{t.name}</div><div style={{fontSize:11,color:"var(--text-muted)"}}>{t.role||"Team Member"} - {ct} assigned</div></div><button onClick={function(){p.setTeams(function(prev){return prev.filter(function(x){return x.id!==t.id})})}} style={{background:"none",border:"none",padding:4,cursor:"pointer"}}><I n="trash" sz={14} c="#EF4444"/></button></div>})}
      {p.teams.length===0&&<div style={{color:"var(--text-muted)",fontSize:13,padding:"20px 0",textAlign:"center"}}>No team members yet</div>}
    </div>}

    {tab==="templates"&&<div style={{background:"var(--card)",borderRadius:22,padding:"26px 28px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
      <h3 style={{fontSize:16,fontWeight:700,color:"var(--text)",marginBottom:16}}>Message Templates</h3>
    ...
    {tab==="email-tpl"&&<div style={{background:"var(--card)",borderRadius:22,padding:"26px 28px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
      <h3 style={{fontSize:16,fontWeight:700,color:"var(--text)",marginBottom:4}}>Email Templates</h3>
    ...
    {tab==="sequences"&&<div style={{background:"var(--card)",borderRadius:22,padding:"26px 28px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
      <h3 style={{fontSize:16,fontWeight:700,color:"var(--text)",marginBottom:4}}>Message Sequences</h3>
    ...
    {tab==="automation"&&<div style={{background:"var(--card)",borderRadius:22,padding:"26px 28px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
      <h3 style={{fontSize:16,fontWeight:700,color:"var(--text)",marginBottom:4}}>Automation Rules</h3>
      ...
      {p.rules.map(function(r){var desc="";if(r.trigger==="days-no-contact")desc="No contact for "+r.days+"d";else if(r.trigger==="stage-reached"){var stg=STAGES.find(function(s){return s.key===r.stage});desc=(stg?stg.label:r.stage)+" reached"}else if(r.trigger==="score-below")desc="Score below "+r.days;else desc="Check-in logged";var act=r.action==="escalate"?"Escalate":r.action==="advance"?"Auto-advance":r.action==="assign"?"Auto-assign":"Add reminder";return <div key={r.id} style={{display:"flex",alignItems:"center",gap:12,padding:"14px 16px",background:"var(--inp)",borderRadius:12,marginBottom:6}}><I n="zap" sz={16} c={r.enabled?"var(--primary)":"var(--text-muted)"}/><div style={{flex:1,fontSize:13,fontWeight:500,color:r.enabled?"var(--text)":"var(--text-muted)"}}>{desc} \u2192 {act}</div><button onClick={function(){p.setRules(function(prev){return prev.map(function(x){return x.id===r.id?{...x,enabled:!x.enabled}:x})})}} style={{background:"none",border:"none",padding:4,cursor:"pointer",fontSize:11,fontWeight:600,color:r.enabled?"#10B981":"var(--text-muted)"}}>{r.enabled?"Active":"Off"}</button><button onClick={function(){p.setRules(function(prev){return prev.filter(function(x){return x.id!==r.id})})}} style={{background:"none",border:"none",padding:4,cursor:"pointer"}}><I n="trash" sz={14} c="#EF4444"/></button></div>})}
      {p.rules.length===0&&<div style={{color:"var(--text-muted)",fontSize:13,padding:"20px 0",textAlign:"center"}}>No automation rules yet</div>}
    </div>}

    {tab==="checkins"&&<div style={{background:"var(--card)",borderRadius:22,padding:"26px 28px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
      <h3 style={{fontSize:16,fontWeight:700,color:"var(--text)",marginBottom:16}}>Check-In Types</h3>
    ...
    {tab==="forms"&&<div style={{background:"var(--card)",borderRadius:22,padding:"26px 28px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
      <h3 style={{fontSize:16,fontWeight:700,color:"var(--text)",marginBottom:4}}>Form Fields</h3>
    ...
    {tab==="visual"&&<div style={{background:"var(--card)",borderRadius:22,padding:"26px 28px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
      <h3 style={{fontSize:16,fontWeight:700,color:"var(--text)",marginBottom:16}}>Visual Settings</h3>
      <div style={{marginBottom:24}}>
        ...
        <div style={{display:"flex",gap:10}}>
          {[{k:"light",label:"Light",bg:"#F0F2F5",fg:"#1E1B4B"},{k:"dark",label:"Dark",bg:"#1E1B4B",fg:"#F1F5F9"},{k:"mono",label:"Simple",bg:"#F8F9FA",fg:"#212529"}].map(function(m){return <button key={m.k} onClick={function(){p.setConfig({...config,theme:m.k})}} style={{flex:1,padding:"16px",borderRadius:14,border:(config.theme||"light")===m.k?"2px solid var(--primary)":"2px solid var(--inp-border)",background:m.bg,cursor:"pointer",textAlign:"center"}}><div style={{fontSize:13,fontWeight:600,color:m.fg}}>{m.label}</div><div style={{fontSize:9,color:m.fg,opacity:0.5,marginTop:3}}>{m.k==="mono"?"Clean & minimal":m.k==="dark"?"Rich & immersive":"Bright & airy"}</div></button>})}
        </div>
      </div>
...
function Panel(p){
  ...
  return <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,0.5)",zIndex:200,display:"flex",justifyContent:"flex-end"}} onClick={p.onClose}><div className="weavr-panel" style={{width:560,maxWidth:"100vw",height:"100vh",background:"var(--card-solid)",display:"flex",flexDirection:"column",animation:"panelSlide 0.4s cubic-bezier(0.22,1,0.36,1)",overflowY:"auto",boxShadow:"-12px 0 48px rgba(0,0,0,0.12)"}} onClick={function(e){e.stopPropagation()}}>
    <div className="weavr-panel-header" style={{padding:"24px 24px 20px",background:"var(--hero)",position:"relative"}}>
      <button style={{position:"absolute",top:12,left:10,background:"none",border:"none",borderRadius:6,width:20,height:20,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",zIndex:2,opacity:0.4,transition:"opacity 0.15s"}} onMouseEnter={function(e){e.currentTarget.style.opacity="1"}} onMouseLeave={function(e){e.currentTarget.style.opacity="0.4"}} onClick={p.onClose}><I n="x" sz={11} c="rgba(255,255,255,0.8)"/></button>
      <div style={{display:"flex",gap:16,alignItems:"center",paddingLeft:16}}>
        <div style={{width:56,height:56,borderRadius:18,background:"rgba(255,255,255,0.15)",border:"2px solid rgba(255,255,255,0.25)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,fontWeight:700,color:"#fff",flexShrink:0,backdropFilter:"blur(4px)"}}>{(person.firstName||"?").charAt(0)}{(person.lastName||"").charAt(0)}</div>
        <div style={{flex:1}}>
          <h2 style={{fontSize:22,fontWeight:800,color:"#fff",letterSpacing:"-0.01em"}}>{person.firstName} {person.lastName}</h2>
          <div style={{display:"flex",alignItems:"center",gap:8,marginTop:4}}><span style={{fontSize:11,fontWeight:700,color:stColor,background:stColor+"25",padding:"2px 8px",borderRadius:6}}>{stLabel}</span>{tm&&<span style={{fontSize:11,color:"rgba(255,255,255,0.5)"}}>Assigned to <span style={{color:tm.color,fontWeight:600}}>{tm.name}</span></span>}</div>
        </div>
        <div style={{textAlign:"center"}}><RingMini value={score} max={100} color={scoreColor(score)} sz={52}/><div style={{fontSize:9,color:"rgba(255,255,255,0.45)",marginTop:4,fontWeight:600}}>{scoreLabel(score)}</div></div>
      </div>
    </div>
    <div style={{padding:"12px 24px",background:"var(--primary)05",borderBottom:"1px solid var(--divider)",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
    ...
    <div className="weavr-panel-actions" style={{padding:"12px 24px",borderBottom:"1px solid var(--divider)",background:"var(--card-solid)",position:"sticky",top:0,zIndex:10}}>
      <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
        <Btn icon="check" label={justFollowed?"Logged!":"Followed Up"} onClick={function(){if(justFollowed)return;setShowFollowUp(!showFollowUp)}} v={justFollowed?"green":"green"} sx={{padding:"7px 14px",fontSize:11}}/>
        <div style={{display:"inline-flex",alignItems:"center",gap:0,borderRadius:12,overflow:"hidden",border:"1px solid var(--inp-border)"}}>
          {linIdx>0&&<button onClick={stepBack} style={{display:"inline-flex",alignItems:"center",padding:"7px 10px",cursor:"pointer",border:"none",background:"var(--inp)",color:"var(--text-sub)",borderRight:"1px solid var(--inp-border)"}} title="Step Back"><span style={{transform:"rotate(180deg)",display:"inline-flex"}}><I n="up" sz={12} c="var(--text-sub)"/></span></button>}
          {linIdx<linearStages.length-1&&<button onClick={adv} style={{display:"inline-flex",alignItems:"center",gap:4,padding:"7px 12px",cursor:"pointer",border:"none",background:"var(--primary)",color:"#fff",fontSize:11,fontWeight:600}}><I n="up" sz={12} c="#fff"/>Advance</button>}
        </div>
        <Btn icon="msg" label="Text" onClick={function(){setContactOpen("text")}} sx={{padding:"7px 14px",fontSize:11}}/>
        <Btn icon="mail" label="Email" onClick={function(){setContactOpen("email")}} v="ghost" sx={{padding:"7px 14px",fontSize:11,opacity:person.email?1:0.4}}/>
        <Btn icon="cal" label="" onClick={function(){setShowCal(!showCal)}} v="ghost" sx={{padding:"7px 10px"}}/>
        <Btn icon="edit" label="" onClick={function(){setEdit(!edit)}} v="ghost" sx={{padding:"7px 10px"}}/>
        <Btn icon="dl" label="" onClick={exportReport} v="ghost" sx={{padding:"7px 10px"}}/>
        <button onClick={function(){if(confirm("Delete "+person.firstName+"?")){p.onDelete(person.id);p.onClose()}}} style={{display:"inline-flex",alignItems:"center",padding:"7px 10px",borderRadius:12,cursor:"pointer",background:"#FEF2F2",color:"#EF4444",border:"1px solid #FECACA"}}><I n="trash" sz={12} c="#EF4444"/></button>
      </div>
    </div>
    <div className="weavr-panel-body" style={{padding:"20px 24px",overflowY:"auto",flex:1}}>
      ...
      <Reveal open={showCal}><div style={{background:"var(--card)",borderRadius:14,padding:"14px 18px",marginBottom:16,boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
      ...
      <Reveal open={showFollowUp}><div style={{marginBottom:16}}>
        <div style={{background:"var(--card)",borderRadius:14,padding:"14px 16px",border:"1px solid var(--primary)20",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
      ...
      {!edit&&<div style={{marginBottom:16}}>
        ...
        <div style={{background:"var(--card)",borderRadius:14,padding:"12px 16px",display:"flex",flexDirection:"column",gap:8,boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
          {person.phone&&<div style={{display:"flex",alignItems:"center",gap:6}}><div style={{color:"var(--text)",fontSize:13,display:"flex",alignItems:"center",gap:10,fontWeight:500,flex:1}}><div style={{width:28,height:28,borderRadius:8,background:"var(--primary)15",display:"flex",alignItems:"center",justifyContent:"center"}}><I n="phone" sz={12} c="var(--primary)"/></div>{fmtPhone(person.phone)}</div><button onClick={function(){setContactOpen("text")}} style={{display:"flex",alignItems:"center",gap:4,padding:"4px 10px",borderRadius:8,background:"#06B6D415",fontSize:11,fontWeight:600,color:"#06B6D4",border:"none",cursor:"pointer"}}>iMessage</button></div>}
      ...
      <Reveal open={edit}><div style={{background:"var(--card)",borderRadius:14,padding:"14px 18px",marginBottom:16,boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
      ...
      <div style={{marginBottom:16}}>
        ...
        <div style={{background:"var(--card)",borderRadius:14,padding:"12px 16px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
...
function ImportModal(p){
  ...
  return <Modal title="Import CSV" onClose={p.onClose} wide>
    ...
    <button onClick={function(){ref.current.click()}} style={{width:"100%",padding:28,borderRadius:20,border:"2px dashed var(--inp-border)",background:"var(--inp)",color:"var(--text-muted)",fontSize:14,fontWeight:600,cursor:"pointer",marginBottom:12}}>Choose CSV file</button>
      ...
      <div style={{padding:sbCollapsed?"20px 0":"20px 18px",borderBottom:"1px solid var(--sb-divider)",display:"flex",alignItems:"center",justifyContent:sbCollapsed?"center":"flex-start",gap:sbCollapsed?0:10,minHeight:72,position:"relative",zIndex:80}}>
        <div style={{position:"relative",width:sbCollapsed?"auto":"100%"}}>
          <button onClick={function(){setToolDrop(!toolDrop)}} style={{display:"flex",alignItems:"center",gap:sbCollapsed?0:10,background:"none",border:"none",cursor:"pointer",padding:0,width:sbCollapsed?"auto":"100%"}}>
            <div style={{width:36,height:36,borderRadius:11,background:"var(--logo)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><WeavrLogo sz={20}/></div>
            {!sbCollapsed&&<div style={{flex:1,display:"flex",alignItems:"center",gap:6}}><div style={{fontSize:16,fontWeight:800,color:"var(--sb-text)",letterSpacing:"0.12em"}}>WEAVR</div><span style={{fontSize:9,fontWeight:600,color:"var(--sb-muted)",background:"var(--sb-active)",padding:"2px 6px",borderRadius:4,letterSpacing:"0.04em"}}>Connect</span></div>}
          ...
          {toolDrop&&<div style={{position:"absolute",top:"100%",left:sbCollapsed?-18:-18,right:sbCollapsed?"auto":-18,width:sbCollapsed?72:"auto",marginTop:1,background:isDark?"#1A1744":"#2E2A8A",borderRadius:"0 0 14px 14px",padding:sbCollapsed?"6px 4px":"6px 8px",zIndex:200,boxShadow:"0 8px 24px rgba(0,0,0,0.3)",borderTop:"1px solid rgba(255,255,255,0.06)"}}>
            {TOOLS.map(function(t){var isCurrent=t.key==="connect";return <button key={t.key} onClick={t.ready?function(){setToolDrop(false);if(!isCurrent)alert("Coming soon: Weavr "+t.name)}:undefined} style={{display:"flex",alignItems:"center",justifyContent:sbCollapsed?"center":"flex-start",gap:sbCollapsed?0:8,width:"100%",padding:sbCollapsed?"6px 0":"6px 8px",borderRadius:8,border:"none",background:isCurrent?"rgba(255,255,255,0.08)":"transparent",cursor:t.ready?"pointer":"default",opacity:t.ready?1:0.6,textAlign:"left",transition:"all 0.15s"}} onMouseEnter={t.ready?function(e){if(!isCurrent)e.currentTarget.style.background="rgba(255,255,255,0.06)"}:undefined} onMouseLeave={t.ready?function(e){if(!isCurrent)e.currentTarget.style.background="transparent"}:undefined}>
              <div style={{width:22,height:22,borderRadius:6,background:isCurrent?t.color:t.color+"25",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><I n={t.icon} sz={10} c={isCurrent?"#fff":t.color}/></div>
              {!sbCollapsed&&<span style={{flex:1,fontSize:11,fontWeight:isCurrent?600:400,color:isCurrent?"rgba(255,255,255,0.9)":"rgba(255,255,255,0.5)"}}>{t.name}</span>}
              {!sbCollapsed&&isCurrent&&<div style={{width:4,height:4,borderRadius:"50%",background:"#10B981"}}/>}
            </button>})}
            <button onClick={function(){setToolDrop(false);p.onBackToPortal()}} style={{display:"flex",alignItems:"center",justifyContent:sbCollapsed?"center":"flex-start",gap:sbCollapsed?0:8,width:"100%",padding:sbCollapsed?"6px 0":"6px 8px",borderRadius:8,border:"none",background:"transparent",cursor:"pointer",textAlign:"left",marginTop:2,borderTop:"1px solid rgba(255,255,255,0.06)",paddingTop:8}}><I n="home" sz={10} c="rgba(255,255,255,0.35)"/>{!sbCollapsed&&<span style={{fontSize:11,color:"rgba(255,255,255,0.35)"}}>Portal</span>}</button>
          </div>}
        </div>
      </div>
      <nav style={{flex:1,padding:sbCollapsed?"14px 8px":"14px 14px",overflowY:"auto",overflowX:"hidden"}}>{sideItems.map(function(item,i){if(item.type==="divider")return <div key={"d"+i} style={{height:1,background:"var(--sb-divider)",margin:sbCollapsed?"8px 4px":"10px 14px"}}/>;var active=activeKey===item.key;return <button key={item.key} onClick={function(){nav(item.vw,item.sf)}} onMouseEnter={function(e){if(!active)e.currentTarget.style.background="var(--sb-active)"}} onMouseLeave={function(e){if(!active)e.currentTarget.style.background="transparent"}} style={{display:"flex",alignItems:"center",justifyContent:sbCollapsed?"center":"flex-start",gap:sbCollapsed?0:12,width:"100%",padding:sbCollapsed?"11px 0":"11px 14px",borderRadius:12,border:"none",fontSize:13,fontWeight:active?600:500,background:active?"var(--sb-active)":"transparent",color:active?"var(--sb-text)":"var(--sb-muted)",textAlign:"left",marginBottom:2,transition:"all 0.15s"}}>{item.icon?<I n={item.icon} sz={16} c={active?"var(--sb-text)":"var(--sb-muted)"}/>:<span style={{width:8,height:8,borderRadius:"50%",background:item.color,display:"inline-block"}}/>}{!sbCollapsed&&<span style={{flex:1}}>{item.label}</span>}{!sbCollapsed&&item.color&&<span style={{fontSize:10,fontWeight:600,color:"var(--sb-muted)",opacity:0.7}}>{countForStage(item.key)}</span>}</button>})}</nav>
      <div style={{padding:sbCollapsed?"10px 8px 14px":"14px 14px 18px",borderTop:"1px solid var(--sb-divider)"}}>
        {!sbCollapsed&&<button style={{width:"100%",padding:"12px 14px",borderRadius:12,border:"none",background:"var(--sb-active)",color:"var(--sb-text)",fontSize:13,fontWeight:600,display:"flex",alignItems:"center",justifyContent:"center",gap:6,transition:"all 0.15s"}} onClick={function(){setAdd(true)}}><I n="plus" sz={14} c="var(--sb-text)"/>Add Person</button>}
        {sbCollapsed&&<button style={{width:"100%",padding:"10px 0",borderRadius:12,border:"none",background:"var(--sb-active)",display:"flex",alignItems:"center",justifyContent:"center"}} onClick={function(){setAdd(true)}} title="Add Person"><I n="plus" sz={16} c="var(--sb-text)"/></button>}
        {!sbCollapsed&&<div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:4,marginTop:12}}>
          <div style={{textAlign:"center",padding:"8px 0",background:"var(--sb-active)",borderRadius:10}}><div style={{fontSize:16,fontWeight:700,color:"var(--sb-text)"}}>{people.length}</div><div style={{fontSize:8,color:"var(--sb-muted)",fontWeight:700,textTransform:"uppercase",letterSpacing:"0.05em",marginTop:1}}>Tracked</div></div>
          <div style={{textAlign:"center",padding:"8px 0",background:"#F59E0B08",borderRadius:10}}><div style={{fontSize:16,fontWeight:700,color:"#F59E0B"}}>{people.filter(function(x){return x.fullyConnected}).length}</div><div style={{fontSize:8,color:"var(--sb-muted)",fontWeight:700,textTransform:"uppercase",letterSpacing:"0.05em",marginTop:1}}>Connected</div></div>
          <div style={{textAlign:"center",padding:"8px 0",background:"#EF444408",borderRadius:10}}><div style={{fontSize:16,fontWeight:700,color:"#EF4444"}}>{people.filter(function(x){return!x.fullyConnected&&(ago(x.lastContactDate)===null||ago(x.lastContactDate)>3)}).length}</div><div style={{fontSize:8,color:"var(--sb-muted)",fontWeight:700,textTransform:"uppercase",letterSpacing:"0.05em",marginTop:1}}>Overdue</div></div>
        </div>}
      </div>
      <button className="weavr-collapse-btn" onClick={function(){setSbCollapsed(!sbCollapsed)}} style={{position:"absolute",top:"50%",right:-12,transform:"translateY(-50%)",width:24,height:24,borderRadius:"50%",border:"1px solid var(--divider)",background:"var(--card-solid)",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 2px 8px rgba(0,0,0,0.1)",zIndex:70,transition:"all 0.15s"}} onMouseEnter={function(e){e.currentTarget.style.boxShadow="0 4px 12px rgba(0,0,0,0.15)"}} onMouseLeave={function(e){e.currentTarget.style.boxShadow="0 2px 8px rgba(0,0,0,0.1)"}}><span style={{fontSize:10,color:"var(--text-muted)",transform:sbCollapsed?"rotate(180deg)":"rotate(0)",display:"inline-flex",transition:"transform 0.25s"}}>{"◂"}</span></button>
    </aside>
    <main style={{flex:1,overflowY:"auto",minHeight:"100vh"}}>
      <div className="weavr-main-header" style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"18px 40px",borderBottom:"1px solid var(--divider)",position:"sticky",top:0,background:isDark?cw.darkSolid+"F0":isMono?"rgba(248,249,250,0.97)":"rgba(245,246,250,0.97)",zIndex:50,backdropFilter:"blur(16px)"}}>
        <div style={{fontSize:13,color:"var(--text-sub)",fontWeight:500}}>{new Date().toLocaleDateString("en-US",{weekday:"long",month:"long",day:"numeric",year:"numeric"})}</div>
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          <div style={{display:"flex",alignItems:"center",background:"var(--inp)",borderRadius:14,border:"1px solid var(--inp-border)",padding:"0 14px",transition:"all 0.2s"}}><I n="search" sz={15} c="var(--text-muted)"/><input value={search} onChange={function(e){setSearch(e.target.value)}} placeholder="Search people..." style={{width:search?200:150,padding:"10px 10px",borderRadius:14,border:"none",background:"transparent",color:"var(--text)",fontSize:13,outline:"none",boxSizing:"border-box",transition:"width 0.3s"}}/>{search&&<button onClick={function(){setSearch("")}} style={{background:"none",border:"none",cursor:"pointer",padding:0,display:"flex"}}><I n="x" sz={13} c="var(--text-muted)"/></button>}</div>
          <button onClick={function(){var next=config.theme==="light"?"dark":config.theme==="dark"?"mono":"light";setConfig({...config,theme:next})}} style={{display:"flex",alignItems:"center",justifyContent:"center",width:38,height:38,borderRadius:12,border:"1px solid var(--inp-border)",background:"var(--inp)",cursor:"pointer"}} title={config.theme==="light"?"Dark Mode":config.theme==="dark"?"Simple Mode":"Light Mode"}><I n={config.theme==="mono"?"eye":"sun"} sz={16} c="var(--text-muted)"/></button>
          <button onClick={function(){setImp(true)}} style={{display:"flex",alignItems:"center",justifyContent:"center",width:38,height:38,borderRadius:12,border:"1px solid var(--inp-border)",background:"var(--inp)",cursor:"pointer"}} title="Import"><I n="upload" sz={16} c="var(--text-muted)"/></button>
        </div>
      </div>
      <div className="weavr-main-content" style={{padding:"24px 40px 40px",animation:"fadeUp 0.5s cubic-bezier(0.22,1,0.36,1)"}}>
...
function Portal(p){
  return <div style={{minHeight:"100vh",background:"linear-gradient(145deg,#3730A3 0%,#4338CA 25%,#3B35B0 50%,#2E2A8A 75%,#252262 100%)",fontFamily:"'Nunito',sans-serif",position:"relative",overflow:"hidden",animation:"portalFadeIn 0.4s ease"}}>
    ...
    <div style={{maxWidth:1100,margin:"0 auto",padding:"0 28px"}}>
      <div className="weavr-portal-header" style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"28px 0",animation:"loginSlideUp 0.5s cubic-bezier(0.22,1,0.36,1) 0.1s both"}}>
        <div style={{display:"flex",alignItems:"center",gap:14}}>
          <div style={{width:48,height:48,borderRadius:14,background:"linear-gradient(135deg,#7C3AED,#06B6D4)",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 4px 20px rgba(124,58,237,0.3)"}}><WeavrLogo sz={26}/></div>
          <div style={{fontSize:22,fontWeight:800,color:"#fff",letterSpacing:"0.14em"}}>WEAVR</div>
        </div>
        <button onClick={p.onLogout} style={{padding:"10px 24px",borderRadius:12,border:"1px solid rgba(255,255,255,0.12)",background:"rgba(255,255,255,0.06)",color:"rgba(255,255,255,0.6)",fontSize:12,fontWeight:600,cursor:"pointer",backdropFilter:"blur(8px)",transition:"all 0.2s"}} onMouseEnter={function(e){e.currentTarget.style.background="rgba(255,255,255,0.12)"}} onMouseLeave={function(e){e.currentTarget.style.background="rgba(255,255,255,0.06)"}}>Sign Out</button>
      </div>

      <div className="weavr-portal-hero" style={{textAlign:"center",padding:"48px 0 56px"}}>
        <h1 style={{fontSize:36,fontWeight:800,color:"#fff",lineHeight:1.25,marginBottom:14,letterSpacing:"-0.01em",animation:"loginSlideUp 0.6s cubic-bezier(0.22,1,0.36,1) 0.2s both"}}>Ministry Tools That Keep<br/>Your Church Connected</h1>
        <p style={{fontSize:15,color:"rgba(255,255,255,0.4)",maxWidth:480,margin:"0 auto",lineHeight:1.7,animation:"loginSlideUp 0.6s cubic-bezier(0.22,1,0.36,1) 0.3s both"}}>Everything your team needs to track, connect, and care for every person who walks through your doors.</p>
      </div>

      <div className="weavr-portal-grid" style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:16,paddingBottom:64}}>
        {TOOLS.map(function(t,idx){var isActive=t.ready;var delay=(0.4+idx*0.08)+"s";return <div key={t.key} className="weavr-portal-card" onClick={isActive?function(){p.onTool(t.key)}:undefined} style={{background:isActive?t.grad:"rgba(255,255,255,0.04)",borderRadius:20,padding:"28px 24px",cursor:isActive?"pointer":"default",border:isActive?"none":"1px solid rgba(255,255,255,0.06)",transition:"all 0.3s ease",opacity:isActive?1:0.55,position:"relative",overflow:"hidden",animation:"portalCardDrop 0.5s cubic-bezier(0.22,1,0.36,1) "+delay+" both"}} onMouseEnter={isActive?function(e){e.currentTarget.style.transform="translateY(-6px)";e.currentTarget.style.boxShadow="0 16px 48px "+t.color+"50"}:function(e){e.currentTarget.style.background="rgba(255,255,255,0.07)"}} onMouseLeave={isActive?function(e){e.currentTarget.style.transform="translateY(0)";e.currentTarget.style.boxShadow="none"}:function(e){e.currentTarget.style.background="rgba(255,255,255,0.04)"}}>
            <div style={{width:48,height:48,borderRadius:14,background:isActive?"rgba(255,255,255,0.2)":"rgba(255,255,255,0.06)",display:"flex",alignItems:"center",justifyContent:"center",marginBottom:20,backdropFilter:"blur(4px)"}}><I n={t.icon} sz={22} c="#fff"/></div>
            <div style={{fontSize:16,fontWeight:700,color:"#fff",marginBottom:6}}>Weavr {t.name}</div>
            <div style={{fontSize:12,color:isActive?"rgba(255,255,255,0.7)":"rgba(255,255,255,0.3)",lineHeight:1.6,marginBottom:16}}>{t.desc}</div>
            {isActive?<div style={{display:"flex",alignItems:"center",gap:6,fontSize:13,fontWeight:700,color:"rgba(255,255,255,0.9)"}}><span>Launch</span><span style={{fontSize:16}}>{"→"}</span></div>:<div style={{fontSize:10,fontWeight:700,color:"rgba(255,255,255,0.2)",textTransform:"uppercase",letterSpacing:"0.12em"}}>Coming Soon</div>}
          </div>})}
      </div>
      ...
      <div style={{textAlign:"center",paddingBottom:40}}>
        <div style={{display:"inline-flex",alignItems:"center",gap:8,padding:"10px 20px",borderRadius:14,background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.06)"}}>
          ...
          <span style={{fontSize:11,color:"rgba(255,255,255,0.2)",fontWeight:600}}>Weavr Platform v2.0</span>
        </div>
      </div>
    </div>
  </div>;
...
export default function App(){
  ...
  return <div style={{position:"relative",minHeight:"100vh",fontFamily:"'Nunito',sans-serif",background:"#252262"}}>
    ...
    {!authed&&<div style={{position:"fixed",inset:0,zIndex:300,display:"flex"}} className="weavr-login-split">
      <div className="weavr-login-left" style={{flex:1,background:"linear-gradient(145deg,#3730A3 0%,#4338CA 25%,#3B35B0 50%,#2E2A8A 75%,#252262 100%)",display:"flex",flexDirection:"column",justifyContent:"center",padding:"60px 56px",position:"relative",overflow:"hidden"}}>

        <div style={{position:"relative",zIndex:1}}>
          <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:56,animation:"loginSlideUp 0.7s cubic-bezier(0.22,1,0.36,1) both"}}>
            <div style={{width:48,height:48,borderRadius:14,background:"linear-gradient(135deg,#7C3AED,#06B6D4)",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 4px 20px rgba(124,58,237,0.3)"}}><WeavrLogo sz={26}/></div>
            <div style={{fontSize:22,fontWeight:800,color:"#fff",letterSpacing:"0.14em"}}>WEAVR</div>
          </div>

          <h1 style={{fontSize:48,fontWeight:800,color:"#fff",lineHeight:1.08,marginBottom:20,letterSpacing:"-0.02em",animation:"loginSlideUp 0.7s cubic-bezier(0.22,1,0.36,1) 0.1s both"}}>The engine<br/>behind healthy<br/>churches.</h1>
          <p style={{fontSize:16,color:"rgba(255,255,255,0.5)",lineHeight:1.8,maxWidth:420,marginBottom:48,animation:"loginSlideUp 0.7s cubic-bezier(0.22,1,0.36,1) 0.2s both"}}>Track every connection. Equip every team. Never let anyone slip through the cracks.</p>

          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12,maxWidth:500}}>
            {[{icon:"users",n:"Pipeline",d:"6-stage journey",c:"#A78BFA",delay:"0.3s"},{icon:"zap",n:"Scoring",d:"Auto-prioritize",c:"#22D3EE",delay:"0.35s"},{icon:"chart",n:"Analytics",d:"Growth trends",c:"#34D399",delay:"0.4s"},{icon:"send",n:"Messaging",d:"Text & email",c:"#FBBF24",delay:"0.45s"},{icon:"cal",n:"Follow-Ups",d:"Smart scheduling",c:"#F472B6",delay:"0.5s"},{icon:"target",n:"Milestones",d:"Track every step",c:"#FB923C",delay:"0.55s"}].map(function(f){return <div key={f.n} style={{background:"rgba(255,255,255,0.06)",borderRadius:16,padding:"20px 14px",border:"1px solid rgba(255,255,255,0.08)",textAlign:"center",cursor:"default",transition:"all 0.25s",animation:"loginCardPop 0.5s cubic-bezier(0.22,1,0.36,1) "+f.delay+" both"}} onMouseEnter={function(e){e.currentTarget.style.background="rgba(255,255,255,0.1)";e.currentTarget.style.borderColor=f.c+"40";e.currentTarget.style.boxShadow="0 0 24px "+f.c+"20";e.currentTarget.style.transform="translateY(-3px)"}} onMouseLeave={function(e){e.currentTarget.style.background="rgba(255,255,255,0.06)";e.currentTarget.style.borderColor="rgba(255,255,255,0.08)";e.currentTarget.style.boxShadow="none";e.currentTarget.style.transform="translateY(0)"}}><div style={{width:38,height:38,borderRadius:12,background:"rgba(255,255,255,0.1)",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 10px"}}><I n={f.icon} sz={17} c={f.c}/></div><div style={{fontSize:12,fontWeight:700,color:"#fff",marginBottom:2}}>{f.n}</div><div style={{fontSize:10,color:"rgba(255,255,255,0.3)"}}>{f.d}</div></div>})}
          </div>
        </div>
      </div>

      <div className="weavr-login-right" style={{width:480,flexShrink:0,background:"linear-gradient(180deg,#252262 0%,#1E1B4B 100%)",display:"flex",flexDirection:"column",justifyContent:"center",padding:"60px 52px"}}>
        <div style={{animation:authAnim?"authFadeOut 0.5s ease forwards":"authFadeIn 0.6s ease 0.3s both"}}>
          <div className="weavr-login-mobile-logo" style={{display:"none",alignItems:"center",gap:12,marginBottom:32}}><div style={{width:40,height:40,borderRadius:12,background:"linear-gradient(135deg,#7C3AED,#06B6D4)",display:"flex",alignItems:"center",justifyContent:"center"}}><WeavrLogo sz={22}/></div><div style={{fontSize:20,fontWeight:800,color:"#fff",letterSpacing:"0.14em"}}>WEAVR</div></div>
          <div style={{marginBottom:40}}>
            <h2 style={{fontSize:32,fontWeight:800,color:"#fff",marginBottom:8,animation:"loginSlideUp 0.6s cubic-bezier(0.22,1,0.36,1) 0.4s both"}}>{authTab==="signin"?"Welcome back.":"Get started."}</h2>
            <p style={{fontSize:14,color:"rgba(255,255,255,0.3)",animation:"loginSlideUp 0.6s cubic-bezier(0.22,1,0.36,1) 0.5s both"}}>{authTab==="signin"?"Sign in to your church dashboard":"Create your Weavr account"}</p>
          </div>

          <div style={{display:"flex",gap:0,marginBottom:32,background:"rgba(255,255,255,0.06)",borderRadius:12,padding:3}}>
            {["signin","signup"].map(function(t){var active=authTab===t;return <button key={t} onClick={function(){setAuthTab(t);setErr("")}} style={{flex:1,padding:"10px",borderRadius:10,border:"none",fontSize:13,fontWeight:active?700:500,color:active?"#fff":"rgba(255,255,255,0.3)",background:active?"rgba(124,58,237,0.4)":"transparent",cursor:"pointer",transition:"all 0.2s"}}>{t==="signin"?"Sign In":"Sign Up"}</button>})}
          </div>

          <div style={{marginBottom:22}}>
            <div style={{fontSize:11,fontWeight:600,color:"rgba(255,255,255,0.25)",marginBottom:8}}>Username</div>
            <input style={{width:"100%",padding:"14px 18px",borderRadius:14,border:"1px solid rgba(255,255,255,0.08)",background:"rgba(255,255,255,0.04)",color:"#fff",fontSize:14,boxSizing:"border-box",outline:"none",transition:"border-color 0.2s"}} value={user} onChange={function(e){setUser(e.target.value)}} onKeyDown={function(e){if(e.key==="Enter"){if(authTab==="signin")doLogin();else doSignup()}}} placeholder="Enter your username" autoFocus onFocus={function(e){e.currentTarget.style.borderColor="rgba(139,92,246,0.5)"}} onBlur={function(e){e.currentTarget.style.borderColor="rgba(255,255,255,0.08)"}}/>
          </div>

          <div style={{marginBottom:authTab==="signup"?22:32}}>
            <div style={{fontSize:11,fontWeight:600,color:"rgba(255,255,255,0.25)",marginBottom:8}}>Password</div>
            <input type="password" style={{width:"100%",padding:"14px 18px",borderRadius:14,border:"1px solid rgba(255,255,255,0.08)",background:"rgba(255,255,255,0.04)",color:"#fff",fontSize:14,boxSizing:"border-box",outline:"none",transition:"border-color 0.2s"}} value={pass} onChange={function(e){setPass(e.target.value)}} onKeyDown={function(e){if(e.key==="Enter"&&authTab==="signin")doLogin()}} placeholder="Enter password" onFocus={function(e){e.currentTarget.style.borderColor="rgba(139,92,246,0.5)"}} onBlur={function(e){e.currentTarget.style.borderColor="rgba(255,255,255,0.08)"}}/>
          </div>

          {authTab==="signup"&&<div style={{marginBottom:32}}>
            <div style={{fontSize:11,fontWeight:600,color:"rgba(255,255,255,0.25)",marginBottom:8}}>Confirm Password</div>
            <input type="password" style={{width:"100%",padding:"14px 18px",borderRadius:14,border:"1px solid rgba(255,255,255,0.08)",background:"rgba(255,255,255,0.04)",color:"#fff",fontSize:14,boxSizing:"border-box",outline:"none",transition:"border-color 0.2s"}} value={confirmPass} onChange={function(e){setConfirmPass(e.target.value)}} onKeyDown={function(e){if(e.key==="Enter")doSignup()}} placeholder="Confirm password" onFocus={function(e){e.currentTarget.style.borderColor="rgba(139,92,246,0.5)"}} onBlur={function(e){e.currentTarget.style.borderColor="rgba(255,255,255,0.08)"}}/>
          </div>}

          {err&&<div style={{fontSize:12,color:"#FCA5A5",marginBottom:16,textAlign:"center",background:"rgba(239,68,68,0.08)",padding:"10px 14px",borderRadius:12,border:"1px solid rgba(239,68,68,0.15)"}}>{err}</div>}

          <button onClick={authTab==="signin"?doLogin:doSignup} style={{width:"100%",padding:"16px",borderRadius:14,border:"none",background:"linear-gradient(135deg,#8B5CF6,#7C3AED)",color:"#fff",fontSize:15,fontWeight:700,cursor:"pointer",transition:"all 0.2s",boxShadow:"0 4px 20px rgba(124,58,237,0.3)"}} onMouseEnter={function(e){e.currentTarget.style.transform="translateY(-2px)";e.currentTarget.style.boxShadow="0 8px 32px rgba(124,58,237,0.5)"}} onMouseLeave={function(e){e.currentTarget.style.transform="translateY(0)";e.currentTarget.style.boxShadow="0 4px 20px rgba(124,58,237,0.3)"}}>{authTab==="signin"?"Sign In":"Create Account"}</button>

          <div style={{textAlign:"center",marginTop:28}}>
            <span style={{fontSize:11,color:"rgba(255,255,255,0.12)"}}>Weavr Platform v2.0</span>
          </div>
        </div>
      </div>
    </div>}
