...
var DEFAULT_CI=[
  ...
  {key:"text-sent",label:"Sent Text",icon:"phone",color:"#3898FF"},
...
var COLORWAYS={
  purple:{primary:"#7C3AED",primaryGrad:"linear-gradient(135deg,#8B5CF6,#7C3AED)",accent:"#A78BFA",sidebar:"linear-gradient(180deg,#3730A3,#2E2A6E 60%,#1E1B4B)",logo:"linear-gradient(135deg,#7C3AED,#06B6D4)",hero:"linear-gradient(135deg,#3D38A5,#2E2A6E)",darkHero:"linear-gradient(135deg,#3730A3,#2E2A6E)",darkBg:"linear-gradient(145deg,#1E1B4B,#2E2A6E,#1E1B4B)",darkCard:"rgba(55,48,163,0.35)",darkSolid:"#2E2A6E"},
...
var getMultiTpl=function(person,config,type){
  var stage=person.currentStage;
  if(type==="email"){
    var eTpls=(config||{}).multiEmailTpl||DEFAULT_EMAIL_TPL;
    var list=eTpls[stage]||[{name:"Default",subj:"Following up",body:DEFAULT_TPL[stage]||""}];
    return list.map(function(t){return{name:t.name,subj:(t.subj||"").replace("{firstName}",person.firstName).replace("{lastName}",person.lastName||""),body:(t.body||"").replace("{firstName}",person.firstName).replace("{lastName}",person.lastName||"")}});
  }
  var tTpls=(config||{}).multiTextTpl||DEFAULT_MULTI_TPL;
  var list2=tTpls[stage]||[{name:"Default",body:DEFAULT_TPL[stage]||""}];
  return list2.map(function(t){return{name:t.name,body:(t.body||"").replace("{firstName}",person.firstName).replace("{lastName}",person.lastName||"")}});
};

var FOLLOW_UP_DELAYS={"first-visit":5,salvation:5,baptism:7,"next-steps":7,bgroup:14,ateam:14};
var FOLLOW_UP_LABELS={"first-visit":"Secondary First Visit Follow-up",salvation:"Salvation Check-in",baptism:"Baptism Check-in","next-steps":"Next Steps Follow-up",bgroup:"BGroup Follow-up",ateam:"ATeam Follow-up"};
var DEFAULT_FOLLOWUP_TPL={"first-visit":"Hey {firstName}, I pray you had a great week - hope to see you at church this weekend!",salvation:"Hey {firstName}, just checking in on you this week. How are things going?",baptism:"Hey {firstName}, praying for you! How has everything been since your baptism?","next-steps":"Hey {firstName}, hope you're doing great! Anything I can help with?",bgroup:"Hey {firstName}, how's your BGroup going? Would love to hear!",ateam:"Hey {firstName}, thanks for serving! How's the ATeam experience been?"};

/* Shared template index — persists across modal opens within session */
var _lastTplIdx={text:{},email:{}};

function TextModal(p){
  var [copied,setCopied]=useState("");
  var stage=p.person.currentStage;
  var [selIdx,setSelIdx]=useState(_lastTplIdx.text[stage]||0);
  var [showPicker,setShowPicker]=useState(false);
  var [sent,setSent]=useState(null);
  var person=p.person;var config=p.config||{};
  var baseTpls=getMultiTpl(person,config,"text");
  var fuTpl=p.fuTemplate;
  var tpls=fuTpl?(fuTpl.allTpls||[{name:fuTpl.label||"Follow-up",body:fuTpl.body}]).concat(baseTpls):baseTpls;
  var msg=tpls[selIdx]?tpls[selIdx].body:"";
  var doCopy=function(text,id){copyText(text,function(ok){if(ok){setCopied(id);setTimeout(function(){setCopied("")},2000)}})};
  var pickTpl=function(i){setSelIdx(i);_lastTplIdx.text[stage]=i;setShowPicker(false)};
  return <Modal title={"Text "+person.firstName} onClose={p.onClose}>
    <div style={{display:"flex",alignItems:"center",gap:7,background:"var(--inp)",borderRadius:8,padding:"10px 13px",marginBottom:10}}>
      <I n="phone" sz={16} c="#3898FF"/>
      <span style={{flex:1,fontSize:13,fontWeight:600,color:"var(--text)",letterSpacing:"0.02em"}}>{fmtPhone(person.phone)}</span>
      <button onClick={function(){doCopy(person.phone,"phone")}} style={{padding:"5px 11px",borderRadius:6,background:copied==="phone"?"#10B981":"#06B6D4",color:"#fff",border:"none",fontSize:9,fontWeight:600,cursor:"pointer"}}>{copied==="phone"?"Copied!":"Copy #"}</button>
    </div>
    <div style={{marginBottom:10}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:5}}>
        <div style={{fontSize:9,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.08em",color:"var(--text-muted)"}}>Template: {tpls[selIdx]?tpls[selIdx].name:""}</div>
        <div style={{display:"flex",gap:4}}>
          {tpls.length>1&&<button onClick={function(){setShowPicker(!showPicker)}} style={{padding:"3px 10px",borderRadius:5,background:"var(--primary)10",color:"var(--primary)",border:"1px solid var(--primary)25",fontSize:9,fontWeight:600,cursor:"pointer"}}>{showPicker?"Close":"Choose Template"}</button>}
      ...
      {showPicker&&<div style={{display:"flex",flexWrap:"wrap",gap:5,marginBottom:8}}>{tpls.map(function(t,i){return <button key={i} onClick={function(){pickTpl(i)}} style={{padding:"5px 12px",borderRadius:6,border:i===selIdx?"1px solid var(--primary)":"1px solid var(--inp-border)",background:i===selIdx?"var(--primary)08":"var(--inp)",fontSize:10,fontWeight:i===selIdx?700:500,color:i===selIdx?"var(--primary)":"var(--text-sub)",cursor:"pointer"}}>{t.name}</button>})}</div>}
      <div style={{background:"var(--inp)",borderRadius:8,padding:"10px 13px",fontSize:11,color:"var(--text)",lineHeight:1.7,border:"1px solid var(--inp-border)",whiteSpace:"pre-wrap"}}>{msg}</div>
    </div>
    {!sent&&<div style={{display:"flex",gap:6}}>
      <a href={smsUrl(person.phone,msg)} target="_top" onClick={function(){setSent("iMessage")}} style={{flex:1,textDecoration:"none",padding:"10px",borderRadius:8,background:"linear-gradient(135deg,#34D399,#10B981)",textAlign:"center",fontSize:11,fontWeight:700,color:"#fff",display:"block"}}>iMessage</a>
      <a href={"https://voice.google.com/u/0/messages"} target="_blank" rel="noopener" onClick={function(){setSent("Google Voice")}} style={{flex:1,textDecoration:"none",padding:"10px",borderRadius:8,background:"linear-gradient(135deg,#60A5FA,#3B82F6)",textAlign:"center",fontSize:11,fontWeight:700,color:"#fff",display:"block"}}>Google Voice</a>
    </div>}
    {sent&&<button onClick={function(){if(p.onLog)p.onLog("text",sent);p.onClose()}} style={{width:"100%",padding:"12px",borderRadius:8,border:"none",background:"linear-gradient(135deg,#10B981,#059669)",color:"#fff",fontSize:12,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:6}}><I n="check" sz={14} c="#fff"/>Log Text via {sent}</button>}
  </Modal>;
}
...
function EmailModal(p){
  var [copied,setCopied]=useState("");
  var stage=p.person.currentStage;
  var [selIdx,setSelIdx]=useState(_lastTplIdx.email[stage]||0);
  var [showPicker,setShowPicker]=useState(false);
  var [sent,setSent]=useState(false);
  var person=p.person;var config=p.config||{};
  var baseTpls=getMultiTpl(person,config,"email");
  var fuTpl=p.fuTemplate;
  var tpls=fuTpl?(fuTpl.allTpls||[{name:fuTpl.label||"Follow-up",subj:fuTpl.subj||"Following up",body:fuTpl.body}]).concat(baseTpls):baseTpls;
  var em=tpls[selIdx]||{subj:"",body:""};
  var doCopy=function(text,id){copyText(text,function(ok){if(ok){setCopied(id);setTimeout(function(){setCopied("")},2000)}})};
  var pickTpl=function(i){setSelIdx(i);_lastTplIdx.email[stage]=i;setShowPicker(false)};
  return <Modal title={"Email "+person.firstName} onClose={p.onClose}>
    <div style={{display:"flex",alignItems:"center",gap:7,background:"var(--inp)",borderRadius:8,padding:"10px 13px",marginBottom:10}}>
      <I n="mail" sz={16} c="#10B981"/>
      <span style={{flex:1,fontSize:12,fontWeight:600,color:"var(--text)"}}>{person.email}</span>
      <button onClick={function(){doCopy(person.email,"email")}} style={{padding:"5px 11px",borderRadius:6,background:copied==="email"?"#10B981":"#10B981",color:"#fff",border:"none",fontSize:9,fontWeight:600,cursor:"pointer"}}>{copied==="email"?"Copied!":"Copy"}</button>
    </div>
    <div style={{marginBottom:10}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:5}}>
        <div style={{fontSize:9,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.08em",color:"var(--text-muted)"}}>Template: {tpls[selIdx]?tpls[selIdx].name:""}</div>
        <div style={{display:"flex",gap:4}}>
          {tpls.length>1&&<button onClick={function(){setShowPicker(!showPicker)}} style={{padding:"3px 10px",borderRadius:5,background:"var(--primary)10",color:"var(--primary)",border:"1px solid var(--primary)25",fontSize:9,fontWeight:600,cursor:"pointer"}}>{showPicker?"Close":"Choose Template"}</button>}
          <button onClick={function(){doCopy((em.subj?"Subject: "+em.subj+"\n\n":"")+em.body,"ebody")}} style={{padding:"3px 10px",borderRadius:5,background:copied==="ebody"?"#10B981":"var(--inp)",color:copied==="ebody"?"#fff":"var(--text-sub)",border:"1px solid var(--inp-border)",fontSize:9,fontWeight:600,cursor:"pointer"}}>{copied==="ebody"?"Copied!":"Copy All"}</button>
        </div>
      </div>
      {showPicker&&<div style={{display:"flex",flexWrap:"wrap",gap:5,marginBottom:8}}>{tpls.map(function(t,i){return <button key={i} onClick={function(){pickTpl(i)}} style={{padding:"5px 12px",borderRadius:6,border:i===selIdx?"1px solid #10B981":"1px solid var(--inp-border)",background:i===selIdx?"#3898FF08":"var(--inp)",fontSize:10,fontWeight:i===selIdx?700:500,color:i===selIdx?"#10B981":"var(--text-sub)",cursor:"pointer"}}>{t.name}</button>})}</div>}
      <div style={{background:"var(--inp)",borderRadius:8,padding:"10px 13px",border:"1px solid var(--inp-border)"}}>
        {em.subj&&<div style={{marginBottom:6}}><div style={{fontSize:8,fontWeight:700,color:"var(--text-muted)",textTransform:"uppercase"}}>Subject</div><div style={{fontSize:11,color:"var(--text)",marginTop:2}}>{em.subj}</div></div>}
        <div><div style={{fontSize:8,fontWeight:700,color:"var(--text-muted)",textTransform:"uppercase"}}>Body</div><div style={{fontSize:11,color:"var(--text-sub)",marginTop:2,lineHeight:1.7,whiteSpace:"pre-wrap"}}>{em.body}</div></div>
      </div>
    </div>
    {!sent&&<a href={mailUrl(person.email,em.subj||"",em.body||"")} target="_top" onClick={function(){setSent(true)}} style={{display:"block",textDecoration:"none",padding:"10px",borderRadius:8,background:"linear-gradient(135deg,#34D399,#10B981)",textAlign:"center",fontSize:11,fontWeight:700,color:"#fff"}}>Open in Mail</a>}
    {sent&&<button onClick={function(){if(p.onLog)p.onLog("email","Email");p.onClose()}} style={{width:"100%",padding:"12px",borderRadius:8,border:"none",background:"linear-gradient(135deg,#10B981,#059669)",color:"#fff",fontSize:12,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:6}}><I n="check" sz={14} c="#fff"/>Log Email Sent</button>}
  </Modal>;
}

function CallModal(p){
  var [copied,setCopied]=useState("");
  var [sent,setSent]=useState(null);
  var person=p.person;
  var doCopy=function(text,id){copyText(text,function(ok){if(ok){setCopied(id);setTimeout(function(){setCopied("")},2000)}})};
  return <Modal title={"Call "+person.firstName} onClose={p.onClose}>
    <div style={{display:"flex",alignItems:"center",gap:7,background:"var(--inp)",borderRadius:8,padding:"10px 13px",marginBottom:13}}>
      <I n="phone" sz={16} c="var(--primary)"/>
      <span style={{flex:1,fontSize:13,fontWeight:600,color:"var(--text)",letterSpacing:"0.02em"}}>{fmtPhone(person.phone)}</span>
      <button onClick={function(){doCopy(person.phone,"phone")}} style={{padding:"5px 11px",borderRadius:6,background:copied==="phone"?"#10B981":"var(--primary)",color:"#fff",border:"none",fontSize:9,fontWeight:600,cursor:"pointer"}}>{copied==="phone"?"Copied!":"Copy #"}</button>
    </div>
    {!sent&&<div style={{display:"flex",gap:8}}>
      <a href={"facetime-audio://"+person.phone} target="_top" onClick={function(){setSent("FaceTime")}} style={{flex:1,textDecoration:"none",padding:"12px",borderRadius:8,background:"linear-gradient(135deg,#A78BFA,#7C3AED)",textAlign:"center",display:"block"}}><div style={{fontSize:12,fontWeight:700,color:"#fff",marginBottom:2}}>FaceTime Audio</div><div style={{fontSize:9,color:"rgba(255,255,255,0.6)"}}>Apple devices</div></a>
      <a href={"https://voice.google.com/u/0/calls"} target="_blank" rel="noopener" onClick={function(){setSent("Google Voice")}} style={{flex:1,textDecoration:"none",padding:"12px",borderRadius:8,background:"linear-gradient(135deg,#60A5FA,#3B82F6)",textAlign:"center",display:"block"}}><div style={{fontSize:12,fontWeight:700,color:"#fff",marginBottom:2}}>Google Voice</div><div style={{fontSize:9,color:"rgba(255,255,255,0.6)"}}>Any device</div></a>
    </div>}
    {sent&&<button onClick={function(){if(p.onLog)p.onLog("call",sent);p.onClose()}} style={{width:"100%",padding:"12px",borderRadius:8,border:"none",background:"linear-gradient(135deg,#10B981,#059669)",color:"#fff",fontSize:12,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:6}}><I n="check" sz={14} c="#fff"/>Log Call via {sent}</button>}
  </Modal>;
}
...
var confettiBurst=function(e){
  var rect=e.currentTarget.getBoundingClientRect();
  var cx=rect.left+rect.width/2;var cy=rect.top+rect.height/2;
  var colors=["#F97316","#FBBF24","#10B981","#7C3AED","#06B6D4","#EC4899","#EF4444","#fff"];
  ...
  for(var i=0;i<40;i++){
    ...
    var vel=Math.random()*280+120;
    var vx=Math.cos(angle)*vel;var vy=Math.sin(angle)*vel-100;
    var rot=Math.random()*720-360;
    var color=colors[Math.floor(Math.random()*colors.length)];
    var shape=Math.random()>0.5;
    p.style.cssText="position:absolute;left:"+cx+"px;top:"+cy+"px;width:"+sz+"px;height:"+(shape?sz:sz*0.4)+"px;background:"+color+";border-radius:"+(shape?"50%":"1px")+";opacity:1;transform:translate(0,0) rotate(0deg)";
    container.appendChild(p);
    (function(el,dvx,dvy,drot){
      var startTime=performance.now();
      var animate=function(now){
        var t=(now-startTime)/1000;
        if(t>1.4){el.remove();return}
        var x=dvx*t;var y=dvy*t+300*t*t;
        var opacity=t<0.8?1:Math.max(0,1-(t-0.8)/0.6);
        var r=drot*t;
...
function Spark(p){var pts=p.data||[];if(pts.length<2)return null;var mx=Math.max.apply(null,pts)||1;var w=p.w||40,h=p.h||14;var coords=pts.map(function(v,i){return(i*w/(pts.length-1))+","+((1-v/mx)*h)}).join(" ");return <svg width={w} height={h} style={{display:"block",flexShrink:0}}><polyline points={coords} fill="none" stroke={p.c||"var(--text-muted)"} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" opacity="0.5"/></svg>}
...
function Btn(p){var v=p.v||"default";var base={display:"inline-flex",alignItems:"center",gap:6,padding:"8px 16px",borderRadius:8,fontSize:11,fontWeight:600,cursor:"pointer",whiteSpace:"nowrap",border:"none",transition:"all 0.2s"};var styles=v==="ghost"?{background:"var(--inp)",color:"var(--text-sub)",border:"1px solid var(--inp-border)"}:v==="red"?{background:"#FEF2F2",color:"#EF4444",border:"1px solid #FECACA"}:v==="green"?{background:"#ECFDF5",color:"#059669",border:"1px solid #D1FAE5"}:v==="teal"?{background:"#ECFEFF",color:"#0891B2",border:"1px solid #CFFAFE"}:{background:"var(--primary)",color:"#fff",boxShadow:"0 2px 8px var(--primary)30"};return <button style={{...base,...styles,...p.sx}} onClick={p.onClick} onMouseEnter={function(e){e.currentTarget.style.transform="translateY(-1px)"}} onMouseLeave={function(e){e.currentTarget.style.transform="translateY(0)"}}>{p.icon && <I n={p.icon} sz={14}/>}{p.label}</button>}

function Field(p){return <div><div style={{fontSize:9,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.08em",color:"var(--text-muted)",marginBottom:6}}>{p.label}</div><input style={{width:"100%",padding:"10px 13px",borderRadius:8,border:"1px solid var(--inp-border)",background:"var(--inp)",color:"var(--text)",fontSize:11,boxSizing:"border-box",outline:"none",transition:"border-color 0.2s"}} value={p.value} onChange={function(e){p.onChange(e.target.value)}} autoFocus={p.autoFocus} placeholder={p.placeholder}/></div>}
...
function WidgetResize(p){
  var [previewLabel,setPreviewLabel]=useState(null);
  var calcSize=function(axis,absDelta,dir){
    var curCols=WS_COLS[p.currentSize]||2;var curRows=WS_ROWS[p.currentSize]||2;
    var nc=curCols;var nr=curRows;
    if(axis==="col"&&absDelta>40){nc=curCols===2?1:2}
    if(axis==="row"&&absDelta>30){nr=dir>0?(curRows<4?curRows===1?2:4:4):(curRows>1?curRows===4?2:1:1)}
    return nc+"x"+(nr===1?"S":nr===2?"M":"L");
  };
  var handle=function(axis,side){return function(e){
    e.preventDefault();e.stopPropagation();
    var sx=e.clientX,sy=e.clientY;
    var move=function(ev){var dx=ev.clientX-sx,dy=ev.clientY-sy;var ns=calcSize(axis,axis==="col"?Math.abs(dx):Math.abs(dy),axis==="row"?dy:0);setPreviewLabel(ns!==p.currentSize?WIDGET_SIZE_LABELS[ns]:null)};
    var up=function(ev){var dx=ev.clientX-sx,dy=ev.clientY-sy;var ns=calcSize(axis,axis==="col"?Math.abs(dx):Math.abs(dy),axis==="row"?dy:0);if(ns!==p.currentSize)p.onResize(ns,side);setPreviewLabel(null);document.removeEventListener("pointermove",move);document.removeEventListener("pointerup",up);document.body.style.cursor="";document.body.style.userSelect=""};
    document.body.style.cursor=axis==="col"?"col-resize":"row-resize";document.body.style.userSelect="none";
    document.addEventListener("pointermove",move);document.addEventListener("pointerup",up);
  }};
  return <div>
    <div onPointerDown={handle("row","top")} style={{position:"absolute",top:0,left:8,right:8,height:6,cursor:"row-resize",zIndex:6}}/>
    <div onPointerDown={handle("row","bottom")} style={{position:"absolute",bottom:0,left:8,right:8,height:6,cursor:"row-resize",zIndex:6}}/>
    <div onPointerDown={handle("col","left")} style={{position:"absolute",left:0,top:8,bottom:8,width:6,cursor:"col-resize",zIndex:6}}/>
    <div onPointerDown={handle("col","right")} style={{position:"absolute",right:0,top:8,bottom:8,width:6,cursor:"col-resize",zIndex:6}}/>
    {previewLabel&&<div style={{position:"absolute",bottom:8,left:"50%",transform:"translateX(-50%)",padding:"4px 10px",borderRadius:6,background:"var(--primary)",color:"#fff",fontSize:8,fontWeight:700,zIndex:10,whiteSpace:"nowrap",boxShadow:"0 2px 8px rgba(0,0,0,0.15)",pointerEvents:"none"}}>{previewLabel}</div>}
  </div>;
}

function Modal(p){return <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,0.5)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",animation:"gentleFade 0.3s ease"}} onClick={p.onClose}><div className="weavr-modal-inner" style={{background:"var(--card-solid)",borderRadius:8,padding:26,width:p.wide?580:400,maxWidth:"92vw",maxHeight:"85vh",overflowY:"auto",boxShadow:"0 32px 80px rgba(0,0,0,0.15)",animation:"gentleFade 0.35s cubic-bezier(0.22,1,0.36,1)"}} onClick={function(e){e.stopPropagation()}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:22}}><h3 style={{fontSize:16,fontWeight:700,color:"var(--text)"}}>{p.title}</h3><button style={{background:"var(--inp)",border:"1px solid var(--inp-border)",borderRadius:8,width:36,height:36,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",transition:"all 0.15s"}} onClick={p.onClose}><I n="x" sz={16} c="var(--text-muted)"/></button></div>{p.children}</div></div>}
...
function MiniCal(p){
  ...
  return <div style={{background:"var(--card)",borderRadius:8,padding:13,border:"1px solid var(--inp-border)"}}>
...
/* ══════ OVERVIEW ══════ */
var OV_WIDGETS=[
  {key:"next-up",label:"Next Up",icon:"zap",defaultSize:"2xS"},
  {key:"mml",label:"Monday Morning List",icon:"sun",defaultSize:"2xS"},
  {key:"funnel",label:"Engagement Funnel",icon:"chart",defaultSize:"2xM"},
  {key:"weekly-stats",label:"Weekly Stats",icon:"target",defaultSize:"2xS"},
  {key:"kpis",label:"KPI Summary",icon:"users",defaultSize:"2xS"},
  {key:"distribution",label:"Engagement Distribution",icon:"chart",defaultSize:"1xM"},
  {key:"velocity",label:"Stage Velocity",icon:"clock",defaultSize:"1xM"},
  ...
  {key:"scheduled",label:"Scheduled Follow-ups",icon:"clock",defaultSize:"1xM"}
];
...
var WS_COLS={"1xS":1,"2xS":2,"1xM":1,"2xM":2,"1xL":1,"2xL":2};
...
var DEFAULT_WIDGET_SIZES={"next-up":"2xS","mml":"2xS","funnel":"2xM","weekly-stats":"2xS","kpis":"2xS"};
...
function WeeklyStats(p){
  ...
  var [form,setForm]=useState({ftg:0,salvations:0,baptisms:0,nextSteps:0,ateamAtt:0,newAteam:0});
  ...
  var addWeek=function(){var entry={...form,week:new Date().toISOString().split("T")[0]};p.setConfig({...p.config,weeklyStats:data.concat([entry])});setShowAdd(false);setForm({ftg:0,salvations:0,baptisms:0,nextSteps:0,ateamAtt:0,newAteam:0})};
  ...
  return <div style={{background:"var(--card)",borderRadius:8,overflow:"hidden",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
    <div style={{background:"var(--hero)",padding:"10px 22px"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <h3 style={{fontSize:13,fontWeight:700,color:"#fff"}}>Weekly Church Stats</h3>
        <div style={{display:"flex",gap:4}}>
          {["week","trends","data"].map(function(t){return <button key={t} onClick={function(){setTab(t)}} style={{padding:"5px 10px",borderRadius:6,border:"none",fontSize:9,fontWeight:tab===t?700:500,color:tab===t?"#fff":"rgba(255,255,255,0.5)",background:tab===t?"rgba(255,255,255,0.15)":"transparent",cursor:"pointer"}}>{t==="week"?"This Week":t==="trends"?"Trends":"Data"}</button>})}
          <button onClick={function(){setShowAdd(!showAdd)}} style={{padding:"5px 10px",borderRadius:6,border:"1px solid rgba(255,255,255,0.2)",background:"rgba(255,255,255,0.08)",fontSize:9,fontWeight:600,color:"#fff",cursor:"pointer"}}>{showAdd?"Cancel":"+ Log Week"}</button>
        </div>
      </div>
    </div>

    <Reveal open={showAdd}><div style={{padding:"13px 19px",background:"var(--inp)",borderBottom:"1px solid var(--divider)"}}>
      <div className="weavr-ws-input" style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>
    ...
    {tab==="week"&&<div>
      {recent.length>0?<div className="weavr-ws-grid" style={{display:"grid",gridTemplateColumns:"repeat("+cats.length+",1fr)",gap:4}}>
        {cats.map(function(c){var val=latest[c.key]||0;var delta=getDelta(c.key);var avg=getAvg(c.key);return <div key={c.key} style={{background:c.color+"08",borderRadius:6,padding:"6px 6px",position:"relative",overflow:"hidden",border:"1px solid "+c.color+"12",textAlign:"center"}}>
          <div style={{fontSize:7,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.04em",color:c.color,marginBottom:2}}>{c.short}</div>
          <div style={{fontSize:16,fontWeight:800,color:"var(--text)",lineHeight:1}}>{val}</div>
          <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:3,marginTop:2}}>
            {delta!==0&&<span style={{fontSize:7,fontWeight:700,color:delta>0?"#10B981":"#EF4444"}}>{delta>0?"+":""}{delta}</span>}
            <span style={{fontSize:7,color:"var(--text-muted)"}}>avg {avg}</span>
          </div>
        </div>})}
      </div>:<div style={{textAlign:"center",padding:"12px 0",color:"var(--text-muted)"}}><div style={{fontSize:11,fontWeight:600}}>No weekly data yet</div><p style={{fontSize:9,marginTop:3,opacity:0.7}}>Click "+ Log Week" to start</p></div>}
    </div>}
...
function Overview(p){
  ...
  var mmlBase=people.filter(function(x){return!x.fullyConnected&&!(x.scheduledFollowUps&&x.scheduledFollowUps.length>0&&x.scheduledFollowUps.some(function(fu){return new Date(fu.date)>new Date()}))}).map(function(x){var sc=calcScore(x),d=ago(x.lastContactDate),pri=100-sc,reason="";if(d===null){pri+=30;reason="Never contacted"}else if(d>7){pri+=20;reason=d+"d since contact"}else if(d>3){pri+=10;reason=d+"d since contact"}if((SIDX[x.currentStage]||0)<=1){pri+=10;if(!reason)reason="Early stage"}if(!reason)reason="Low engagement";return{...x,engScore:sc,priority:pri,reason:reason}}).sort(function(a,b){return b.priority-a.priority}).slice(0,10);
  ...
  var [wDragKey,setWDragKey]=useState(null);
  ...
  var startDrag=function(key,e){
    ...
    var handleMove=function(ev){
      ...
      for(var i=0;i<els.length;i++){
        ...
        if(dk&&dk!==wDragRef.current){target=dk;break}
        var sk=els[i].getAttribute("data-slotid");
        if(sk){target="slot:"+sk;break}
      }
    ...
    var handleUp=function(ev){
      ...
      if(dropSlot){
        var parts=dropSlot.split("_");
        doEmptyDrop({row:parseInt(parts[0]),col:parseInt(parts[1]),cols:parseInt(parts[2])});
      }else if(dropWidget){
        doSwapDrop(dropWidget);
      }
      wDragRef.current=null;setWDragKey(null);setWDropTarget(null);
  ...
  var doSwapDrop=function(targetKey){
    var dk=wDragRef.current;if(!dk||dk===targetKey)return;
    var arr=ovWidgets.slice();var fi=arr.indexOf(dk);var ti=arr.indexOf(targetKey);
    if(fi<0||ti<0)return;arr.splice(fi,1);ti=arr.indexOf(targetKey);arr.splice(ti,0,dk);
    p.setConfig({...p.config,overviewWidgets:arr});
  };
  var doEmptyDrop=function(slot){
    var dk=wDragRef.current;if(!dk)return;
    var curSz=getWidgetSize(dk);var curCols=WS_COLS[curSz]||2;
    var nCols={...(p.config.widgetColPrefs||{})};
    if(curCols===1){if(slot.col===1)nCols[dk]="right";else nCols[dk]="left"}else{delete nCols[dk]}
    var arr=ovWidgets.slice();var fi=arr.indexOf(dk);if(fi>=0)arr.splice(fi,1);
    var grid={};var wRows={};
    arr.forEach(function(key){
      var sz=getWidgetSize(key);var wc=WS_COLS[sz]||2;var wr=WS_ROWS[sz]||2;
      var cp=nCols[key]||(p.config.widgetColPrefs||{})[key]||null;
      for(var r=0;r<200;r++){var cols=wc===2?[0]:cp==="right"?[1]:[0,1];var placed=false;
        for(var ci=0;ci<cols.length&&!placed;ci++){var c=cols[ci];if(c+wc>2)continue;var fits=true;
          for(var dr=0;dr<wr&&fits;dr++){for(var dc=0;dc<wc&&fits;dc++){if(grid[(r+dr)+","+(c+dc)])fits=false}}
          if(fits){for(var dr2=0;dr2<wr;dr2++){for(var dc2=0;dc2<wc;dc2++){grid[(r+dr2)+","+(c+dc2)]=key}}wRows[key]=r;placed=true}
        }if(placed)break;}
    });
    var insertAt=0;for(var wi=0;wi<arr.length;wi++){if((wRows[arr[wi]]||0)<slot.row)insertAt=wi+1}
    arr.splice(insertAt,0,dk);
    p.setConfig({...p.config,overviewWidgets:arr,widgetColPrefs:nCols});
  };

  var [qsStep,setQsStep]=useState(null);var [qsFading,setQsFading]=useState(false);
  var [qsTeam,setQsTeam]=useState({name:"",color:"#7C3AED"});
  var [qsPerson,setQsPerson]=useState({firstName:"",lastName:"",phone:"",email:"",stage:"first-visit"});
  var qsColors=["#7C3AED","#3898FF","#10B981","#F59E0B","#EF4444","#EC4899","#06B6D4","#F97316"];

  if(!total&&!p.config.onboardDismissed)return <div style={{display:"flex",alignItems:"center",justifyContent:"center",minHeight:"70vh",padding:"16px"}}><div style={{maxWidth:440,width:"100%"}}>
    {!qsStep&&<div style={{textAlign:"center"}}>
      <h2 style={{fontSize:20,fontWeight:800,color:"var(--text-sub)",marginBottom:6,letterSpacing:"0.1em",textTransform:"uppercase"}}>Welcome to Weavr</h2>
      <p style={{fontSize:11,color:"var(--text-muted)",marginBottom:28}}>Add your first person to get started.</p>
      <div style={{display:"flex",gap:10,justifyContent:"center"}}>
        <button onClick={function(){setQsStep("team")}} style={{padding:"10px 22px",borderRadius:8,border:"none",background:"var(--text-sub)",color:"var(--bg)",fontSize:12,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",gap:6}}><I n="zap" sz={14} c="var(--bg)"/>Quick Start</button>
        <button onClick={p.onAdd} style={{padding:"10px 22px",borderRadius:8,border:"none",background:"var(--text-muted)",color:"var(--bg)",fontSize:12,fontWeight:600,cursor:"pointer",display:"flex",alignItems:"center",gap:6,opacity:0.6}}><I n="plus" sz={14} c="var(--bg)"/>Add Person</button>
      </div>
    </div>}

    {qsStep==="team"&&<div>
      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:16}}>
        <div style={{width:28,height:28,borderRadius:6,background:"var(--primary)12",display:"flex",alignItems:"center",justifyContent:"center"}}><span style={{fontSize:11,fontWeight:700,color:"var(--primary)"}}>1</span></div>
        <div><div style={{fontSize:13,fontWeight:700,color:"var(--text)"}}>Add Your First Team Member</div><div style={{fontSize:10,color:"var(--text-muted)"}}>Who will be following up with visitors?</div></div>
      </div>
      <div style={{background:"var(--card)",borderRadius:8,padding:"16px",border:"1px solid var(--divider)"}}>
        <div style={{marginBottom:10}}>
          <div style={{fontSize:9,fontWeight:700,color:"var(--text-muted)",textTransform:"uppercase",marginBottom:3}}>Name</div>
          <input value={qsTeam.name} onChange={function(e){setQsTeam({...qsTeam,name:e.target.value})}} placeholder="e.g. Dustin West" style={{width:"100%",padding:"8px 10px",borderRadius:6,border:"1px solid var(--inp-border)",background:"var(--inp)",color:"var(--text)",fontSize:12,boxSizing:"border-box",outline:"none"}}/>
        </div>
        <div style={{marginBottom:12}}>
          <div style={{fontSize:9,fontWeight:700,color:"var(--text-muted)",textTransform:"uppercase",marginBottom:3}}>Color</div>
          <div style={{display:"flex",gap:6}}>{qsColors.map(function(c){return <button key={c} onClick={function(){setQsTeam({...qsTeam,color:c})}} style={{width:24,height:24,borderRadius:6,background:c,border:qsTeam.color===c?"2px solid var(--text)":"2px solid transparent",cursor:"pointer"}}/>})}</div>
        </div>
        <div style={{display:"flex",gap:8}}>
          <button onClick={function(){if(!qsTeam.name.trim())return;var nt={id:Date.now().toString(36),name:qsTeam.name.trim(),color:qsTeam.color};p.setConfig({...p.config,teams:(p.config.teams||[]).concat([nt])});setQsStep("person")}} style={{padding:"8px 18px",borderRadius:6,border:"none",background:qsTeam.name.trim()?"var(--text-sub)":"var(--inp)",color:qsTeam.name.trim()?"var(--bg)":"var(--text-muted)",fontSize:11,fontWeight:600,cursor:qsTeam.name.trim()?"pointer":"default"}}>Next →</button>
          <button onClick={function(){setQsStep("person")}} style={{padding:"8px 14px",borderRadius:6,border:"none",background:"transparent",color:"var(--text-muted)",fontSize:10,cursor:"pointer"}}>Skip</button>
        </div>
      </div>
    </div>}
    ...
    {qsStep==="person"&&<div>
      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:16}}>
        <div style={{width:28,height:28,borderRadius:6,background:teams.length>0?"#10B98112":"var(--primary)12",display:"flex",alignItems:"center",justifyContent:"center"}}>{teams.length>0?<I n="check" sz={12} c="#10B981"/>:<span style={{fontSize:11,fontWeight:700,color:"var(--primary)"}}>2</span>}</div>
        <div><div style={{fontSize:13,fontWeight:700,color:"var(--text)"}}>Add Your First Person</div><div style={{fontSize:10,color:"var(--text-muted)"}}>A visitor, a new believer — anyone you want to track.</div></div>
      </div>
      <div style={{background:"var(--card)",borderRadius:8,padding:"16px",border:"1px solid var(--divider)"}}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
          <div><div style={{fontSize:9,fontWeight:700,color:"var(--text-muted)",textTransform:"uppercase",marginBottom:3}}>First Name</div><input value={qsPerson.firstName} onChange={function(e){setQsPerson({...qsPerson,firstName:e.target.value})}} style={{width:"100%",padding:"8px 10px",borderRadius:6,border:"1px solid var(--inp-border)",background:"var(--inp)",color:"var(--text)",fontSize:12,boxSizing:"border-box",outline:"none"}}/></div>
          <div><div style={{fontSize:9,fontWeight:700,color:"var(--text-muted)",textTransform:"uppercase",marginBottom:3}}>Last Name</div><input value={qsPerson.lastName} onChange={function(e){setQsPerson({...qsPerson,lastName:e.target.value})}} style={{width:"100%",padding:"8px 10px",borderRadius:6,border:"1px solid var(--inp-border)",background:"var(--inp)",color:"var(--text)",fontSize:12,boxSizing:"border-box",outline:"none"}}/></div>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
          <div><div style={{fontSize:9,fontWeight:700,color:"var(--text-muted)",textTransform:"uppercase",marginBottom:3}}>Phone</div><input value={qsPerson.phone} onChange={function(e){setQsPerson({...qsPerson,phone:e.target.value})}} style={{width:"100%",padding:"8px 10px",borderRadius:6,border:"1px solid var(--inp-border)",background:"var(--inp)",color:"var(--text)",fontSize:12,boxSizing:"border-box",outline:"none"}}/></div>
          <div><div style={{fontSize:9,fontWeight:700,color:"var(--text-muted)",textTransform:"uppercase",marginBottom:3}}>Email</div><input value={qsPerson.email} onChange={function(e){setQsPerson({...qsPerson,email:e.target.value})}} style={{width:"100%",padding:"8px 10px",borderRadius:6,border:"1px solid var(--inp-border)",background:"var(--inp)",color:"var(--text)",fontSize:12,boxSizing:"border-box",outline:"none"}}/></div>
        </div>
        <div style={{marginBottom:12}}>
          <div style={{fontSize:9,fontWeight:700,color:"var(--text-muted)",textTransform:"uppercase",marginBottom:3}}>Stage</div>
          <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>{STAGES.filter(function(s){return s.key!=="bgroup"&&s.key!=="ateam"}).map(function(s){return <button key={s.key} onClick={function(){setQsPerson({...qsPerson,stage:s.key})}} style={{padding:"4px 10px",borderRadius:6,border:qsPerson.stage===s.key?"1px solid "+s.color:"1px solid var(--inp-border)",background:qsPerson.stage===s.key?s.color+"10":"var(--inp)",fontSize:9,fontWeight:600,color:qsPerson.stage===s.key?s.color:"var(--text-muted)",cursor:"pointer"}}>{s.label}</button>})}</div>
        </div>
        {teams.length>0&&<div style={{marginBottom:12}}>
          <div style={{fontSize:9,fontWeight:700,color:"var(--text-muted)",textTransform:"uppercase",marginBottom:3}}>Assign to</div>
          <div style={{display:"flex",gap:4}}>{(p.config.teams||[]).map(function(t){return <button key={t.id} onClick={function(){setQsPerson({...qsPerson,assignTo:t.id})}} style={{padding:"4px 10px",borderRadius:6,border:qsPerson.assignTo===t.id?"1px solid "+t.color:"1px solid var(--inp-border)",background:qsPerson.assignTo===t.id?t.color+"10":"var(--inp)",fontSize:9,fontWeight:600,color:qsPerson.assignTo===t.id?t.color:"var(--text-muted)",cursor:"pointer"}}>{t.name}</button>})}</div>
        </div>}
        <button onClick={function(){if(!qsPerson.firstName.trim())return;p.onAddPerson({id:Date.now().toString(36)+Math.random().toString(36).slice(2,6),firstName:qsPerson.firstName.trim(),lastName:qsPerson.lastName.trim(),phone:qsPerson.phone,email:qsPerson.email,currentStage:qsPerson.stage,assignedTo:qsPerson.assignTo||"",createdAt:new Date().toISOString(),stages:{[qsPerson.stage]:{date:new Date().toISOString(),completed:true}},checkIns:[],milestones:{}});setQsStep("features")}} style={{padding:"8px 18px",borderRadius:6,border:"none",background:qsPerson.firstName.trim()?"var(--text-sub)":"var(--inp)",color:qsPerson.firstName.trim()?"var(--bg)":"var(--text-muted)",fontSize:11,fontWeight:600,cursor:qsPerson.firstName.trim()?"pointer":"default"}}>Add to Pipeline →</button>
      </div>
    </div>}
  </div></div>;

  if(qsStep==="features")return <div style={{display:"flex",alignItems:"center",justifyContent:"center",minHeight:"70vh",padding:"16px"}}><div style={{maxWidth:480,width:"100%",animation:"fadeUp 0.5s ease"}}>
    <div style={{textAlign:"center",marginBottom:24}}>
      <div style={{fontSize:11,fontWeight:700,color:"#10B981",textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:4}}>Person Added</div>
      <h2 style={{fontSize:18,fontWeight:800,color:"var(--text)",marginBottom:6}}>Here's what you can do next</h2>
      <p style={{fontSize:10,color:"var(--text-muted)"}}>Weavr helps you move people through every step of their journey.</p>
    </div>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
      {[
        {icon:"msg",title:"Follow Up",desc:"Text, call, or email from a contact card with stage-specific templates.",color:"#3898FF"},
        {icon:"clock",title:"Auto Follow-ups",desc:"Log a contact and Weavr schedules the next follow-up automatically.",color:"#06B6D4"},
        {icon:"zap",title:"Work Mode",desc:"Flip through assigned cards one at a time for focused follow-up sessions.",color:"#7C3AED"},
        {icon:"chart",title:"Track Progress",desc:"Engagement funnel, weekly stats, and goals to see your church grow.",color:"#10B981"},
        {icon:"card",title:"Assigned Cards",desc:"Distribute people to team members and track who's following up.",color:"#F59E0B"},
        {icon:"target",title:"Fully Connected",desc:"Celebrate when someone completes their journey from visitor to volunteer.",color:"#F97316"}
      ].map(function(f,i){return <div key={i} style={{background:"var(--card)",borderRadius:8,padding:"12px",border:"1px solid var(--divider)",display:"flex",gap:10,alignItems:"flex-start"}}>
        <div style={{width:28,height:28,borderRadius:6,background:f.color+"12",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><I n={f.icon} sz={13} c={f.color}/></div>
        <div><div style={{fontSize:11,fontWeight:700,color:"var(--text)",marginBottom:2}}>{f.title}</div><div style={{fontSize:9,color:"var(--text-muted)",lineHeight:1.5}}>{f.desc}</div></div>
      </div>})}
    </div>
    <div style={{textAlign:"center",marginTop:20}}>
      <button onClick={function(){setQsStep("celebrate")}} style={{padding:"10px 28px",borderRadius:8,border:"none",background:"var(--text-sub)",color:"var(--bg)",fontSize:12,fontWeight:700,cursor:"pointer"}}>Get Started →</button>
    </div>
  </div></div>;

  if(qsStep==="celebrate"){return <div style={{display:"flex",alignItems:"center",justifyContent:"center",minHeight:"70vh",padding:"16px"}}><div style={{textAlign:"center",animation:"fadeUp 0.5s ease",opacity:qsFading?0:1,transform:qsFading?"translateY(-20px)":"translateY(0)",transition:"all 0.8s cubic-bezier(0.22,1,0.36,1)"}}>
    <div style={{width:64,height:64,borderRadius:8,background:"linear-gradient(135deg,#10B981,#059669)",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 16px",boxShadow:"0 8px 32px rgba(16,185,129,0.3)"}}><I n="check" sz={28} c="#fff"/></div>
    <h2 style={{fontSize:20,fontWeight:800,color:"var(--text)",marginBottom:6}}>You're all set!</h2>
    <p style={{fontSize:11,color:"var(--text-muted)",marginBottom:24}}>Your pipeline is ready. Let's start weaving.</p>
    <button onClick={function(){setQsFading(true);setTimeout(function(){setQsStep(null);p.setConfig({...p.config,onboardDismissed:true})},800)}} style={{padding:"10px 28px",borderRadius:8,border:"none",background:"var(--text-sub)",color:"var(--bg)",fontSize:12,fontWeight:700,cursor:"pointer"}}>Open Dashboard</button>
  </div></div>;}

  var renderWidget=function(key){
    if(key==="next-up"){var top1=mml[0];if(!top1)return null;var stg=STAGES.find(function(s){return s.key===top1.currentStage});var d=ago(top1.lastContactDate);var tm=teams.find(function(t){return t.id===top1.assignedTo});return <div className="weavr-next-up" style={{display:"flex",gap:13,height:"100%",minHeight:0}}>
      <div style={{flex:"1 1 0",minWidth:0,background:"var(--card)",borderRadius:8,padding:"19px 22px",boxShadow:"var(--card-shadow)",display:"flex",alignItems:"center",gap:15,cursor:"pointer",transition:"all 0.2s",border:"1px solid var(--divider)",overflow:"hidden"}} onClick={function(){p.onPerson(top1)}} onMouseEnter={function(e){e.currentTarget.style.transform="translateY(-2px)";e.currentTarget.style.boxShadow="0 4px 20px rgba(0,0,0,0.08)"}} onMouseLeave={function(e){e.currentTarget.style.transform="translateY(0)";e.currentTarget.style.boxShadow="var(--card-shadow)"}}>
        <div style={{width:44,height:44,borderRadius:8,background:stg?stg.grad:"var(--divider)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><I n="zap" sz={18} c="#fff"/></div>
        <div style={{flex:1,minWidth:0}}>
          ...
          <div style={{fontSize:15,fontWeight:700,color:"var(--text)",marginBottom:2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{top1.firstName} {top1.lastName}</div>
          <div style={{fontSize:9,color:"var(--text-muted)",display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}><span style={{color:stg?stg.color:"#999",fontWeight:600}}>{stg?stg.label:"?"}</span><span>{d===null?"Never contacted":d+"d ago"}</span>{tm&&<span style={{color:tm.color}}>{tm.name}</span>}</div>
        </div>
        <div style={{display:"flex",gap:6,flexShrink:0}}>
          {top1.phone&&<button onClick={function(e){e.stopPropagation();p.onContact({person:top1,mode:"text"})}} style={{width:34,height:34,borderRadius:8,background:"#3898FF08",border:"1px solid #3898FF15",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}><I n="msg" sz={14} c="#3898FF"/></button>}
          {top1.phone&&<button onClick={function(e){e.stopPropagation();p.onContact({person:top1,mode:"call"})}} style={{width:34,height:34,borderRadius:8,background:"var(--primary)08",border:"1px solid var(--primary)15",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}><I n="phone" sz={14} c="var(--primary)"/></button>}
          {top1.email&&<button onClick={function(e){e.stopPropagation();p.onContact({person:top1,mode:"email"})}} style={{width:34,height:34,borderRadius:8,background:"#3898FF08",border:"1px solid #3898FF15",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}><I n="mail" sz={14} c="#3898FF"/></button>}
        </div>
      </div>
      <div style={{display:"flex",flexDirection:"column",gap:6,width:160,flexShrink:0}}>
        {[{l:"New This Week",v:people.filter(function(x){return ago(x.createdAt)<=7}).length,c:"#10B981"},{l:"Need Follow-Up",v:people.filter(function(x){return!x.fullyConnected&&(ago(x.lastContactDate)===null||ago(x.lastContactDate)>3)}).length,c:"#EF4444"},{l:"Fully Connected",v:people.filter(function(x){return x.fullyConnected}).length,c:"#F97316"}].map(function(k){return <div key={k.l} style={{flex:1,background:k.c+"08",borderRadius:8,padding:"8px 12px",display:"flex",alignItems:"center",gap:7,border:"1px solid "+k.c+"12",minHeight:0}}><div style={{fontSize:15,fontWeight:800,color:k.c}}>{k.v}</div><div style={{fontSize:8,fontWeight:600,color:"var(--text-muted)",lineHeight:1.2}}>{k.l}</div></div>})}
      </div>
    </div>}

    if(key==="mml")return <div style={{background:"var(--card)",borderRadius:8,padding:"13px 19px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
      <div style={{display:"flex",alignItems:"center",gap:7,marginBottom:mml.length>0?10:0}}>
        <I n="sun" sz={16} c="#F59E0B"/>
        <h3 style={{fontSize:13,fontWeight:700,color:"var(--text)",flex:1}}>Monday Morning List</h3>
        <span style={{fontSize:10,color:"var(--text-muted)",fontWeight:600,background:"var(--inp)",padding:"2px 8px",borderRadius:6}}>{mml.length}</span>
      </div>
      {mml.length===0&&<div style={{display:"flex",alignItems:"center",justifyContent:"center",flex:1,padding:"20px 0",color:"var(--text-muted)",fontSize:10,textAlign:"center"}}>No pending follow-ups — nice work!</div>}
      {mml.length>0&&<div>
        {mml.map(function(x,i){var stg=STAGES.find(function(s){return s.key===x.currentStage});var d=ago(x.lastContactDate);var tm=teams.find(function(t){return t.id===x.assignedTo});return <div key={x.id} style={{display:"flex",alignItems:"center",gap:9,padding:"7px 6px",borderTop:i>0?"1px solid var(--divider)":"none",cursor:"pointer",transition:"all 0.15s",borderRadius:8,margin:"0 -6px"}} onMouseEnter={function(e){e.currentTarget.style.background="var(--hover)"}} onMouseLeave={function(e){e.currentTarget.style.background="transparent"}}><div style={{width:28,height:28,borderRadius:6,background:stg?stg.color+"12":"var(--inp)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:8,fontWeight:700,color:stg?stg.color:"var(--text-muted)",flexShrink:0}} onClick={function(){p.onPerson(x)}}>{(x.firstName||"?").charAt(0)}{(x.lastName||"").charAt(0)}</div><div style={{flex:1,minWidth:0}} onClick={function(){p.onPerson(x)}}><div style={{fontSize:10,fontWeight:600,color:"var(--text)"}}>{x.firstName} {x.lastName}</div><div style={{fontSize:8,color:"var(--text-muted)",display:"flex",alignItems:"center",gap:6,marginTop:1}}><span style={{color:stg?stg.color:"#999",fontWeight:600}}>{stg?stg.label:"?"}</span><span>{d===null?"Never":d+"d ago"}</span>{tm&&<span style={{color:tm.color}}>{tm.name}</span>}{x.reason&&<span style={{background:"var(--divider)",padding:"1px 5px",borderRadius:5,fontSize:7,fontWeight:600,color:"var(--text-muted)"}}>{x.reason}</span>}</div></div><div style={{display:"flex",gap:3,flexShrink:0}}>
          {x.phone&&<button onClick={function(e){e.stopPropagation();p.onContact({person:x,mode:"text"})}} style={{width:22,height:22,borderRadius:6,background:"#3898FF08",border:"1px solid #3898FF15",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}><I n="msg" sz={10} c="#3898FF"/></button>}
          {x.phone&&<button onClick={function(e){e.stopPropagation();p.onContact({person:x,mode:"call"})}} style={{width:22,height:22,borderRadius:6,background:"var(--primary)08",border:"1px solid var(--primary)15",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}><I n="phone" sz={10} c="var(--primary)"/></button>}
          {x.email&&<button onClick={function(e){e.stopPropagation();p.onContact({person:x,mode:"email"})}} style={{width:22,height:22,borderRadius:6,background:"#3898FF08",border:"1px solid #3898FF15",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}><I n="mail" sz={10} c="#3898FF"/></button>}
        </div></div>})}
      </div>}
    </div>;

    if(key==="funnel"){var fGoals={...DEFAULT_FUNNEL_GOALS,...((p.config||{}).funnelGoals||{})};var sparkData=function(stageKey){var now=Date.now();return[28,21,14,7,0].map(function(d){var cutoff=now-d*864e5;return people.filter(function(x){if(stageKey==="bgroup")return x.stages&&x.stages.bgroup&&x.stages.bgroup.completed&&new Date(x.stages.bgroup.date||x.createdAt).getTime()<=cutoff;if(stageKey==="ateam")return x.stages&&x.stages.ateam&&x.stages.ateam.completed&&new Date(x.stages.ateam.date||x.createdAt).getTime()<=cutoff;return x.currentStage===stageKey||(x.stages&&x.stages[stageKey]&&new Date(x.stages[stageKey].date||x.createdAt).getTime()<=cutoff)}).length})};return <div style={{background:"var(--card)",borderRadius:8,padding:"21px 22px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
      <h3 style={{fontSize:13,fontWeight:700,color:"var(--text)",marginBottom:19}}>Engagement Funnel</h3>
      {STAGES.map(function(s){var ct=s.key==="bgroup"?people.filter(function(x){return x.stages&&x.stages.bgroup&&x.stages.bgroup.completed}).length:s.key==="ateam"?people.filter(function(x){return x.stages&&x.stages.ateam&&x.stages.ateam.completed}).length:people.filter(function(x){return x.currentStage===s.key}).length;var goal=fGoals[s.key]||50;var pct=Math.min(ct/goal*100,100);var sd=sparkData(s.key);return <div key={s.key} style={{display:"flex",alignItems:"center",gap:10,marginBottom:11,cursor:"pointer",transition:"all 0.15s"}} onClick={function(){p.navTo("people",s.key)}} onMouseEnter={function(e){e.currentTarget.style.opacity="0.8"}} onMouseLeave={function(e){e.currentTarget.style.opacity="1"}}><div style={{width:80,fontSize:10,fontWeight:600,color:"var(--text-sub)",textAlign:"right",flexShrink:0}}>{s.label}</div><div style={{flex:1,height:10,background:"var(--divider)",borderRadius:5,overflow:"hidden"}}><div style={{height:"100%",width:Math.max(pct,2)+"%",background:s.color,borderRadius:5,transition:"width 1s ease"}}/></div><Spark data={sd} c={s.color} w={36} h={12}/><div style={{width:28,fontSize:12,fontWeight:800,color:s.color,textAlign:"right"}}>{ct}</div></div>})}
      {(function(){var fc=people.filter(function(x){return x.fullyConnected}).length;if(fc===0)return null;var fcGoal=fGoals["fully-connected"]||50;return <div style={{display:"flex",alignItems:"center",gap:13,marginTop:6,cursor:"pointer",padding:"8px 0 0",borderTop:"1px solid var(--divider)"}} onClick={function(){p.navTo("connected")}}><div style={{width:90,fontSize:10,fontWeight:700,color:"#F97316",textAlign:"right",flexShrink:0}}>Connected</div><div style={{flex:1,height:10,background:"var(--divider)",borderRadius:5,overflow:"hidden"}}><div style={{height:"100%",width:Math.max(Math.min(fc/fcGoal*100,100),2)+"%",background:"#F97316",borderRadius:5}}/></div><div style={{width:36,fontSize:12,fontWeight:800,color:"#F97316",textAlign:"right"}}>{fc}</div></div>})()}
    </div>}

    if(key==="weekly-stats")return <WeeklyStats config={p.config} setConfig={p.setConfig}/>;

    if(key==="kpis"){var added7=people.filter(function(x){return ago(x.createdAt)<=7}).length;var needFollow=people.filter(function(x){return!x.fullyConnected&&(ago(x.lastContactDate)===null||ago(x.lastContactDate)>3)}).length;var avgScore=total>0?Math.round(people.map(calcScore).reduce(function(a,b){return a+b},0)/total):0;return <div className="weavr-kpi-grid" style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(110px,1fr))",gap:10}}>{[{l:"Total People",v:total,i:"users",c:"var(--primary)",bg:"var(--primary)"},{l:"Added (7d)",v:added7,i:"plus",c:"#10B981",bg:"#10B981"},{l:"Need Follow-Up",v:needFollow,i:"flag",c:"#EF4444",bg:"#EF4444"},{l:"Avg Score",v:avgScore,i:"target",c:"#06B6D4",bg:"#06B6D4"}].map(function(k){return <div key={k.l} style={{background:k.bg+"0A",borderRadius:8,padding:"10px",overflow:"hidden",border:"1px solid "+k.bg+"15",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}><div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:5,marginBottom:4}}><I n={k.i} sz={13} c={k.c}/><div style={{fontSize:8,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.06em",color:k.c,lineHeight:1.3}}>{k.l}</div></div><div style={{fontSize:20,fontWeight:800,color:"var(--text)",lineHeight:1,textAlign:"center"}}>{k.v}</div></div>})}</div>}

    if(key==="stale"){var stale=people.filter(function(x){return!x.fullyConnected}).filter(function(x){var d=ago(x.lastContactDate);return d===null||d>=14}).sort(function(a,b){var da=ago(a.lastContactDate),db=ago(b.lastContactDate);return(db===null?999:db)-(da===null?999:da)});return <div style={{background:"var(--card)",borderRadius:8,padding:"21px 22px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}><h3 style={{fontSize:13,fontWeight:700,color:"var(--text)",marginBottom:13}}>Stale Contacts (14d+)</h3>{stale.length===0?<div style={{color:"var(--text-muted)",padding:"16px 0",textAlign:"center",fontSize:11}}>No stale contacts</div>:<div style={{maxHeight:255,overflowY:"auto"}}>{stale.slice(0,15).map(function(x,i){var d=ago(x.lastContactDate);var stg=STAGES.find(function(s){return s.key===x.currentStage});return <div key={x.id} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 0",borderTop:i>0?"1px solid var(--divider)":"none",cursor:"pointer"}} onClick={function(){p.onPerson(x)}}><Dot color={stg?stg.color:"#ccc"} sz={8}/><div style={{flex:1,fontSize:10,color:"var(--text)"}}>{x.firstName} {x.lastName}</div><span style={{fontSize:9,fontWeight:600,color:"#EF4444"}}>{d===null?"Never":d+"d"}</span></div>})}</div>}</div>}

    if(key==="recent"){var recentCI=[];people.forEach(function(x){(x.checkIns||[]).forEach(function(c){if(ago(c.date)<=7)recentCI.push({...c,person:x})})});recentCI.sort(function(a,b){return new Date(b.date)-new Date(a.date)});var ciTypes=p.config.checkInTypes||DEFAULT_CI;return <div style={{background:"var(--card)",borderRadius:8,padding:"21px 22px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}><h3 style={{fontSize:13,fontWeight:700,color:"var(--text)",marginBottom:13}}>Recent Check-ins (7d)</h3>{recentCI.length===0?<div style={{color:"var(--text-muted)",padding:"16px 0",textAlign:"center",fontSize:11}}>No recent check-ins</div>:<div style={{maxHeight:255,overflowY:"auto"}}>{recentCI.slice(0,15).map(function(c,i){var ct=ciTypes.find(function(t){return t.key===c.type});return <div key={i} style={{display:"flex",alignItems:"center",gap:7,padding:"5px 0",borderTop:i>0?"1px solid var(--divider)":"none"}}><I n={ct?ct.icon:"msg"} sz={12} c={ct?ct.color:"var(--text-muted)"}/><div style={{flex:1,fontSize:10,color:"var(--text-sub)"}}>{c.person.firstName} {c.person.lastName} - {ct?ct.label:c.type}</div><div style={{fontSize:9,color:"var(--text-muted)"}}>{fmtS(c.date)}</div></div>})}</div>}</div>}

    if(key==="weekly-summary"){var w7=people.filter(function(x){return ago(x.createdAt)<=7}).length;var fu7=0;var adv7=0;var fc7=people.filter(function(x){return x.fullyConnected&&ago(x.fullyConnectedDate)<=7}).length;people.forEach(function(x){(x.checkIns||[]).forEach(function(c){if(ago(c.date)<=7)fu7++});STAGES.forEach(function(s){if(s.key!=="first-visit"&&x.stages&&x.stages[s.key]&&ago(x.stages[s.key].date)<=7)adv7++})});return <div style={{background:"var(--card)",borderRadius:8,padding:"21px 22px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}><h3 style={{fontSize:13,fontWeight:700,color:"var(--text)",marginBottom:13}}>This Week</h3><div className="weavr-weekly-summary-grid" style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:10}}>{[{l:"New People",v:w7,c:"#10B981",i:"plus"},{l:"Follow-ups Done",v:fu7,c:"var(--primary)",i:"check"},{l:"Stage Advances",v:adv7,c:"#06B6D4",i:"up"},{l:"Fully Connected",v:fc7,c:"#F97316",i:"target"}].map(function(k){return <div key={k.l} style={{background:k.c+"08",borderRadius:8,padding:"13px 14px",display:"flex",alignItems:"center",gap:10,border:"1px solid "+k.c+"12"}}><div style={{width:40,height:40,borderRadius:8,background:k.c+"15",display:"flex",alignItems:"center",justifyContent:"center"}}><I n={k.i} sz={17} c={k.c}/></div><div><div style={{fontSize:19,fontWeight:800,color:"var(--text)"}}>{k.v}</div><div style={{fontSize:9,color:"var(--text-muted)",fontWeight:600}}>{k.l}</div></div></div>})}</div></div>}

    if(key==="distribution"){var scoreDist=[{label:"0-25 (Cold)",color:"#EF4444",count:people.filter(function(x){var s=calcScore(x);return s>=0&&s<=25}).length},{label:"26-50 (Warming)",color:"#F59E0B",count:people.filter(function(x){var s=calcScore(x);return s>25&&s<=50}).length},{label:"51-75 (Engaged)",color:"#3898FF",count:people.filter(function(x){var s=calcScore(x);return s>50&&s<=75}).length},{label:"76-100 (Connected)",color:"#10B981",count:people.filter(function(x){var s=calcScore(x);return s>75}).length}];var maxB=Math.max.apply(null,scoreDist.map(function(b){return b.count}).concat([1]));return <div style={{background:"var(--card)",borderRadius:8,padding:"21px 22px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}><h3 style={{fontSize:13,fontWeight:700,color:"var(--text)",marginBottom:13}}>Engagement Distribution</h3>{scoreDist.map(function(b){return <div key={b.label} style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}><div style={{width:50,fontSize:16,fontWeight:700,color:b.color,textAlign:"right"}}>{b.count}</div><div style={{flex:1}}><div style={{height:28,background:"var(--divider)",borderRadius:6,overflow:"hidden"}}><div style={{height:"100%",width:Math.max(b.count/maxB*100,3)+"%",background:b.color,borderRadius:6}}/></div><div style={{fontSize:9,color:"var(--text-muted)",marginTop:2}}>{b.label}</div></div></div>})}</div>}

    if(key==="velocity"){var stageStats=STAGES.filter(function(s){return s.key!=="bgroup"&&s.key!=="ateam"}).map(function(s){var inStg=people.filter(function(x){return x.currentStage===s.key});var avgD=0;if(inStg.length>0){avgD=Math.round(inStg.reduce(function(a,x){var sd=x.stages&&x.stages[s.key]?x.stages[s.key].date:x.createdAt;return a+(ago(sd)||0)},0)/inStg.length)}return{...s,count:inStg.length,avgDays:avgD}});return <div style={{background:"var(--card)",borderRadius:8,padding:"21px 22px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}><h3 style={{fontSize:13,fontWeight:700,color:"var(--text)",marginBottom:13}}>Stage Velocity</h3>{stageStats.map(function(s){return <div key={s.key} style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}><Dot color={s.color} sz={8}/><div style={{flex:1,fontSize:11,fontWeight:500,color:"var(--text-sub)"}}>{s.label}</div><div style={{fontSize:9,color:"var(--text-muted)"}}>{s.count}</div><div style={{width:60,textAlign:"right"}}><span style={{fontSize:13,fontWeight:700,color:"var(--text)"}}>{s.avgDays}</span><span style={{fontSize:9,color:"var(--text-muted)"}}>d</span></div></div>})}</div>}

    if(key==="team"){var teamStats=teams.map(function(t){var assigned=people.filter(function(x){return x.assignedTo===t.id});var followedUp=assigned.filter(function(x){return ago(x.lastContactDate)!==null&&ago(x.lastContactDate)<=7}).length;var asc=assigned.length>0?Math.round(assigned.map(calcScore).reduce(function(a,b){return a+b},0)/assigned.length):0;return{...t,assigned:assigned.length,followedUp:followedUp,avgScore:asc}}).filter(function(t){return t.assigned>0});if(teamStats.length===0)return null;return <div style={{background:"var(--card)",borderRadius:8,padding:"21px 22px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}><h3 style={{fontSize:13,fontWeight:700,color:"var(--text)",marginBottom:13}}>Team Performance</h3>{teamStats.map(function(t){var rate=t.assigned>0?Math.round(t.followedUp/t.assigned*100):0;return <div key={t.id} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 11px",background:"var(--inp)",borderRadius:8,marginBottom:5}}><div style={{width:36,height:36,borderRadius:8,background:t.color+"20",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:700,color:t.color}}>{t.name.charAt(0)}</div><div style={{flex:1}}><div style={{fontSize:11,fontWeight:600,color:"var(--text)"}}>{t.name}</div><div style={{fontSize:9,color:"var(--text-muted)"}}>{t.assigned} assigned - {rate}% followed up</div></div><ScoreRing score={t.avgScore} sz={36}/></div>})}</div>}
    ...
    if(key==="goals"){var fGoals={...DEFAULT_FUNNEL_GOALS,...((p.config||{}).funnelGoals||{})};var customGoals=(p.config||{}).customGoals||[];
      ...
      return <div style={{background:"var(--card)",borderRadius:8,padding:"21px 22px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
        ...
        {allGoals.map(function(g,i){var pct=g.goal>0?Math.min(Math.round(g.current/g.goal*100),100):0;var hit=pct>=100;return <div key={i} style={{marginBottom:10}}>
          ...
          <div style={{height:8,background:"var(--divider)",borderRadius:5,overflow:"hidden"}}>
            <div style={{height:"100%",width:Math.max(pct,2)+"%",background:hit?"#10B981":g.color,borderRadius:5,transition:"width 0.8s ease"}}/>
          </div>
        </div>})}
      </div>}
    ...
    if(key==="scheduled"){
      var allFu=[];
      people.forEach(function(x){(x.scheduledFollowUps||[]).forEach(function(fu){allFu.push({...fu,person:x})})});
      allFu.sort(function(a,b){return new Date(a.date)-new Date(b.date)});
      var now=new Date();
      var due=allFu.filter(function(f){return new Date(f.date)<=now});
      var upcoming=allFu.filter(function(f){return new Date(f.date)>now});
      var fuRow=function(f,i,isDue){var stg=STAGES.find(function(s){return s.key===f.stage});var daysUntil=isDue?0:Math.ceil((new Date(f.date)-now)/864e5);var fuTplBody=(DEFAULT_FOLLOWUP_TPL[f.stage]||"Hey {firstName}, just following up!").replace("{firstName}",f.person.firstName).replace("{lastName}",f.person.lastName||"");return <div key={(isDue?"d":"u")+i} style={{display:"flex",alignItems:"center",gap:7,padding:"7px 0",borderTop:i>0?"1px solid var(--divider)":"none"}}>
        <div style={{width:24,height:24,borderRadius:6,background:isDue?"#EF444412":stg?stg.color+"12":"var(--inp)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:7,fontWeight:700,color:isDue?"#EF4444":stg?stg.color:"var(--text-muted)",flexShrink:0,cursor:"pointer"}} onClick={function(){p.onPerson(f.person)}}>{(f.person.firstName||"?").charAt(0)}{(f.person.lastName||"").charAt(0)}</div>
        <div style={{flex:1,minWidth:0,cursor:"pointer"}} onClick={function(){p.onPerson(f.person)}}>
          <div style={{fontSize:10,fontWeight:600,color:"var(--text)"}}>{f.person.firstName} {f.person.lastName}</div>
          <div style={{fontSize:8,color:isDue?"#EF4444":"var(--text-muted)",fontWeight:isDue?600:500}}>{f.label}{!isDue?" — in "+daysUntil+"d":""}</div>
        </div>
        <div style={{display:"flex",gap:3,flexShrink:0}}>
          {f.person.phone&&<button onClick={function(e){e.stopPropagation();var fuCfg2=(p.config.followUpConfig||{})[f.stage]||{};var fuBody=((fuCfg2.templates&&fuCfg2.templates[0]?fuCfg2.templates[0].body:fuCfg2.template)||DEFAULT_FOLLOWUP_TPL[f.stage]||"").replace("{firstName}",f.person.firstName).replace("{lastName}",f.person.lastName||"");p.onContact({person:f.person,mode:"text",fuTemplate:{label:f.label,body:fuBody,allTpls:(fuCfg2.templates||[{name:"Default",body:fuBody}]).map(function(t){return{name:t.name,body:(t.body||"").replace("{firstName}",f.person.firstName).replace("{lastName}",f.person.lastName||"")}})}})}} style={{width:22,height:22,borderRadius:6,background:"#3898FF08",border:"1px solid #3898FF15",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}><I n="msg" sz={10} c="#3898FF"/></button>}
          {f.person.phone&&<button onClick={function(e){e.stopPropagation();p.onContact({person:f.person,mode:"call"})}} style={{width:22,height:22,borderRadius:6,background:"var(--primary)08",border:"1px solid var(--primary)15",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}><I n="phone" sz={10} c="var(--primary)"/></button>}
          {f.person.email&&<button onClick={function(e){e.stopPropagation();var fuCfg2=(p.config.followUpConfig||{})[f.stage]||{};var fuBody=((fuCfg2.templates&&fuCfg2.templates[0]?fuCfg2.templates[0].body:fuCfg2.template)||DEFAULT_FOLLOWUP_TPL[f.stage]||"").replace("{firstName}",f.person.firstName).replace("{lastName}",f.person.lastName||"");p.onContact({person:f.person,mode:"email",fuTemplate:{label:f.label,subj:"Following up with "+f.person.firstName,body:fuBody,allTpls:(fuCfg2.templates||[{name:"Default",body:fuBody}]).map(function(t){return{name:t.name,subj:"Following up with "+f.person.firstName,body:(t.body||"").replace("{firstName}",f.person.firstName).replace("{lastName}",f.person.lastName||"")}})}})}} style={{width:22,height:22,borderRadius:6,background:"#3898FF08",border:"1px solid #3898FF15",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}><I n="mail" sz={10} c="#3898FF"/></button>}
        </div>
        {isDue&&<span style={{fontSize:7,fontWeight:700,color:"#EF4444",background:"#EF444410",padding:"2px 5px",borderRadius:5,flexShrink:0}}>Due</span>}
        <button onClick={function(e){e.stopPropagation();if(p.onUpdatePerson){var nfu=(f.person.scheduledFollowUps||[]).filter(function(fu2){return fu2.date!==f.date||fu2.label!==f.label});p.onUpdatePerson({...f.person,scheduledFollowUps:nfu});if(p.showToast)p.showToast(f.person.firstName+" — follow-up completed")}}} style={{width:20,height:20,borderRadius:5,background:"var(--inp)",border:"1px solid var(--inp-border)",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",flexShrink:0}} title="Mark done"><I n="check" sz={9} c="var(--text-muted)"/></button>
      </div>};
      return <div style={{background:"var(--card)",borderRadius:8,padding:"21px 22px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
          <h3 style={{fontSize:13,fontWeight:700,color:"var(--text)"}}>Scheduled Follow-ups</h3>
          {due.length>0&&<span style={{fontSize:9,fontWeight:700,color:"#EF4444",background:"#EF444410",padding:"2px 8px",borderRadius:6}}>{due.length} due</span>}
        </div>
        {allFu.length===0&&<div style={{color:"var(--text-muted)",fontSize:11,textAlign:"center",padding:"16px 0"}}>No scheduled follow-ups yet. Log a contact to auto-schedule.</div>}
        {due.map(function(f,i){return fuRow(f,i,true)})}
        {due.length>0&&upcoming.length>0&&<div style={{height:1,background:"var(--divider)",margin:"6px 0"}}/>}
        {upcoming.slice(0,8).map(function(f,i){return fuRow(f,i,false)})}
      </div>}
  ...
  var computeLayout=function(excludeKey){
    var grid={};var positions={};
    var activeWidgets=ovWidgets.filter(function(k){return k!==excludeKey&&renderWidget(k)!==null});
    activeWidgets.forEach(function(key){
      var sz=getWidgetSize(key);var wc=WS_COLS[sz]||2;var wr=WS_ROWS[sz]||2;
      var cp=(p.config.widgetColPrefs||{})[key]||null;
      var placed=false;
      for(var r=0;r<100&&!placed;r++){
        var startCols=wc===2?[0]:cp==="right"?[1]:[0,1];
        for(var ci=0;ci<startCols.length&&!placed;ci++){
          var c=startCols[ci];if(c+wc>2)continue;
          var fits=true;
          for(var dr=0;dr<wr&&fits;dr++){for(var dc=0;dc<wc&&fits;dc++){if(grid[(r+dr)+","+(c+dc)])fits=false}}
          if(fits){
            for(var dr2=0;dr2<wr;dr2++){for(var dc2=0;dc2<wc;dc2++){grid[(r+dr2)+","+(c+dc2)]=key}}
            positions[key]={row:r,col:c,cols:wc,rows:wr};
            placed=true;
          }
        }
      }
    });
    var maxRow=0;Object.keys(grid).forEach(function(k){var r=parseInt(k.split(",")[0]);if(r>=maxRow)maxRow=r+1});
    maxRow=Math.max(maxRow+2,4);
    /* Build granular empty slots — each is exactly 1 row tall, 1 or 2 cols wide */
    var emptySlots=[];
    for(var r=0;r<maxRow;r++){
      var c=0;
      while(c<2){
        if(grid[r+","+c]){c++;continue}
        /* Find how many consecutive empty cols starting at c */
        var ew=0;while(c+ew<2&&!grid[r+","+(c+ew)])ew++;
        emptySlots.push({row:r,col:c,cols:ew,rows:1,size:ew+"xS"});
        c+=ew;
      }
    }
    return{positions:positions,emptySlots:emptySlots};
  };
  
  /* Always compute full layout for rendering all widgets */
  var fullLayout=computeLayout(null);
  var widgetPositions=fullLayout.positions;
  /* For empty slots during drag: use the SAME grid as displayed, but treat dragged widget cells as empty */
  var emptySlots=[];
  if(wDragKey){
    /* Rebuild the occupied grid from displayed positions, excluding dragged widget */
    var dispGrid={};
    Object.keys(widgetPositions).forEach(function(k){
      if(k===wDragKey)return;
      var pos=widgetPositions[k];
      for(var dr=0;dr<pos.rows;dr++){for(var dc=0;dc<pos.cols;dc++){dispGrid[(pos.row+dr)+","+(pos.col+dc)]=k}}
    });
    var maxRow=0;Object.keys(dispGrid).forEach(function(k){var r=parseInt(k.split(",")[0]);if(r>=maxRow)maxRow=r+1});
    maxRow=Math.max(maxRow+2,4);
    for(var r=0;r<maxRow;r++){
      var c=0;
      while(c<2){
        if(dispGrid[r+","+c]){c++;continue}
        var ew=0;while(c+ew<2&&!dispGrid[r+","+(c+ew)])ew++;
        emptySlots.push({row:r,col:c,cols:ew,rows:1,size:ew+"xS"});
        c+=ew;
      }
    }
  ...
  return <div>
    <div className="weavr-pipeline-hero" style={{background:"var(--hero)",borderRadius:8,padding:"28px 30px 24px",marginBottom:22,position:"relative",overflow:"hidden"}}>
      <div style={{position:"absolute",inset:0,background:"radial-gradient(ellipse at 20% 50%,rgba(255,255,255,0.06) 0%,transparent 60%)",pointerEvents:"none"}}/>
      <div style={{position:"absolute",top:0,left:0,right:0,height:1,background:"linear-gradient(90deg,transparent,rgba(255,255,255,0.12) 30%,rgba(255,255,255,0.12) 70%,transparent)",pointerEvents:"none"}}/>
      <div style={{position:"relative",zIndex:1}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
          <div>
            <h2 style={{fontSize:16,fontWeight:700,color:"#fff",letterSpacing:"0.1em",lineHeight:1.1,textTransform:"uppercase"}}>Connect Engine</h2>
            <p style={{fontSize:9,color:"rgba(255,255,255,0.35)",marginTop:4,fontWeight:500,letterSpacing:"0.04em"}}>{total} {total===1?"person":"people"} in pipeline</p>
          </div>
          <button onClick={function(){setShowWidgetPicker(!showWidgetPicker)}} style={{padding:"6px 11px",borderRadius:8,background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",cursor:"pointer",display:"flex",alignItems:"center",gap:4,fontSize:9,fontWeight:600,color:"rgba(255,255,255,0.5)",backdropFilter:"blur(4px)",transition:"all 0.2s"}} onMouseEnter={function(e){e.currentTarget.style.background="rgba(255,255,255,0.12)"}} onMouseLeave={function(e){e.currentTarget.style.background="rgba(255,255,255,0.06)"}}><I n="gear" sz={11} c="rgba(255,255,255,0.5)"/>Customize</button>
        </div>
        <div className="weavr-pipeline-grid" style={{display:"grid",gridTemplateColumns:"repeat(6,1fr)",gap:8}}>{STAGES.map(function(s,i){var ct=s.key==="bgroup"?people.filter(function(x){return x.stages&&x.stages.bgroup&&x.stages.bgroup.completed}).length:s.key==="ateam"?people.filter(function(x){return x.stages&&x.stages.ateam&&x.stages.ateam.completed}).length:people.filter(function(x){return x.currentStage===s.key}).length;return <div key={s.key} data-stagecolor={s.color} style={{background:"rgba(255,255,255,0.05)",borderRadius:8,padding:"18px 10px",cursor:"pointer",border:"1px solid rgba(255,255,255,0.06)",textAlign:"center",transition:"all 0.25s cubic-bezier(0.22,1,0.36,1)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}} onClick={function(){p.navTo("people",s.key)}} onMouseEnter={function(e){e.currentTarget.style.background=s.color+"18";e.currentTarget.style.borderColor=s.color+"30";e.currentTarget.style.transform="translateY(-3px)";e.currentTarget.style.boxShadow="0 8px 24px rgba(0,0,0,0.15)";e.currentTarget.querySelector("[data-num]").style.color=s.color}} onMouseLeave={function(e){e.currentTarget.style.background="rgba(255,255,255,0.05)";e.currentTarget.style.borderColor="rgba(255,255,255,0.06)";e.currentTarget.style.transform="translateY(0)";e.currentTarget.style.boxShadow="none";e.currentTarget.querySelector("[data-num]").style.color="#fff"}}><div data-num="1" style={{fontSize:24,fontWeight:800,color:"#fff",marginBottom:4,lineHeight:1,transition:"color 0.25s"}}>{ct}</div><div style={{fontSize:9,fontWeight:600,color:"rgba(255,255,255,0.5)"}}>{s.label}</div></div>})}</div>
      </div>
    </div>

    <Reveal open={showWidgetPicker}><div style={{background:"var(--card)",borderRadius:8,padding:"19px 22px",marginBottom:19,boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
...
function PeopleView(p){
  ...
  return <div>
    ...
    <div style={{display:"flex",gap:10,alignItems:"center",marginBottom:13}}>
      <div style={{flex:1,display:"flex",alignItems:"center",gap:10,background:"var(--card)",borderRadius:8,padding:"0 14px",border:"1px solid var(--inp-border)"}}><I n="search" sz={16} c="var(--text-muted)"/><input style={{flex:1,padding:"10px 0",border:"none",background:"transparent",color:"var(--text)",fontSize:12,outline:"none"}} placeholder="Search..." value={p.search} onChange={function(e){p.setSearch(e.target.value)}}/>{p.search&&<button style={{background:"none",border:"none",padding:3,cursor:"pointer"}} onClick={function(){p.setSearch("")}}><I n="x" sz={14} c="var(--text-muted)"/></button>}</div>
      <SortBar value={sortKey} onChange={setSortKey}/>
    </div>
    <div style={{background:"var(--card)",borderRadius:8,padding:0,overflow:"hidden",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
      ...
      <tbody>{fil.map(function(x){var s=STAGES.find(function(st){return st.key===x.currentStage});var d=ago(x.lastContactDate);var urg=d===null||d>3;var sc=calcScore(x);var tm=p.teams.find(function(t){return t.id===x.assignedTo});return <tr key={x.id} style={{cursor:"pointer",transition:"background 0.15s"}} onMouseEnter={function(e){e.currentTarget.style.background="var(--hover)"}} onMouseLeave={function(e){e.currentTarget.style.background=""}}>
        <td style={{padding:"11px 14px",borderBottom:"1px solid var(--divider)"}} onClick={function(){p.onPerson(x)}}><div style={{display:"flex",alignItems:"center",gap:10}}><div style={{width:36,height:36,borderRadius:8,background:s?s.color+"10":"var(--inp)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:700,color:s?s.color:"var(--text-muted)",flexShrink:0}}>{(x.firstName||"?").charAt(0)}{(x.lastName||"").charAt(0)}</div><span style={{fontSize:12,fontWeight:600,color:"var(--text)"}}>{x.firstName} {x.lastName}</span></div></td>
        <td style={{padding:"11px 14px",borderBottom:"1px solid var(--divider)"}} onClick={function(){p.onPerson(x)}}><span style={{display:"inline-flex",alignItems:"center",gap:5,fontSize:10,fontWeight:600,color:s?s.color:"#999",background:s?s.color+"10":"var(--inp)",padding:"3px 8px",borderRadius:6}}><Dot color={s?s.color:"#ccc"} sz={6}/>{s?s.label:"?"}</span></td>
        ...
        <td style={{padding:"11px 14px",borderBottom:"1px solid var(--divider)"}} onClick={function(){p.onPerson(x)}}>{urg?<span style={{fontSize:9,fontWeight:600,padding:"3px 10px",borderRadius:6,background:"#FEF2F2",color:"#EF4444",border:"1px solid #FECACA"}}>Follow Up</span>:<span style={{fontSize:9,fontWeight:600,padding:"3px 10px",borderRadius:6,background:"#ECFDF5",color:"#10B981",border:"1px solid #D1FAE5"}}>On Track</span>}</td>
        ...
        <td style={{padding:"11px 14px",borderBottom:"1px solid var(--divider)",textAlign:"center"}}><div style={{display:"flex",gap:2,justifyContent:"center"}}><button onClick={function(e){e.stopPropagation();var lin=STAGES.filter(function(st){return st.key!=="bgroup"&&st.key!=="ateam"});var li=lin.findIndex(function(st){return st.key===x.currentStage});if(li<lin.length-1){var nk=lin[li+1].key;p.onUpdate({...x,currentStage:nk,stages:{...(x.stages||{}),[nk]:{date:new Date().toISOString(),completed:true}}});if(p.showToast)p.showToast(x.firstName+" → "+lin[li+1].label)}}} style={{background:"none",border:"none",padding:3,cursor:"pointer",opacity:0.3}} onMouseEnter={function(e){e.currentTarget.style.opacity="1"}} onMouseLeave={function(e){e.currentTarget.style.opacity="0.3"}} title="Advance stage"><I n="up" sz={14} c="var(--primary)"/></button><button onClick={function(e){e.stopPropagation();if(confirm("Delete "+x.firstName+" "+x.lastName+"?"))p.onDelete(x.id)}} style={{background:"none",border:"none",padding:3,cursor:"pointer",opacity:0.3}} onMouseEnter={function(e){e.currentTarget.style.opacity="1"}} onMouseLeave={function(e){e.currentTarget.style.opacity="0.3"}}><I n="trash" sz={14} c="#EF4444"/></button></div></td>
      </tr>})}</tbody></table>
...
function AssignedCards(p){
  ...
  var acView=p.acView||"grid";var setAcView=p.setAcView||function(){};
  ...
  var [workLastLog,setWorkLastLog]=useState(null);
  var [workTab,setWorkTab]=useState("cards");
  var tm=p.teams.find(function(t){return t.id===selTeam});
  var assigned=sortPeople(p.people.filter(function(x){return x.assignedTo===selTeam&&!x.fullyConnected&&!(x.scheduledFollowUps&&x.scheduledFollowUps.length>0)}),acSort);
  ...
  var workNext=function(){if(workIdx<assigned.length-1){setWorkIdx(workIdx+1);setWorkLastLog(null)}};
  var workPrev=function(){if(workIdx>0){setWorkIdx(workIdx-1);setWorkLastLog(null)}};
  ...
  return <div>
    <div className="weavr-ac-header" style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
      ...
      <div style={{display:"flex",gap:5}}>
        <div style={{display:"flex",gap:2,background:"var(--inp)",borderRadius:8,padding:2}}>
          {["grid","list"].map(function(v){return <button key={v} onClick={function(){setAcView(v);setWorkMode(false)}} style={{padding:"5px 10px",borderRadius:6,border:"none",fontSize:9,fontWeight:600,cursor:"pointer",background:!workMode&&acView===v?"var(--card)":"transparent",color:!workMode&&acView===v?"var(--text)":"var(--text-muted)",boxShadow:!workMode&&acView===v?"0 1px 4px rgba(0,0,0,0.08)":"none"}}>{v==="grid"?"Grid":"List"}</button>})}
        </div>
        <button onClick={function(){setWorkMode(!workMode);setWorkIdx(0);setWorkFlash("")}} style={{display:"flex",alignItems:"center",gap:5,padding:"6px 13px",borderRadius:8,border:workMode?"2px solid var(--primary)":"2px solid var(--inp-border)",background:workMode?"var(--primary)08":"var(--inp)",fontSize:10,fontWeight:600,color:workMode?"var(--primary)":"var(--text-sub)",cursor:"pointer"}}><I n="zap" sz={14} c={workMode?"var(--primary)":"var(--text-muted)"}/>{workMode?"Exit Work Mode":"Work Mode"}</button>
      </div>
    </div>
    <div className="weavr-ac-teams" style={{display:"flex",gap:7,marginBottom:16,flexWrap:"wrap"}}>
      {p.teams.map(function(t){var ct=p.people.filter(function(x){return x.assignedTo===t.id&&!x.fullyConnected}).length;return <button key={t.id} onClick={function(){setSelTeam(t.id);setWorkIdx(0)}} style={{display:"flex",alignItems:"center",gap:5,padding:"6px 13px",borderRadius:8,border:selTeam===t.id?"2px solid "+t.color:"2px solid transparent",background:selTeam===t.id?"var(--card)":"var(--inp)",fontSize:10,fontWeight:600,color:selTeam===t.id?t.color:"var(--text-sub)",cursor:"pointer"}}><div style={{width:24,height:24,borderRadius:6,background:t.color+"20",display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,fontWeight:700,color:t.color}}>{t.name.charAt(0)}</div>{t.name} ({ct})</button>})}
    </div>
    {!workMode&&<div style={{marginBottom:13}}><SortBar value={acSort} onChange={setAcSort}/></div>}
    {p.teams.length===0?<div style={{background:"var(--card)",borderRadius:8,padding:32,textAlign:"center",color:"var(--text-muted)"}}><I n="users" sz={28} c="var(--text-muted)"/><div style={{fontWeight:600,marginTop:8}}>Add team members in Settings first</div></div>:
    workMode?(function(){
      var teamFus=[];
      p.people.filter(function(x){return x.assignedTo===selTeam}).forEach(function(x){(x.scheduledFollowUps||[]).forEach(function(fu){teamFus.push({...fu,person:x})})});
      teamFus.sort(function(a,b){return new Date(a.date)-new Date(b.date)});
      var now=new Date();
      var dueFus=teamFus.filter(function(f){return new Date(f.date)<=now});
      return <div>
        <div style={{display:"flex",gap:3,marginBottom:13,background:"var(--inp)",borderRadius:8,padding:2,width:"fit-content"}}>
          <button onClick={function(){setWorkTab("cards")}} style={{padding:"5px 14px",borderRadius:6,border:"none",fontSize:10,fontWeight:600,cursor:"pointer",background:workTab==="cards"?"var(--card)":"transparent",color:workTab==="cards"?"var(--text)":"var(--text-muted)",boxShadow:workTab==="cards"?"0 1px 4px rgba(0,0,0,0.08)":"none"}}>New Cards ({assigned.length})</button>
          <button onClick={function(){setWorkTab("scheduled")}} style={{padding:"5px 14px",borderRadius:6,border:"none",fontSize:10,fontWeight:600,cursor:"pointer",background:workTab==="scheduled"?"var(--card)":"transparent",color:workTab==="scheduled"?"var(--text)":"var(--text-muted)",boxShadow:workTab==="scheduled"?"0 1px 4px rgba(0,0,0,0.08)":"none"}}>Scheduled {dueFus.length>0?"("+dueFus.length+" due)":""}</button>
        </div>
        {workTab==="scheduled"?<div style={{background:"var(--card)",borderRadius:8,padding:"21px 22px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
          <h3 style={{fontSize:13,fontWeight:700,color:"var(--text)",marginBottom:13}}>Scheduled Follow-ups{tm?" — "+tm.name:""}</h3>
          {teamFus.length===0&&<div style={{color:"var(--text-muted)",fontSize:11,textAlign:"center",padding:"24px 0"}}>No scheduled follow-ups for this team.</div>}
          {teamFus.map(function(f,i){var stg=STAGES.find(function(s){return s.key===f.stage});var isDue=new Date(f.date)<=now;var daysUntil=Math.ceil((new Date(f.date)-now)/864e5);var fuCfg=(p.config.followUpConfig||{})[f.stage]||{};var fuTpl=(fuCfg.template||DEFAULT_FOLLOWUP_TPL[f.stage]||"").replace("{firstName}",f.person.firstName).replace("{lastName}",f.person.lastName||"");return <div key={i} style={{display:"flex",alignItems:"center",gap:8,padding:"9px 0",borderTop:i>0?"1px solid var(--divider)":"none"}}>
            <div style={{width:28,height:28,borderRadius:6,background:isDue?"#EF444412":stg?stg.color+"12":"var(--inp)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:8,fontWeight:700,color:isDue?"#EF4444":stg?stg.color:"var(--text-muted)",flexShrink:0,cursor:"pointer"}} onClick={function(){p.onPerson(f.person)}}>{(f.person.firstName||"?").charAt(0)}{(f.person.lastName||"").charAt(0)}</div>
            <div style={{flex:1,minWidth:0,cursor:"pointer"}} onClick={function(){p.onPerson(f.person)}}>
              <div style={{fontSize:11,fontWeight:600,color:"var(--text)"}}>{f.person.firstName} {f.person.lastName}</div>
              <div style={{fontSize:9,color:isDue?"#EF4444":"var(--text-muted)",fontWeight:isDue?600:500}}>{f.label}{!isDue?" — in "+daysUntil+"d":""}</div>
            </div>
            <div style={{display:"flex",gap:4,flexShrink:0}}>
              {f.person.phone&&<button onClick={function(){var fuCfg2=(p.config.followUpConfig||{})[f.stage]||{};var fuBody=((fuCfg2.templates&&fuCfg2.templates[0]?fuCfg2.templates[0].body:fuCfg2.template)||DEFAULT_FOLLOWUP_TPL[f.stage]||"").replace("{firstName}",f.person.firstName).replace("{lastName}",f.person.lastName||"");p.onContact({person:f.person,mode:"text",fuTemplate:{label:f.label,body:fuBody,allTpls:(fuCfg2.templates||[{name:"Default",body:fuBody}]).map(function(t){return{name:t.name,body:(t.body||"").replace("{firstName}",f.person.firstName).replace("{lastName}",f.person.lastName||"")}})}})}} style={{padding:"5px 10px",borderRadius:6,background:"#3898FF10",border:"1px solid #3898FF20",fontSize:9,fontWeight:600,color:"#3898FF",cursor:"pointer",display:"flex",alignItems:"center",gap:3}}><I n="msg" sz={10} c="#3898FF"/>Text</button>}
              {f.person.phone&&<button onClick={function(){p.onContact({person:f.person,mode:"call"})}} style={{padding:"5px 10px",borderRadius:6,background:"var(--primary)10",border:"1px solid var(--primary)20",fontSize:9,fontWeight:600,color:"var(--primary)",cursor:"pointer",display:"flex",alignItems:"center",gap:3}}><I n="phone" sz={10} c="var(--primary)"/>Call</button>}
              {f.person.email&&<button onClick={function(){var fuCfg2=(p.config.followUpConfig||{})[f.stage]||{};var fuBody=((fuCfg2.templates&&fuCfg2.templates[0]?fuCfg2.templates[0].body:fuCfg2.template)||DEFAULT_FOLLOWUP_TPL[f.stage]||"").replace("{firstName}",f.person.firstName).replace("{lastName}",f.person.lastName||"");p.onContact({person:f.person,mode:"email",fuTemplate:{label:f.label,subj:"Following up with "+f.person.firstName,body:fuBody,allTpls:(fuCfg2.templates||[{name:"Default",body:fuBody}]).map(function(t){return{name:t.name,subj:"Following up with "+f.person.firstName,body:(t.body||"").replace("{firstName}",f.person.firstName).replace("{lastName}",f.person.lastName||"")}})}})}} style={{padding:"5px 10px",borderRadius:6,background:"#3898FF10",border:"1px solid #3898FF20",fontSize:9,fontWeight:600,color:"#10B981",cursor:"pointer",display:"flex",alignItems:"center",gap:3}}><I n="mail" sz={10} c="#3898FF"/>Email</button>}
            </div>
            {isDue&&<span style={{fontSize:8,fontWeight:700,color:"#EF4444",background:"#EF444410",padding:"2px 6px",borderRadius:5,flexShrink:0}}>Due</span>}
          </div>})}
        </div>
        :workPerson?<div style={{background:"var(--card)",borderRadius:8,overflow:"hidden",boxShadow:"0 4px 24px rgba(0,0,0,0.06)",animation:"gentleFade 0.4s cubic-bezier(0.22,1,0.36,1)"}}>
      ...
      <div style={{padding:"16px 26px"}}>
        <div style={{display:"flex",alignItems:"center",gap:7,marginBottom:13,flexWrap:"wrap"}}>
          {workPerson.phone&&<button onClick={function(){p.onContact({person:workPerson,mode:"text",onLogDone:function(type,via){setWorkLastLog({type:type,via:via,fuLabel:FOLLOW_UP_LABELS[workPerson.currentStage],fuDays:FOLLOW_UP_DELAYS[workPerson.currentStage]})}})}} style={{padding:"6px 13px",borderRadius:8,background:"#3898FF10",fontSize:10,fontWeight:600,color:"#3898FF",display:"flex",alignItems:"center",gap:4,border:"1px solid #3898FF25",cursor:"pointer"}}><I n="msg" sz={13} c="#3898FF"/>Text</button>}
          {workPerson.phone&&<button onClick={function(){p.onContact({person:workPerson,mode:"call",onLogDone:function(type,via){setWorkLastLog({type:type,via:via})}})}} style={{padding:"6px 13px",borderRadius:8,background:"var(--primary)10",fontSize:10,fontWeight:600,color:"var(--primary)",display:"flex",alignItems:"center",gap:4,border:"1px solid var(--primary)25",cursor:"pointer"}}><I n="phone" sz={13} c="var(--primary)"/>Call</button>}
          {workPerson.email&&<button onClick={function(){p.onContact({person:workPerson,mode:"email",onLogDone:function(type,via){setWorkLastLog({type:type,via:via,fuLabel:FOLLOW_UP_LABELS[workPerson.currentStage],fuDays:FOLLOW_UP_DELAYS[workPerson.currentStage]})}})}} style={{padding:"6px 13px",borderRadius:8,background:"#3898FF10",fontSize:10,fontWeight:600,color:"#3898FF",display:"flex",alignItems:"center",gap:4,border:"1px solid #3898FF25",cursor:"pointer"}}><I n="mail" sz={13} c="#3898FF"/>Email</button>}
          <div style={{flex:1}}/>
        ...
        {workLastLog?<div style={{marginBottom:11,borderRadius:8,overflow:"hidden"}}>
          <div style={{display:"flex",alignItems:"center",gap:7,padding:"8px 12px",background:"#3898FF10",border:"1px solid #3898FF25",borderRadius:workLastLog.fuLabel?"10px 10px 0 0":"10px"}}><I n="check" sz={13} c="#10B981"/><span style={{fontSize:10,fontWeight:600,color:"#10B981"}}>{workLastLog.type==="call"?"Call":"Message"} logged via {workLastLog.via}</span></div>
          {workLastLog.fuLabel&&<div style={{display:"flex",alignItems:"center",gap:7,padding:"8px 12px",background:"#3898FF08",border:"1px solid #3898FF15",borderTop:"none",borderRadius:"0 0 10px 10px"}}><I n="clock" sz={13} c="#3898FF"/><span style={{fontSize:10,fontWeight:500,color:"#3898FF"}}>Auto follow-up scheduled: <b>{workLastLog.fuLabel}</b> in {workLastLog.fuDays} days</span></div>}
        </div>
        :<div style={{background:"var(--inp)",borderRadius:8,padding:"8px 11px",marginBottom:11,fontSize:10,color:"var(--text-sub)",display:"flex",alignItems:"center",gap:7}}><I n="zap" sz={14} c={workStg?workStg.color:"var(--text-muted)"}/><b style={{color:"var(--text)"}}>Next:</b>{NEXT_ACT[workPerson.currentStage]}</div>}
        {workPerson.currentStage==="salvation"&&<div style={{background:"var(--inp)",borderRadius:8,padding:"8px 11px",marginBottom:11,display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:9,fontWeight:600,color:"var(--text-muted)"}}>Date Saved</span><input type="date" value={workMs.dateSaved||""} onChange={function(e){workUpMs({dateSaved:e.target.value})}} style={{padding:"5px 8px",borderRadius:6,border:"1px solid var(--inp-border)",background:"var(--card)",color:"var(--text)",fontSize:10}}/></div>}
        {workPerson.currentStage==="baptism"&&<div style={{background:"var(--inp)",borderRadius:8,padding:"8px 11px",marginBottom:11,display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:9,fontWeight:600,color:"var(--text-muted)"}}>Date Baptized</span><input type="date" value={workMs.dateBaptized||""} onChange={function(e){workUpMs({dateBaptized:e.target.value})}} style={{padding:"5px 8px",borderRadius:6,border:"1px solid var(--inp-border)",background:"var(--card)",color:"var(--text)",fontSize:10}}/></div>}
        {workPerson.currentStage==="next-steps"&&<div style={{background:"var(--inp)",borderRadius:8,padding:"11px 13px",marginBottom:11}}>
        ...
        {(function(){var wHasBG=workPerson.stages&&workPerson.stages.bgroup&&workPerson.stages.bgroup.completed;var wHasAT=workPerson.stages&&workPerson.stages.ateam&&workPerson.stages.ateam.completed;var wToggleBG=function(){var ns={...workPerson.stages};if(wHasBG){delete ns.bgroup}else{ns.bgroup={date:new Date().toISOString(),completed:true}}p.onUpdate({...workPerson,stages:ns})};var wToggleAT=function(){var ns={...workPerson.stages};if(wHasAT){delete ns.ateam}else{ns.ateam={date:new Date().toISOString(),completed:true}}p.onUpdate({...workPerson,stages:ns})};return <div style={{marginBottom:11}}>
          <div style={{display:"flex",gap:7}}>
            <button onClick={wToggleBG} style={{flex:1,padding:"8px 11px",borderRadius:8,border:wHasBG?"2px solid #EC4899":"2px dashed var(--inp-border)",background:wHasBG?"#EC489908":"var(--inp)",cursor:"pointer",display:"flex",alignItems:"center",gap:5}}><div style={{width:18,height:18,borderRadius:5,background:wHasBG?"#EC4899":"var(--divider)",display:"flex",alignItems:"center",justifyContent:"center"}}>{wHasBG&&<I n="check" sz={10} c="#fff"/>}</div><span style={{fontSize:10,fontWeight:600,color:wHasBG?"#EC4899":"var(--text-muted)"}}>BGroup</span></button>
            <button onClick={wToggleAT} style={{flex:1,padding:"8px 11px",borderRadius:8,border:wHasAT?"2px solid #34D399":"2px dashed var(--inp-border)",background:wHasAT?"#34D39908":"var(--inp)",cursor:"pointer",display:"flex",alignItems:"center",gap:5}}><div style={{width:18,height:18,borderRadius:5,background:wHasAT?"#34D399":"var(--divider)",display:"flex",alignItems:"center",justifyContent:"center"}}>{wHasAT&&<I n="check" sz={10} c="#fff"/>}</div><span style={{fontSize:10,fontWeight:600,color:wHasAT?"#34D399":"var(--text-muted)"}}>ATeam</span></button>
          </div>
          {wHasBG&&wHasAT&&!workPerson.fullyConnected&&<button onClick={function(e){confettiBurst(e);p.onUpdate({...workPerson,fullyConnected:true,fullyConnectedDate:new Date().toISOString()});setWorkFlash("Fully Connected!");setTimeout(function(){setWorkFlash("");if(workIdx<assigned.length-1)setWorkIdx(workIdx+1)},1500)}} style={{width:"100%",marginTop:6,padding:"10px 13px",borderRadius:8,border:"2px solid #F97316",background:"linear-gradient(135deg,#FFF7ED,#FFEDD5)",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:7,fontSize:11,fontWeight:700,color:"#92400E"}} onMouseEnter={function(e){e.currentTarget.style.background="linear-gradient(135deg,#F97316,#FB923C)";e.currentTarget.style.color="#fff"}} onMouseLeave={function(e){e.currentTarget.style.background="linear-gradient(135deg,#FFF7ED,#FFEDD5)";e.currentTarget.style.color="#92400E"}}><I n="target" sz={11} c="#F97316"/> Mark Fully Connected</button>}
          {workPerson.fullyConnected&&<div style={{marginTop:6,background:"#F9731610",border:"1px solid #F9731630",borderRadius:8,padding:"6px 10px",display:"flex",alignItems:"center",gap:5,fontSize:10,fontWeight:600,color:"#F97316"}}><I n="target" sz={11} c="#F97316"/> Fully Connected</div>}
        </div>})()}
        ...
        <div style={{display:"flex",gap:7,alignItems:"center"}}>
          ...
          {workLinIdx>0&&<button onClick={workStepBack} style={{display:"inline-flex",alignItems:"center",gap:3,padding:"8px 11px",borderRadius:8,fontSize:11,fontWeight:600,cursor:"pointer",border:"1px solid var(--inp-border)",background:"var(--card)",color:"var(--text-sub)"}}><span style={{transform:"rotate(180deg)",display:"inline-block"}}><I n="up" sz={13}/></span>Back</button>}
          {workLinIdx<3&&<Btn icon="up" label="Advance" onClick={workAdvance} sx={{padding:"8px 13px",fontSize:11}}/>}
          <div style={{flex:1}}/>
          <button onClick={workPrev} disabled={workIdx===0} style={{width:40,height:40,borderRadius:8,border:"1px solid var(--inp-border)",background:"var(--card)",cursor:workIdx===0?"default":"pointer",display:"flex",alignItems:"center",justifyContent:"center",opacity:workIdx===0?0.3:1}}><span style={{transform:"rotate(180deg)",display:"inline-flex"}}><I n="up" sz={16} c="var(--text-sub)"/></span></button>
          <button onClick={workNext} disabled={workIdx>=assigned.length-1} style={{width:40,height:40,borderRadius:8,border:"1px solid var(--inp-border)",background:"var(--card)",cursor:workIdx>=assigned.length-1?"default":"pointer",display:"flex",alignItems:"center",justifyContent:"center",opacity:workIdx>=assigned.length-1?0.3:1}}><I n="up" sz={16} c="var(--text-sub)"/></button>
        </div>
      </div>
    </div>:
    workMode&&!workPerson?<div style={{background:"var(--card)",borderRadius:8,padding:38,textAlign:"center",color:"var(--text-muted)"}}><I n="check" sz={32} c="#10B981"/><div style={{fontWeight:600,marginTop:10,color:"#10B981"}}>All caught up!</div><div style={{fontSize:10,marginTop:3}}>No pending contacts for {tm?tm.name:"this team"}</div></div>
        :null}
      </div>})():
    acView==="list"?<div style={{background:"var(--card)",borderRadius:8,overflow:"hidden",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
      {assigned.map(function(x,i){var stg=STAGES.find(function(s){return s.key===x.currentStage});var sc=calcScore(x);var d=ago(x.lastContactDate);return <div key={x.id} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 16px",borderTop:i>0?"1px solid var(--divider)":"none",cursor:"pointer"}} onClick={function(){p.onPerson(x)}}><div style={{width:32,height:32,borderRadius:6,background:stg?stg.color+"15":"var(--inp)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:700,color:stg?stg.color:"var(--text-muted)"}}>{(x.firstName||"?").charAt(0)}</div><div style={{flex:1,minWidth:0}}><div style={{fontSize:11,fontWeight:600,color:"var(--text)"}}>{x.firstName} {x.lastName}</div><div style={{fontSize:9,color:"var(--text-muted)",display:"flex",gap:7}}><span style={{color:stg?stg.color:"#999"}}>{stg?stg.label:"?"}</span><span>{d===null?"Never":d+"d ago"}</span></div></div><ScoreRing score={sc} sz={30}/></div>})}
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(300px,1fr))",gap:15}}>
      {assigned.map(function(x){
        ...
        return <div key={x.id} style={{background:"var(--card)",borderRadius:8,padding:"19px",cursor:"pointer",border:"1px solid var(--divider)",boxShadow:"var(--card-shadow)",transition:"all 0.2s"}} onClick={function(){p.onPerson(x)}} onMouseEnter={function(e){e.currentTarget.style.transform="translateY(-3px)";e.currentTarget.style.boxShadow="0 8px 24px rgba(0,0,0,0.08)"}} onMouseLeave={function(e){e.currentTarget.style.transform="translateY(0)";e.currentTarget.style.boxShadow="var(--card-shadow)"}}>
          <div style={{display:"flex",alignItems:"center",gap:11,marginBottom:13}}>
            <div style={{width:44,height:44,borderRadius:8,background:stg?stg.color+"12":"var(--inp)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:700,color:stg?stg.color:"var(--text-muted)",flexShrink:0}}>{(x.firstName||"?").charAt(0)}{(x.lastName||"").charAt(0)}</div>
      ...
      {assigned.length===0&&<div style={{background:"var(--card)",borderRadius:8,padding:32,textAlign:"center",color:"var(--text-muted)",gridColumn:"1/-1"}}>{tm?"No pending contacts for "+tm.name:"Select a team member"}</div>}
    </div>}
  </div>;
}
...
function QuickEntry(p){
  ...
  return <div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:19}}>
      ...
      {count>0&&<div style={{background:flash?"linear-gradient(135deg,#34D399,#10B981)":"var(--inp)",color:flash?"#fff":"var(--text-sub)",padding:"6px 16px",borderRadius:8,fontSize:12,fontWeight:700,transition:"all 0.3s"}}>{count} added</div>}
    </div>
    <div className="weavr-qe-layout" style={{display:"flex",gap:16,alignItems:"flex-start"}}>
      <div style={{flex:1,background:"var(--card)",borderRadius:8,padding:"21px 22px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
      ...
      {recent.length>0&&<div className="weavr-qe-recent" style={{width:320,flexShrink:0}}>
        ...
        <div style={{background:"var(--card)",borderRadius:8,padding:"6px",maxHeight:425,overflowY:"auto",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
...
function BulkMessage(p){
  ...
  return <div>
    ...
    <div style={{display:"flex",gap:7,marginBottom:16,flexWrap:"wrap"}}>{STAGES.map(function(s){var ct=p.people.filter(function(x){return x.currentStage===s.key&&x.phone}).length;return <button key={s.key} onClick={function(){setStage(s.key);setCopied("")}} style={{display:"flex",alignItems:"center",gap:5,padding:"6px 11px",borderRadius:8,border:stage===s.key?"2px solid "+s.color:"2px solid transparent",background:stage===s.key?"var(--card)":"var(--inp)",fontSize:10,fontWeight:600,color:stage===s.key?s.color:"var(--text-sub)",cursor:"pointer"}}><Dot color={s.color} sz={6}/>{s.label} ({ct})</button>})}</div>
    <div style={{background:"var(--card)",borderRadius:8,padding:"21px 22px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:13}}>
      ...
      <div style={{maxHeight:425,overflowY:"auto"}}>{msgs.map(function(m,i){return <div key={m.person.id} style={{padding:"11px 0",borderTop:i>0?"1px solid var(--divider)":"none"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:5}}>
          ...
          <div style={{display:"flex",gap:5}}>
            ...
            <a href={smsUrl(m.person.phone,m.text)} target="_top" style={{textDecoration:"none",padding:"3px 8px",borderRadius:6,background:"linear-gradient(135deg,#67E8F9,#06B6D4)",color:"#fff",fontSize:9,fontWeight:600,display:"inline-flex",alignItems:"center",gap:3}}><I n="send" sz={11} c="#fff"/>SMS</a>
          </div>
        </div>
...
function Reports(p){
  ...
  var buckets=[{label:"Highly Engaged",min:75,max:100,color:"#10B981"},{label:"Engaged",min:50,max:74,color:"#3898FF"},{label:"At Risk",min:30,max:49,color:"#F59E0B"},{label:"Disengaging",min:0,max:29,color:"#EF4444"}];
  ...
  return <div>
    ...
    {widgets.includes("kpis")&&<div className="weavr-kpi-grid" style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(110px,1fr))",gap:10,marginBottom:16}}>{[{l:"Total People",v:total,i:"users",c:"var(--primary)",bg:"var(--primary)"},{l:"Added (7d)",v:added7,i:"plus",c:"#10B981",bg:"#10B981"},{l:"Need Follow-Up",v:needFollow,i:"flag",c:"#EF4444",bg:"#EF4444"},{l:"Avg Score",v:avgScore,i:"target",c:"#06B6D4",bg:"#06B6D4"}].map(function(k){return <div key={k.l} style={{background:k.bg+"0A",borderRadius:8,padding:"10px",overflow:"hidden",border:"1px solid "+k.bg+"15",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}><div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:5,marginBottom:4}}><I n={k.i} sz={13} c={k.c}/><div style={{fontSize:8,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.06em",color:k.c,lineHeight:1.3}}>{k.l}</div></div><div style={{fontSize:20,fontWeight:800,color:"var(--text)",lineHeight:1,textAlign:"center"}}>{k.v}</div></div>})}</div>}
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
      ...
      {widgets.includes("dropoff")&&(function(){var linearS=STAGES.filter(function(s){return s.key!=="bgroup"&&s.key!=="ateam"});var drops=linearS.map(function(s,i){var inStage=people.filter(function(x){return x.currentStage===s.key});var stalled=inStage.filter(function(x){var stDate=x.stages&&x.stages[s.key]?x.stages[s.key].date:x.createdAt;return ago(stDate)>14});var pct=inStage.length>0?Math.round(stalled.length/inStage.length*100):0;return{...s,total:inStage.length,stalled:stalled.length,pct:pct}});var maxDrop=Math.max.apply(null,drops.map(function(d){return d.total}).concat([1]));return <div style={{background:"var(--card)",borderRadius:8,padding:"21px 22px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}><h3 style={{fontSize:13,fontWeight:700,color:"var(--text)",marginBottom:3}}>Drop-off Analysis</h3><p style={{fontSize:9,color:"var(--text-muted)",marginBottom:11}}>Where people stall (14+ days in stage)</p>{drops.map(function(d){return <div key={d.key} style={{marginBottom:8}}><div style={{display:"flex",alignItems:"center",gap:8,marginBottom:3}}><Dot color={d.color} sz={7}/><span style={{fontSize:10,fontWeight:500,color:"var(--text)",flex:1}}>{d.label}</span><span style={{fontSize:9,color:d.pct>50?"#EF4444":d.pct>25?"#F59E0B":"var(--text-muted)",fontWeight:600}}>{d.stalled}/{d.total} stalled ({d.pct}%)</span></div><div style={{height:6,background:"var(--divider)",borderRadius:8,overflow:"hidden"}}><div style={{height:"100%",borderRadius:8,background:d.pct>50?"#EF4444":d.pct>25?"#F59E0B":"#10B981",width:Math.max(d.total/maxDrop*100,3)+"%",position:"relative"}}><div style={{position:"absolute",right:0,top:0,height:"100%",width:d.pct+"%",background:"rgba(239,68,68,0.4)",borderRadius:"0 3px 3px 0"}}/></div></div></div>})}</div>})()}
  ...
  return <div>
    <div className="weavr-fc-header" style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:11}}>
      <div style={{textAlign:"left"}}>
        <h2 style={{fontSize:17,fontWeight:800,color:"var(--text)",display:"flex",alignItems:"center",gap:8}}><I n="target" sz={18} c="#F97316"/> Fully Connected</h2>
      ...
      <div style={{display:"flex",gap:3,background:"var(--inp)",borderRadius:8,padding:2}}>
        {["grid","list"].map(function(v){return <button key={v} onClick={function(){setFcView(v)}} style={{padding:"5px 10px",borderRadius:6,border:"none",fontSize:9,fontWeight:600,cursor:"pointer",background:fcView===v?"var(--card)":"transparent",color:fcView===v?"var(--text)":"var(--text-muted)",boxShadow:fcView===v?"0 1px 4px rgba(0,0,0,0.08)":"none"}}>{v==="grid"?"Grid":"List"}</button>})}
      </div>
    </div>
    <div style={{marginBottom:13}}><SortBar value={fcSort} onChange={setFcSort}/></div>
    {connected.length===0?<div style={{background:"var(--card)",borderRadius:8,padding:38,textAlign:"center",color:"var(--text-muted)",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}><div style={{fontSize:12,fontWeight:600}}>No one fully connected yet</div><p style={{fontSize:10,marginTop:5}}>When someone joins both a BGroup and the ATeam, they'll appear here.</p></div>:
    fcView==="list"?<div style={{background:"var(--card)",borderRadius:8,overflow:"hidden",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(320px,1fr))",gap:15}}>
      {connected.map(function(x){
        ...
        return <div key={x.id} style={{background:"var(--card)",borderRadius:8,padding:"19px",cursor:"pointer",boxShadow:"var(--card-shadow)",border:"1px solid #F9731620",transition:"all 0.2s"}} onClick={function(){p.onPerson(x)}} onMouseEnter={function(e){e.currentTarget.style.transform="translateY(-2px)";e.currentTarget.style.borderColor="#F9731640"}} onMouseLeave={function(e){e.currentTarget.style.transform="translateY(0)";e.currentTarget.style.borderColor="#F9731620"}}>
          <div style={{display:"flex",alignItems:"center",gap:11,marginBottom:13}}>
            <div style={{width:44,height:44,borderRadius:8,background:"#F9731610",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:700,color:"#F97316"}}>{(x.firstName||"?").charAt(0)}{(x.lastName||"").charAt(0)}</div>
          ...
          <div style={{display:"flex",gap:8}}>
            <div style={{flex:1,background:"#EC489908",borderRadius:8,padding:"8px 11px",border:"1px solid #EC489912"}}><div style={{fontSize:8,fontWeight:700,color:"#EC4899",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:2}}>BGroup</div><div style={{fontSize:11,fontWeight:600,color:"var(--text)"}}>{ns.bGroupLeader||"Connected"}</div></div>
            <div style={{flex:1,background:"#34D39908",borderRadius:8,padding:"8px 11px",border:"1px solid #34D39912"}}><div style={{fontSize:8,fontWeight:700,color:"#34D399",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:2}}>ATeam</div><div style={{fontSize:11,fontWeight:600,color:"var(--text)"}}>{ns.aTeamArea||"Serving"}</div></div>
          </div>
        </div>;
      })}
    </div>}
...
function Settings(p){
  ...
  var tabGroups=[{label:"Communication",tabs:[{key:"templates",label:"Text Templates",icon:"msg"},{key:"email-tpl",label:"Email Templates",icon:"mail"},{key:"follow-ups",label:"Follow-ups",icon:"clock"},{key:"sequences",label:"Sequences",icon:"clock"}]},{label:"Pipeline",tabs:[{key:"goals",label:"Goals",icon:"target"},{key:"checkins",label:"Check-ins",icon:"check"},{key:"forms",label:"Form Fields",icon:"card"},{key:"automation",label:"Automation",icon:"zap"}]},{label:"System",tabs:[{key:"teams",label:"Teams",icon:"users"},{key:"visual",label:"Visual",icon:"palette"},{key:"data",label:"Data",icon:"dl"}]}];
  var tabs=tabGroups.reduce(function(a,g){return a.concat(g.tabs)},[]);

  return <div>
    <h2 style={{fontSize:17,fontWeight:800,color:"var(--text)",marginBottom:16}}>Settings</h2>
    <div style={{marginBottom:16}}>{tabGroups.map(function(g,gi){return <div key={gi} style={{marginBottom:8}}><div style={{fontSize:8,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.08em",color:"var(--text-muted)",marginBottom:4,paddingLeft:2}}>{g.label}</div><div style={{display:"flex",gap:4,flexWrap:"wrap"}}>{g.tabs.map(function(t){return <button key={t.key} onClick={function(){setTab(t.key)}} style={{display:"flex",alignItems:"center",gap:4,padding:"6px 11px",borderRadius:8,border:tab===t.key?"2px solid var(--primary)":"2px solid transparent",background:tab===t.key?"var(--primary)08":"var(--inp)",fontSize:10,fontWeight:600,color:tab===t.key?"var(--primary)":"var(--text-sub)",cursor:"pointer"}}><I n={t.icon} sz={13} c={tab===t.key?"var(--primary)":"var(--text-muted)"}/>{t.label}</button>})}</div></div>})}</div>

    {tab==="teams"&&<div style={{background:"var(--card)",borderRadius:8,padding:"21px 22px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
      ...
      {p.teams.map(function(t){var ct=p.people.filter(function(x){return x.assignedTo===t.id}).length;return <div key={t.id} style={{display:"flex",alignItems:"center",gap:10,padding:"11px 13px",background:"var(--inp)",borderRadius:8,marginBottom:5}}><div style={{width:36,height:36,borderRadius:8,background:t.color+"20",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:700,color:t.color}}>{t.name.charAt(0)}</div><div style={{flex:1}}><div style={{fontSize:11,fontWeight:600,color:"var(--text)"}}>{t.name}</div><div style={{fontSize:9,color:"var(--text-muted)"}}>{t.role||"Team Member"} - {ct} assigned</div></div><button onClick={function(){p.setTeams(function(prev){return prev.filter(function(x){return x.id!==t.id})})}} style={{background:"none",border:"none",padding:3,cursor:"pointer"}}><I n="trash" sz={14} c="#EF4444"/></button></div>})}
      {p.teams.length===0&&<div style={{color:"var(--text-muted)",fontSize:11,padding:"16px 0",textAlign:"center"}}>No team members yet</div>}
    </div>}

    {tab==="templates"&&<div style={{background:"var(--card)",borderRadius:8,padding:"21px 22px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
      <h3 style={{fontSize:13,fontWeight:700,color:"var(--text)",marginBottom:3}}>Text Templates</h3>
      <p style={{fontSize:10,color:"var(--text-muted)",marginBottom:13}}>{"Multiple templates per stage. Use {firstName} and {lastName} as placeholders."}</p>
      <div style={{display:"flex",gap:16}}><div style={{width:180,flexShrink:0}}>{STAGES.map(function(s){return <button key={s.key} onClick={function(){setEk(s.key);setSaved(false)}} style={{display:"flex",alignItems:"center",gap:7,width:"100%",padding:"8px 11px",borderRadius:8,border:"none",fontSize:11,fontWeight:ek===s.key?600:500,background:ek===s.key?"var(--primary)08":"transparent",color:ek===s.key?"var(--primary)":"var(--text-sub)",textAlign:"left",marginBottom:2,cursor:"pointer"}}><Dot color={s.color} sz={7}/>{s.label}</button>})}</div>
      <div style={{flex:1}}>{ek?(function(){var stg=STAGES.find(function(s){return s.key===ek});var multiTpls=(config.multiTextTpl||DEFAULT_MULTI_TPL)[ek]||DEFAULT_MULTI_TPL[ek]||[];
        var updateMultiText=function(newArr){var mt={...(config.multiTextTpl||DEFAULT_MULTI_TPL)};mt[ek]=newArr;p.setConfig({...config,multiTextTpl:mt});setSaved(true);setTimeout(function(){setSaved(false)},2000)};
        var addTpl=function(){updateMultiText(multiTpls.concat([{name:"New Template",body:"Hey {firstName}, "}]))};
        var removeTpl=function(idx){var a=multiTpls.slice();a.splice(idx,1);updateMultiText(a)};
        var updateTpl=function(idx,field,val){var a=multiTpls.slice();a[idx]={...a[idx]};a[idx][field]=val;updateMultiText(a)};
        return <div>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
            <div style={{fontSize:12,fontWeight:600,color:"var(--text)",display:"flex",alignItems:"center",gap:7}}><Dot color={stg?stg.color:"#ccc"} sz={9}/>{stg?stg.label:""} Text Templates ({multiTpls.length})</div>
            <button onClick={addTpl} style={{padding:"4px 12px",borderRadius:6,background:"var(--primary)10",border:"1px solid var(--primary)25",fontSize:9,fontWeight:600,color:"var(--primary)",cursor:"pointer"}}>+ Add Template</button>
          </div>
          {multiTpls.map(function(t,i){return <div key={i} style={{background:"var(--inp)",borderRadius:8,padding:"12px",marginBottom:8,border:"1px solid var(--inp-border)"}}>
            <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:6}}>
              <div style={{fontSize:9,fontWeight:700,color:"var(--text-muted)",textTransform:"uppercase",letterSpacing:"0.06em",flexShrink:0}}>Name</div>
              <input value={t.name} onChange={function(e){updateTpl(i,"name",e.target.value)}} style={{flex:1,padding:"5px 8px",borderRadius:6,border:"1px solid var(--inp-border)",background:"var(--card)",color:"var(--text)",fontSize:11,fontWeight:600,boxSizing:"border-box",outline:"none"}}/>
              {multiTpls.length>1&&<button onClick={function(){removeTpl(i)}} style={{width:22,height:22,borderRadius:6,background:"#EF444410",border:"1px solid #EF444420",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",flexShrink:0}}><I n="x" sz={9} c="#EF4444"/></button>}
            </div>
            <textarea value={t.body} onChange={function(e){updateTpl(i,"body",e.target.value)}} style={{width:"100%",minHeight:70,padding:10,borderRadius:8,border:"1px solid var(--inp-border)",background:"var(--card)",color:"var(--text)",fontSize:11,lineHeight:1.7,resize:"vertical",boxSizing:"border-box",outline:"none"}}/>
          </div>})}
          {saved&&<div style={{fontSize:10,color:"#10B981",fontWeight:600,marginTop:4}}>Saved!</div>}
        </div>})():<div style={{color:"var(--text-muted)",fontSize:12,padding:"32px 0",textAlign:"center"}}>Select a stage to manage its text templates</div>}</div></div>
    </div>}

    {tab==="email-tpl"&&<div style={{background:"var(--card)",borderRadius:8,padding:"21px 22px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
      <h3 style={{fontSize:13,fontWeight:700,color:"var(--text)",marginBottom:3}}>Email Templates</h3>
      <p style={{fontSize:10,color:"var(--text-muted)",marginBottom:13}}>{"Multiple email templates per stage. Use {firstName} and {lastName} as placeholders."}</p>
      <div style={{display:"flex",gap:16}}>
        <div style={{width:180,flexShrink:0}}>{STAGES.map(function(s){return <button key={s.key} onClick={function(){setEmailEk(s.key);setEmailSaved(false)}} style={{display:"flex",alignItems:"center",gap:7,width:"100%",padding:"8px 11px",borderRadius:8,border:"none",fontSize:11,fontWeight:emailEk===s.key?600:500,background:emailEk===s.key?"var(--primary)08":"transparent",color:emailEk===s.key?"var(--primary)":"var(--text-sub)",textAlign:"left",marginBottom:2,cursor:"pointer"}}><Dot color={s.color} sz={7}/>{s.label}</button>})}</div>
        <div style={{flex:1}}>{emailEk?(function(){var stg=STAGES.find(function(s){return s.key===emailEk});var multiEmail=(config.multiEmailTpl||DEFAULT_EMAIL_TPL)[emailEk]||DEFAULT_EMAIL_TPL[emailEk]||[];
          var updateMultiEmail=function(newArr){var me={...(config.multiEmailTpl||DEFAULT_EMAIL_TPL)};me[emailEk]=newArr;p.setConfig({...config,multiEmailTpl:me});setEmailSaved(true);setTimeout(function(){setEmailSaved(false)},2000)};
          var addETpl=function(){updateMultiEmail(multiEmail.concat([{name:"New Email Template",subj:"Following up, {firstName}",body:"Hey {firstName},\n\n"}]))};
          var removeETpl=function(idx){var a=multiEmail.slice();a.splice(idx,1);updateMultiEmail(a)};
          var updateETpl=function(idx,field,val){var a=multiEmail.slice();a[idx]={...a[idx]};a[idx][field]=val;updateMultiEmail(a)};
          return <div>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
              <div style={{fontSize:12,fontWeight:600,color:"var(--text)",display:"flex",alignItems:"center",gap:7}}><Dot color={stg?stg.color:"#ccc"} sz={9}/>{stg?stg.label:""} Email Templates ({multiEmail.length})</div>
              <button onClick={addETpl} style={{padding:"4px 12px",borderRadius:6,background:"var(--primary)10",border:"1px solid var(--primary)25",fontSize:9,fontWeight:600,color:"var(--primary)",cursor:"pointer"}}>+ Add Template</button>
            </div>
            {multiEmail.map(function(t,i){return <div key={i} style={{background:"var(--inp)",borderRadius:8,padding:"12px",marginBottom:8,border:"1px solid var(--inp-border)"}}>
              <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:6}}>
                <div style={{fontSize:9,fontWeight:700,color:"var(--text-muted)",textTransform:"uppercase",letterSpacing:"0.06em",flexShrink:0}}>Name</div>
                <input value={t.name} onChange={function(e){updateETpl(i,"name",e.target.value)}} style={{flex:1,padding:"5px 8px",borderRadius:6,border:"1px solid var(--inp-border)",background:"var(--card)",color:"var(--text)",fontSize:11,fontWeight:600,boxSizing:"border-box",outline:"none"}}/>
                {multiEmail.length>1&&<button onClick={function(){removeETpl(i)}} style={{width:22,height:22,borderRadius:6,background:"#EF444410",border:"1px solid #EF444420",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",flexShrink:0}}><I n="x" sz={9} c="#EF4444"/></button>}
              </div>
              <div style={{marginBottom:6}}><div style={{fontSize:9,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.06em",color:"var(--text-muted)",marginBottom:3}}>Subject</div><input value={t.subj||""} onChange={function(e){updateETpl(i,"subj",e.target.value)}} style={{width:"100%",padding:"6px 8px",borderRadius:6,border:"1px solid var(--inp-border)",background:"var(--card)",color:"var(--text)",fontSize:11,boxSizing:"border-box",outline:"none"}}/></div>
              <div style={{fontSize:9,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.06em",color:"var(--text-muted)",marginBottom:3}}>Body</div>
              <textarea value={t.body||""} onChange={function(e){updateETpl(i,"body",e.target.value)}} style={{width:"100%",minHeight:80,padding:10,borderRadius:8,border:"1px solid var(--inp-border)",background:"var(--card)",color:"var(--text)",fontSize:11,lineHeight:1.7,resize:"vertical",boxSizing:"border-box",outline:"none"}}/>
            </div>})}
    ...
    {tab==="follow-ups"&&<div style={{background:"var(--card)",borderRadius:8,padding:"21px 22px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
      <h3 style={{fontSize:13,fontWeight:700,color:"var(--text)",marginBottom:3}}>Auto Follow-up Settings</h3>
      <p style={{fontSize:10,color:"var(--text-muted)",marginBottom:16}}>When you log a text or email, an automatic follow-up is scheduled. Customize the delay, label, and template per stage.</p>
      {STAGES.map(function(s){
        var fuConfig=(config.followUpConfig||{})[s.key]||{};
        var delay=fuConfig.delay||FOLLOW_UP_DELAYS[s.key]||5;
        var label=fuConfig.label||FOLLOW_UP_LABELS[s.key]||"Follow-up";
        var tpls=fuConfig.templates||[{name:"Default",body:fuConfig.template||DEFAULT_FOLLOWUP_TPL[s.key]||""}];
        var updateFU=function(field,val){var fc={...(config.followUpConfig||{})};fc[s.key]={...(fc[s.key]||{}),delay:delay,label:label,templates:tpls};fc[s.key][field]=val;p.setConfig({...config,followUpConfig:fc})};
        var updateTpls=function(newTpls){var fc={...(config.followUpConfig||{})};fc[s.key]={...(fc[s.key]||{}),delay:delay,label:label,templates:newTpls};p.setConfig({...config,followUpConfig:fc})};
        return <div key={s.key} style={{background:"var(--inp)",borderRadius:8,padding:"12px",marginBottom:8,border:"1px solid var(--inp-border)"}}>
          <div style={{display:"flex",alignItems:"center",gap:7,marginBottom:8}}>
            <Dot color={s.color} sz={8}/>
            <span style={{fontSize:11,fontWeight:700,color:"var(--text)",flex:1}}>{s.label}</span>
          </div>
          <div style={{display:"flex",gap:8,marginBottom:8}}>
            <div style={{width:70}}>
              <div style={{fontSize:8,fontWeight:700,color:"var(--text-muted)",textTransform:"uppercase",marginBottom:2}}>Delay (days)</div>
              <input type="number" min="1" value={delay} onChange={function(e){updateFU("delay",parseInt(e.target.value)||1)}} style={{width:"100%",padding:"5px 6px",borderRadius:6,border:"1px solid var(--inp-border)",background:"var(--card)",color:"var(--text)",fontSize:11,fontWeight:700,textAlign:"center",boxSizing:"border-box",outline:"none"}}/>
            </div>
            <div style={{flex:1}}>
              <div style={{fontSize:8,fontWeight:700,color:"var(--text-muted)",textTransform:"uppercase",marginBottom:2}}>Label</div>
              <input value={label} onChange={function(e){updateFU("label",e.target.value)}} style={{width:"100%",padding:"5px 8px",borderRadius:6,border:"1px solid var(--inp-border)",background:"var(--card)",color:"var(--text)",fontSize:10,boxSizing:"border-box",outline:"none"}}/>
            </div>
          </div>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:4}}>
            <div style={{fontSize:8,fontWeight:700,color:"var(--text-muted)",textTransform:"uppercase"}}>Follow-up Templates ({tpls.length})</div>
            <button onClick={function(){updateTpls(tpls.concat([{name:"New Template",body:"Hey {firstName}, "}]))}} style={{padding:"2px 8px",borderRadius:5,background:"var(--card)",border:"1px solid var(--inp-border)",fontSize:8,fontWeight:600,color:"var(--text-muted)",cursor:"pointer"}}>+ Add</button>
          </div>
          {tpls.map(function(t,ti){return <div key={ti} style={{background:"var(--card)",borderRadius:8,padding:"8px",marginBottom:4,border:"1px solid var(--inp-border)"}}>
            <div style={{display:"flex",gap:4,marginBottom:4,alignItems:"center"}}>
              <input value={t.name} onChange={function(e){var nt=tpls.slice();nt[ti]={...nt[ti],name:e.target.value};updateTpls(nt)}} style={{flex:1,padding:"3px 6px",borderRadius:5,border:"1px solid var(--inp-border)",background:"var(--inp)",color:"var(--text)",fontSize:9,fontWeight:600,boxSizing:"border-box",outline:"none"}} placeholder="Template name"/>
              {tpls.length>1&&<button onClick={function(){var nt=tpls.slice();nt.splice(ti,1);updateTpls(nt)}} style={{width:16,height:16,borderRadius:5,background:"#EF444410",border:"1px solid #EF444420",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",flexShrink:0}}><I n="x" sz={7} c="#EF4444"/></button>}
            </div>
            <textarea value={t.body} onChange={function(e){var nt=tpls.slice();nt[ti]={...nt[ti],body:e.target.value};updateTpls(nt)}} style={{width:"100%",minHeight:40,padding:6,borderRadius:5,border:"1px solid var(--inp-border)",background:"var(--inp)",color:"var(--text)",fontSize:9,lineHeight:1.6,resize:"vertical",boxSizing:"border-box",outline:"none"}} placeholder="Hey {firstName}, just following up..."/>
          </div>})}
        </div>
      })}
    </div>}

    {tab==="goals"&&<div style={{background:"var(--card)",borderRadius:8,padding:"21px 22px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
      ...
      <div style={{marginBottom:20}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:8}}>
          {STAGES.concat([{key:"fully-connected",label:"Fully Connected",color:"#F97316"}]).map(function(s){var fGoals={...DEFAULT_FUNNEL_GOALS,...(config.funnelGoals||{})};var val=fGoals[s.key]||50;return <div key={s.key} style={{display:"flex",alignItems:"center",gap:8,background:"var(--inp)",borderRadius:8,padding:"8px 12px",border:"1px solid var(--inp-border)"}}>
      ...
      <div>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
          ...
          <button onClick={function(){var cg=(config.customGoals||[]).concat([{label:"New Goal",metric:"total",target:100,color:"var(--primary)"}]);p.setConfig({...config,customGoals:cg})}} style={{padding:"4px 12px",borderRadius:6,background:"var(--primary)10",border:"1px solid var(--primary)25",fontSize:9,fontWeight:600,color:"var(--primary)",cursor:"pointer"}}>+ Add Goal</button>
        </div>
        <p style={{fontSize:9,color:"var(--text-muted)",marginBottom:10}}>Track custom metrics. These appear in the Goals Tracker widget.</p>
        {(config.customGoals||[]).map(function(g,i){return <div key={i} style={{background:"var(--inp)",borderRadius:8,padding:"10px 12px",marginBottom:6,border:"1px solid var(--inp-border)"}}>
    ...
    {tab==="sequences"&&<div style={{background:"var(--card)",borderRadius:8,padding:"21px 22px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
      ...
      {(function(){var seqs=config.sequences||{};var selSeqStage=ek||"first-visit";var stg=STAGES.find(function(s){return s.key===selSeqStage});var stageSeq=seqs[selSeqStage]||[{day:1,type:"text",message:"Hey {firstName}, it was great having you! How was your experience?"},{day:3,type:"text",message:"Hey {firstName}, just checking in! Would love to connect this week."},{day:7,type:"email",message:"Hi {firstName}, we'd love to see you again. Is there anything we can help with?"}];var updateSeq=function(newSeq){var ns={...seqs};ns[selSeqStage]=newSeq;p.setConfig({...config,sequences:ns})};return <div style={{display:"flex",gap:16}}>
        <div style={{width:180,flexShrink:0}}>{STAGES.filter(function(s){return s.key!=="bgroup"&&s.key!=="ateam"}).map(function(s){return <button key={s.key} onClick={function(){setEk(s.key)}} style={{display:"flex",alignItems:"center",gap:7,width:"100%",padding:"8px 11px",borderRadius:8,border:"none",fontSize:11,fontWeight:selSeqStage===s.key?600:500,background:selSeqStage===s.key?"var(--primary)08":"transparent",color:selSeqStage===s.key?"var(--primary)":"var(--text-sub)",textAlign:"left",marginBottom:2,cursor:"pointer"}}><Dot color={s.color} sz={7}/>{s.label}</button>})}</div>
        <div style={{flex:1}}>
          ...
          {stageSeq.map(function(step,i){return <div key={i} style={{display:"flex",gap:8,marginBottom:10,alignItems:"flex-start"}}>
            ...
            <div style={{flex:1,background:"var(--inp)",borderRadius:8,padding:"8px 11px"}}>
              ...
              <textarea value={step.message} onChange={function(e){var ns=stageSeq.slice();ns[i]={...ns[i],message:e.target.value};updateSeq(ns)}} style={{width:"100%",minHeight:48,padding:"6px 8px",borderRadius:6,border:"1px solid var(--inp-border)",background:"var(--card)",color:"var(--text)",fontSize:10,lineHeight:1.5,resize:"vertical",boxSizing:"border-box",outline:"none"}}/>
            </div>
          </div>})}
    ...
    {tab==="automation"&&<div style={{background:"var(--card)",borderRadius:8,padding:"21px 22px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
      ...
      <div style={{background:"var(--inp)",borderRadius:8,padding:13,marginBottom:13}}>
      ...
      {p.rules.map(function(r){var desc="";if(r.trigger==="days-no-contact")desc="No contact for "+r.days+"d";else if(r.trigger==="stage-reached"){var stg=STAGES.find(function(s){return s.key===r.stage});desc=(stg?stg.label:r.stage)+" reached"}else if(r.trigger==="score-below")desc="Score below "+r.days;else desc="Check-in logged";var act=r.action==="escalate"?"Escalate":r.action==="advance"?"Auto-advance":r.action==="assign"?"Auto-assign":"Add reminder";return <div key={r.id} style={{display:"flex",alignItems:"center",gap:10,padding:"11px 13px",background:"var(--inp)",borderRadius:8,marginBottom:5}}><I n="zap" sz={16} c={r.enabled?"var(--primary)":"var(--text-muted)"}/><div style={{flex:1,fontSize:11,fontWeight:500,color:r.enabled?"var(--text)":"var(--text-muted)"}}>{desc} \u2192 {act}</div><button onClick={function(){p.setRules(function(prev){return prev.map(function(x){return x.id===r.id?{...x,enabled:!x.enabled}:x})})}} style={{background:"none",border:"none",padding:3,cursor:"pointer",fontSize:9,fontWeight:600,color:r.enabled?"#10B981":"var(--text-muted)"}}>{r.enabled?"Active":"Off"}</button><button onClick={function(){p.setRules(function(prev){return prev.filter(function(x){return x.id!==r.id})})}} style={{background:"none",border:"none",padding:3,cursor:"pointer"}}><I n="trash" sz={14} c="#EF4444"/></button></div>})}
      {p.rules.length===0&&<div style={{color:"var(--text-muted)",fontSize:11,padding:"16px 0",textAlign:"center"}}>No automation rules yet</div>}
    </div>}

    {tab==="checkins"&&<div style={{background:"var(--card)",borderRadius:8,padding:"21px 22px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
    ...
    {tab==="forms"&&<div style={{background:"var(--card)",borderRadius:8,padding:"21px 22px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
      ...
      <div style={{background:"var(--inp)",borderRadius:8,padding:13,marginBottom:13}}>
        ...
        <div style={{display:"flex",gap:7,flexWrap:"wrap",alignItems:"end"}}>
          <div style={{flex:1,minWidth:140}}><div style={{fontSize:9,color:"var(--text-muted)",marginBottom:3}}>Label</div><input style={{width:"100%",padding:"6px 10px",borderRadius:6,border:"1px solid var(--inp-border)",background:"var(--card)",color:"var(--text)",fontSize:10,boxSizing:"border-box",outline:"none"}} value={newField.label} onChange={function(e){setNewField({...newField,label:e.target.value})}} placeholder="e.g. Marital Status"/></div>
          <div style={{width:130}}><div style={{fontSize:9,color:"var(--text-muted)",marginBottom:3}}>Type</div><select style={{width:"100%",padding:"6px 10px",borderRadius:6,border:"1px solid var(--inp-border)",background:"var(--card)",color:"var(--text)",fontSize:10,cursor:"pointer"}} value={newField.type} onChange={function(e){setNewField({...newField,type:e.target.value})}}><option value="text">Text</option><option value="dropdown">Dropdown</option><option value="checkbox">Checkbox</option></select></div>
          {newField.type==="dropdown"&&<div style={{flex:1,minWidth:200}}><div style={{fontSize:9,color:"var(--text-muted)",marginBottom:3}}>Options (comma separated)</div><input style={{width:"100%",padding:"6px 10px",borderRadius:6,border:"1px solid var(--inp-border)",background:"var(--card)",color:"var(--text)",fontSize:10,boxSizing:"border-box",outline:"none"}} value={newField.optionsStr||""} onChange={function(e){setNewField({...newField,optionsStr:e.target.value})}} placeholder="Option 1, Option 2, Option 3"/></div>}
      ...
      {formFields.map(function(fd,idx){return <div key={fd.key} style={{display:"flex",alignItems:"center",gap:8,padding:"8px 11px",background:"var(--inp)",borderRadius:8,marginBottom:3}}>
        ...
        <div style={{flex:1,minWidth:0}}>
          <div style={{display:"flex",alignItems:"center",gap:5}}>
            ...
            <span style={{fontSize:9,padding:"1px 5px",borderRadius:8,background:"var(--primary)10",color:"var(--primary)",fontWeight:600}}>{fd.type}</span>
            {fd.required&&<span style={{fontSize:9,padding:"1px 5px",borderRadius:8,background:"#EF444415",color:"#EF4444",fontWeight:600}}>required</span>}
          </div>
    ...
    {tab==="visual"&&<div style={{background:"var(--card)",borderRadius:8,padding:"21px 22px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
  ...
  return <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,0.5)",zIndex:200,display:"flex",justifyContent:"flex-end"}} onClick={p.onClose}><div className="weavr-panel" style={{width:560,maxWidth:"100vw",height:"100vh",background:"var(--card-solid)",display:"flex",flexDirection:"column",animation:"panelSlide 0.4s cubic-bezier(0.22,1,0.36,1)",overflowY:"auto",boxShadow:"-12px 0 48px rgba(0,0,0,0.12)"}} onClick={function(e){e.stopPropagation()}}>
    <div className="weavr-panel-header" style={{padding:"19px 19px 16px",background:"var(--hero)",position:"relative"}}>
      ...
      <div style={{display:"flex",gap:13,alignItems:"center",paddingLeft:16}}>
        <div style={{width:56,height:56,borderRadius:6,background:"rgba(255,255,255,0.15)",border:"2px solid rgba(255,255,255,0.25)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,fontWeight:700,color:"#fff",flexShrink:0,backdropFilter:"blur(4px)"}}>{(person.firstName||"?").charAt(0)}{(person.lastName||"").charAt(0)}</div>
        <div style={{flex:1}}>
    ...
    <div className="weavr-panel-actions" style={{padding:"10px 19px",borderBottom:"1px solid var(--divider)",background:"var(--card-solid)",position:"sticky",top:0,zIndex:10}}>
      <div style={{display:"flex",alignItems:"center",gap:5,flexWrap:"wrap"}}>
        ...
        <div style={{display:"inline-flex",alignItems:"center",gap:0,borderRadius:8,overflow:"hidden",border:"1px solid var(--inp-border)"}}>
        ...
        <Btn icon="phone" label="Call" onClick={function(){setContactOpen("call")}} v="ghost" sx={{padding:"6px 11px",fontSize:9,opacity:person.phone?1:0.4}}/>
        ...
        <button onClick={function(){if(confirm("Delete "+person.firstName+"?")){p.onDelete(person.id);p.onClose()}}} style={{display:"inline-flex",alignItems:"center",padding:"6px 8px",borderRadius:8,cursor:"pointer",background:"#FEF2F2",color:"#EF4444",border:"1px solid #FECACA"}}><I n="trash" sz={12} c="#EF4444"/></button>
      </div>
    </div>
    <div className="weavr-panel-body" style={{padding:"16px 19px",overflowY:"auto",flex:1}}>
      ...
      {contactOpen==="text"&&<TextModal person={person} config={p.config} onLog={function(type,via){var now=new Date().toISOString();var fuCfg=(p.config.followUpConfig||{})[person.currentStage]||{};var fuDelay=fuCfg.delay||FOLLOW_UP_DELAYS[person.currentStage]||5;var fuDate=new Date();fuDate.setDate(fuDate.getDate()+fuDelay);var fuLabel=fuCfg.label||FOLLOW_UP_LABELS[person.currentStage]||"Follow-up";p.onUpdate({...person,lastContactDate:now,checkIns:[...(person.checkIns||[]),{type:type,note:"Sent via "+via,date:now}],scheduledFollowUps:[...(person.scheduledFollowUps||[]).filter(function(fu){return new Date(fu.date)>new Date()}),{date:fuDate.toISOString(),label:fuLabel,stage:person.currentStage,type:"text"}]})}} onClose={function(){setContactOpen(null)}}/>}
      {contactOpen==="email"&&<EmailModal person={person} config={p.config} onLog={function(type,via){var now=new Date().toISOString();var fuCfg=(p.config.followUpConfig||{})[person.currentStage]||{};var fuDelay=fuCfg.delay||FOLLOW_UP_DELAYS[person.currentStage]||5;var fuDate=new Date();fuDate.setDate(fuDate.getDate()+fuDelay);var fuLabel=fuCfg.label||FOLLOW_UP_LABELS[person.currentStage]||"Follow-up";p.onUpdate({...person,lastContactDate:now,checkIns:[...(person.checkIns||[]),{type:type,note:"Sent via "+via,date:now}],scheduledFollowUps:[...(person.scheduledFollowUps||[]).filter(function(fu){return new Date(fu.date)>new Date()}),{date:fuDate.toISOString(),label:fuLabel,stage:person.currentStage,type:"email"}]})}} onClose={function(){setContactOpen(null)}}/>}
      {contactOpen==="call"&&<CallModal person={person} config={p.config} onLog={function(type,via){var now=new Date().toISOString();p.onUpdate({...person,lastContactDate:now,checkIns:[...(person.checkIns||[]),{type:type,note:"Called via "+via,date:now}]})}} onClose={function(){setContactOpen(null)}}/>}

      <Reveal open={showCal}><div style={{background:"var(--card)",borderRadius:8,padding:"11px 14px",marginBottom:13,boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
        <div style={{fontSize:10,fontWeight:700,color:"var(--text)",marginBottom:8}}>Schedule Follow-Up</div>
        <div style={{display:"flex",gap:8,marginBottom:10}}>
          {[{key:"remind",label:"Remind Me"},{key:"text",label:"Auto-Send Text"},{key:"email",label:"Auto-Send Email"}].map(function(t){return <button key={t.key} onClick={function(){setFuType(t.key)}} style={{padding:"5px 10px",borderRadius:6,border:fuType===t.key?"2px solid var(--primary)":"2px solid var(--inp-border)",background:fuType===t.key?"var(--primary)08":"var(--inp)",fontSize:9,fontWeight:600,color:fuType===t.key?"var(--primary)":"var(--text-sub)",cursor:"pointer"}}>{t.label}</button>})}
        </div>
      ...
      <Reveal open={showFollowUp}><div style={{marginBottom:13}}>
        <div style={{background:"var(--card)",borderRadius:8,padding:"11px 13px",border:"1px solid var(--primary)20",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
          ...
          {(person.checkIns||[]).slice().reverse().slice(0,10).map(function(c,i){var ct=ciTypes.find(function(t){return t.key===c.type});var origIdx=(person.checkIns||[]).length-1-i;return <div key={i} style={{display:"flex",alignItems:"flex-start",gap:8,padding:"6px 0",position:"relative",animation:i===0&&justFollowed?"gentleFade 0.4s ease":"none"}}>
            ...
            <div style={{flex:1,marginLeft:3}}>
              <div style={{display:"flex",alignItems:"center",gap:5,marginBottom:2}}>
                ...
                {c.loggedBy&&<span style={{fontSize:8,color:"var(--text-muted)",background:"var(--inp)",padding:"1px 5px",borderRadius:8}}>by {c.loggedBy}</span>}
      ...
      {!edit&&<div style={{marginBottom:13}}>
        ...
        <div style={{background:"var(--card)",borderRadius:8,padding:"10px 13px",display:"flex",flexDirection:"column",gap:7,boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
          {person.phone&&<div style={{display:"flex",alignItems:"center",gap:5}}><div style={{color:"var(--text)",fontSize:11,display:"flex",alignItems:"center",gap:8,fontWeight:500,flex:1}}><div style={{width:28,height:28,borderRadius:6,background:"var(--primary)15",display:"flex",alignItems:"center",justifyContent:"center"}}><I n="phone" sz={12} c="var(--primary)"/></div>{fmtPhone(person.phone)}</div><button onClick={function(){setContactOpen("text")}} style={{display:"flex",alignItems:"center",gap:3,padding:"3px 8px",borderRadius:6,background:"#3898FF15",fontSize:9,fontWeight:600,color:"#3898FF",border:"none",cursor:"pointer"}}>Text</button><button onClick={function(){setContactOpen("call")}} style={{display:"flex",alignItems:"center",gap:3,padding:"3px 8px",borderRadius:6,background:"var(--primary)15",fontSize:9,fontWeight:600,color:"var(--primary)",border:"none",cursor:"pointer"}}>Call</button></div>}
          {person.email&&<div style={{display:"flex",alignItems:"center",gap:5}}><div style={{color:"var(--text)",fontSize:11,display:"flex",alignItems:"center",gap:8,fontWeight:500,flex:1}}><div style={{width:28,height:28,borderRadius:6,background:"#3898FF15",display:"flex",alignItems:"center",justifyContent:"center"}}><I n="mail" sz={12} c="#10B981"/></div>{person.email}</div><button onClick={function(){setContactOpen("email")}} style={{display:"flex",alignItems:"center",gap:3,padding:"3px 8px",borderRadius:6,background:"#F59E0B15",fontSize:9,fontWeight:600,color:"#F59E0B",border:"none",cursor:"pointer"}}>Send Email</button></div>}
          {person.serviceAttended&&<div style={{fontSize:11,color:"var(--text-sub)",display:"flex",alignItems:"center",gap:8}}><div style={{width:28,height:28,borderRadius:6,background:"#F59E0B15",display:"flex",alignItems:"center",justifyContent:"center"}}><I n="home" sz={12} c="#D97706"/></div>{person.serviceAttended}</div>}
        </div>
      </div>}

      <Reveal open={edit}><div style={{background:"var(--card)",borderRadius:8,padding:"11px 14px",marginBottom:13,boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
        ...
        <div style={{display:"flex",gap:7,marginTop:11}}>
          ...
          <div style={{flex:1}}/>
          <button onClick={function(){if(confirm("Permanently delete "+person.firstName+" "+person.lastName+"?"))doDelete()}} style={{display:"inline-flex",alignItems:"center",gap:5,padding:"7px 14px",borderRadius:8,fontSize:11,fontWeight:600,cursor:"pointer",background:"linear-gradient(135deg,#FEE2E2,#FECACA)",color:"#EF4444",border:"none"}}><I n="trash" sz={14} c="#EF4444"/>Delete</button>
        </div>
      </div></Reveal>
      ...
      {(function(){var nsMs=(person.milestones||{})["next-steps"]||{};var bgMs=(person.milestones||{}).bgroup||{};var atMs=(person.milestones||{}).ateam||{};var hasBG=person.stages&&person.stages.bgroup&&person.stages.bgroup.completed;var hasAT=person.stages&&person.stages.ateam&&person.stages.ateam.completed;
        ...
        return <div style={{marginBottom:13}}>
          ...
          <div style={{display:"flex",gap:8}}>
            <button onClick={toggleBG} style={{flex:1,padding:"11px 13px",borderRadius:8,border:hasBG?"2px solid #EC4899":"2px dashed var(--inp-border)",background:hasBG?"#EC489910":"var(--inp)",cursor:"pointer",textAlign:"left",transition:"all 0.2s"}}>
              ...
              {hasBG&&<div onClick={function(e){e.stopPropagation()}} style={{marginTop:3}}><input style={{width:"100%",padding:"5px 8px",borderRadius:6,border:"1px solid #EC489930",background:"var(--card)",color:"var(--text)",fontSize:9,boxSizing:"border-box",outline:"none"}} placeholder="Group Leader" value={nsMs.bGroupLeader||bgMs.bGroupLeader||""} onChange={function(e){saveBGLeader(e.target.value)}}/></div>}
            </button>
            <button onClick={toggleAT} style={{flex:1,padding:"11px 13px",borderRadius:8,border:hasAT?"2px solid #34D399":"2px dashed var(--inp-border)",background:hasAT?"#34D39910":"var(--inp)",cursor:"pointer",textAlign:"left",transition:"all 0.2s"}}>
              ...
              {hasAT&&<div onClick={function(e){e.stopPropagation()}} style={{marginTop:3}}><select style={{width:"100%",padding:"5px 8px",borderRadius:6,border:"1px solid #34D39930",background:"var(--card)",color:"var(--text)",fontSize:9,cursor:"pointer"}} value={nsMs.aTeamArea||atMs.aTeamArea||""} onChange={function(e){saveATArea(e.target.value)}}><option value="">Select Area...</option>{["Worship","Production","Kids","Youth","Hospitality","Creative","Admin","Outreach"].map(function(a){return <option key={a} value={a}>{a}</option>})}</select></div>}
            </button>
          </div>
          {hasBG&&hasAT&&!person.fullyConnected&&<button onClick={function(e){confettiBurst(e);p.onUpdate({...person,fullyConnected:true,fullyConnectedDate:new Date().toISOString()})}} style={{width:"100%",marginTop:10,padding:"13px 16px",borderRadius:8,border:"2px solid #F97316",background:"linear-gradient(135deg,#FFF7ED,#FFEDD5)",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8}} onMouseEnter={function(e){e.currentTarget.style.background="linear-gradient(135deg,#F97316,#FB923C)";e.currentTarget.style.color="#fff"}} onMouseLeave={function(e){e.currentTarget.style.background="linear-gradient(135deg,#FFF7ED,#FFEDD5)";e.currentTarget.style.color="#92400E"}}><I n="target" sz={16} c="#F97316"/><div style={{textAlign:"left"}}><div style={{fontSize:12,fontWeight:700,color:"inherit"}}>Mark as Fully Connected</div><div style={{fontSize:9,fontWeight:500,opacity:0.7}}>In a BGroup and on the ATeam</div></div></button>}
          {person.fullyConnected&&<div style={{marginTop:10,background:"linear-gradient(135deg,#F9731610,#FB923C10)",border:"2px solid #F9731630",borderRadius:8,padding:"11px 14px",display:"flex",alignItems:"center",gap:8}}><I n="target" sz={16} c="#F97316"/><div style={{flex:1}}><div style={{fontSize:12,fontWeight:700,color:"#F97316"}}>Fully Connected</div><div style={{fontSize:9,color:"var(--text-muted)"}}>Completed {fmt(person.fullyConnectedDate)}</div></div></div>}
        </div>})()}
      {(function(){var fus=(person.scheduledFollowUps||[]).slice().sort(function(a,b){return new Date(a.date)-new Date(b.date)});if(fus.length===0)return null;var now=new Date();return <div style={{marginBottom:13}}><div style={{fontSize:9,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.08em",color:"var(--text-muted)",marginBottom:5}}>Scheduled Follow-ups</div><div style={{background:"var(--card)",borderRadius:8,padding:"10px 13px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>{fus.map(function(fu,i){var isDue=new Date(fu.date)<=now;var daysUntil=Math.ceil((new Date(fu.date)-now)/864e5);var stg=STAGES.find(function(s){return s.key===fu.stage});return <div key={i} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 0",borderTop:i>0?"1px solid var(--divider)":"none"}}><I n="clock" sz={12} c={isDue?"#EF4444":"#06B6D4"}/><div style={{flex:1,fontSize:10,fontWeight:600,color:isDue?"#EF4444":"var(--text)"}}>{fu.label}</div><input type="date" value={fu.date?fu.date.split("T")[0]:""} onChange={function(e){var nfu=(person.scheduledFollowUps||[]).slice();nfu[i]={...nfu[i],date:e.target.value+"T09:00:00.000Z"};p.onUpdate({...person,scheduledFollowUps:nfu})}} style={{padding:"3px 6px",borderRadius:6,border:"1px solid var(--inp-border)",background:"var(--inp)",color:"var(--text-sub)",fontSize:9,fontWeight:500,fontFamily:"'Nunito',sans-serif",appearance:"none",WebkitAppearance:"none",cursor:"pointer",width:90,boxSizing:"border-box",outline:"none"}}/><span style={{fontSize:8,fontWeight:600,color:isDue?"#EF4444":"var(--text-muted)",flexShrink:0,minWidth:28,textAlign:"right"}}>{isDue?"Due":daysUntil+"d"}</span><button onClick={function(){var nfu=(person.scheduledFollowUps||[]).slice();nfu.splice(i,1);p.onUpdate({...person,scheduledFollowUps:nfu})}} style={{width:18,height:18,borderRadius:5,background:"var(--inp)",border:"1px solid var(--inp-border)",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",flexShrink:0}}><I n="x" sz={7} c="var(--text-muted)"/></button></div>})}</div></div>})()}
      {(function(){var entries=[];var allMs=person.milestones||{};var stages=person.stages||{};STAGES.forEach(function(s){var stData=stages[s.key];var ms=allMs[s.key]||{};if(!stData||!stData.completed)return;if(s.key==="salvation"&&ms.dateSaved)entries.push({date:ms.dateSaved,label:"Saved",color:s.color,bold:true});if(s.key==="baptism"&&ms.dateBaptized)entries.push({date:ms.dateBaptized,label:"Baptized",color:s.color,bold:true});if(s.key==="next-steps"&&ms.attended)entries.push({date:ms.attendedDate||stData.date,label:"Next Steps: Attended",color:s.color,bold:false});if(s.key==="bgroup")entries.push({date:stData.date,label:"Joined BGroup"+(((allMs["next-steps"]||{}).bGroupLeader||ms.bGroupLeader)?" \u2014 "+((allMs["next-steps"]||{}).bGroupLeader||ms.bGroupLeader):""),color:s.color,bold:true});if(s.key==="ateam")entries.push({date:stData.date,label:"Joined ATeam"+(((allMs["next-steps"]||{}).aTeamArea||ms.aTeamArea)?" \u2014 "+((allMs["next-steps"]||{}).aTeamArea||ms.aTeamArea):""),color:s.color,bold:true})});if(person.fullyConnected)entries.push({date:person.fullyConnectedDate||new Date().toISOString(),label:"Fully Connected",color:"#F97316",bold:true});entries.sort(function(a,b){return new Date(a.date)-new Date(b.date)});if(entries.length===0)return null;var jDur=person.fullyConnected&&entries.length>=2?Math.round((new Date(entries[entries.length-1].date)-new Date(person.createdAt))/864e5):null;return <div style={{marginBottom:13}}><div style={{fontSize:9,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.08em",color:"var(--text-muted)",marginBottom:5}}>Timeline</div><div style={{background:"var(--card)",borderRadius:8,padding:"10px 13px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>{entries.map(function(e,i){return <div key={i} style={{display:"flex",alignItems:"flex-start",gap:8,padding:"6px 0",borderTop:i>0?"1px solid var(--divider)":"none"}}><div style={{width:8,height:8,borderRadius:"50%",background:e.color,marginTop:4,flexShrink:0}}/><div style={{flex:1,fontSize:10,fontWeight:e.bold?600:400,color:e.bold?"var(--text)":"var(--text-sub)"}}>{e.label}</div><div style={{fontSize:9,color:"var(--text-muted)",flexShrink:0}}>{fmtS(e.date)}</div></div>})}{jDur!==null&&<div style={{borderTop:"1px solid var(--divider)",paddingTop:8,marginTop:3,fontSize:9,color:"#F97316",fontWeight:600,textAlign:"center"}}>Journey: {jDur} days from First Visit to Fully Connected</div>}</div></div>})()}
      ...
      <div>
        ...
        <div style={{display:"flex",gap:5,marginBottom:8}}><input style={{flex:1,padding:"6px 10px",borderRadius:6,border:"1px solid var(--inp-border)",background:"var(--inp)",color:"var(--text)",fontSize:10,boxSizing:"border-box",outline:"none"}} placeholder="Add a note..." value={note} onChange={function(e){setNote(e.target.value)}} onKeyDown={function(e){if(e.key==="Enter")sNote()}}/><Btn label="Save" onClick={sNote} v="ghost" sx={{padding:"6px 10px",fontSize:9}}/></div>
...
function ImportModal(p){
  ...
  return <Modal title="Import CSV" onClose={p.onClose} wide>
    ...
    <button onClick={function(){ref.current.click()}} style={{width:"100%",padding:22,borderRadius:8,border:"2px dashed var(--inp-border)",background:"var(--inp)",color:"var(--text-muted)",fontSize:12,fontWeight:600,cursor:"pointer",marginBottom:10}}>Choose CSV file</button>
    <textarea style={{width:"100%",minHeight:80,padding:11,borderRadius:8,border:"1px solid var(--inp-border)",background:"var(--inp)",color:"var(--text-sub)",fontFamily:"monospace",fontSize:10,resize:"vertical",boxSizing:"border-box",outline:"none"}} placeholder="Or paste CSV here..." value={csv} onChange={function(e){proc(e.target.value)}}/>
...
function AppMain(p){
  ...
  var [view,setView]=useState("overview");var [sf,setSf]=useState(null);var [sel,setSel]=useState(null);var [add,setAdd]=useState(false);var [imp,setImp]=useState(false);var [search,setSearch]=useState("");var [ready,setReady]=useState(false);var [contactTarget,setContactTarget]=useState(null);var [acViewMode,setAcViewMode]=useState("grid");var [toast,setToast]=useState(null);var [onboardStep,setOnboardStep]=useState(null);
  var showToast=function(msg){setToast(msg);setTimeout(function(){setToast(null)},3000)};
  ...
  var [sbExpand,setSbExpand]=useState({connected:false,tools:false});
  var sideItems=[{key:"overview",icon:"home",label:"Overview",vw:"overview"},{key:"all",icon:"users",label:"All People",vw:"people"},{type:"divider"}].concat(STAGES.filter(function(s){return s.key!=="bgroup"&&s.key!=="ateam"}).map(function(s){return{key:s.key,label:s.label,color:s.color,vw:"people",sf:s.key}})).concat([{type:"divider"},{key:"_connected",icon:"target",label:"Connected",group:"connected",children:[{key:"bgroup",label:"BGroup",color:"#EC4899",vw:"people",sf:"bgroup"},{key:"ateam",label:"ATeam",color:"#34D399",vw:"people",sf:"ateam"},{key:"fully-connected",label:"Fully Connected",color:"#F97316",icon:"target",vw:"connected"}]},{type:"divider"},{key:"assigned",icon:"card",label:"Assigned Cards",vw:"assigned"},{key:"_tools",icon:"zap",label:"Tools",group:"tools",children:[{key:"quick",icon:"plus",label:"Quick Entry",vw:"quick"},{key:"bulk",icon:"send",label:"Bulk Message",vw:"bulk"},{key:"reports",icon:"chart",label:"Reports",vw:"reports"}]},{type:"divider"},{key:"settings",icon:"gear",label:"Settings",vw:"settings"}]);
  ...
  return <div style={{display:"flex",height:"100vh",fontFamily:"'Nunito',sans-serif",background:"var(--bg)",color:"var(--text)",...cssVars}}>
    <style>{["@import url('https://fonts.googleapis.com/css2?family=Nunito:wght@300;400;500;600;700;800;900&display=swap');","*,*::before,*::after{box-sizing:border-box;margin:0;padding:0;font-family:'Nunito',sans-serif !important}","::-webkit-scrollbar{width:4px;height:4px}::-webkit-scrollbar-track{background:transparent}::-webkit-scrollbar-thumb{background:rgba(0,0,0,0.12);border-radius:4px}::-webkit-scrollbar-thumb:hover{background:rgba(0,0,0,0.2)}*{scrollbar-width:thin;scrollbar-color:rgba(0,0,0,0.12) transparent}","@keyframes slideIn{from{transform:translateX(100%);opacity:0}to{transform:translateX(0);opacity:1}}","@keyframes panelSlide{from{transform:translateX(100%)}to{transform:translateX(0)}}","@keyframes fadeUp{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:translateY(0)}}","@keyframes revealDown{from{opacity:0;transform:translateY(-12px)}to{opacity:1;transform:translateY(0)}}","@keyframes revealUp{from{opacity:1;transform:translateY(0)}to{opacity:0;transform:translateY(-12px)}}","@keyframes gentleFade{from{opacity:0;transform:translateY(6px)}to{opacity:1;transform:translateY(0)}}","@keyframes pulse{0%{transform:scale(1)}50%{transform:scale(1.08)}100%{transform:scale(1)}}","@keyframes shimmer{0%{background-position:-200% 0}100%{background-position:200% 0}}","@keyframes confettiFall{0%{transform:translateY(-10px) rotate(0deg);opacity:1}100%{transform:translateY(60px) rotate(360deg);opacity:0}}",".score-ring-pulse{animation:pulse 2s ease-in-out infinite}",".shimmer-btn{background-size:200% 100%;animation:shimmer 2s ease infinite}","input:focus,textarea:focus,select:focus{outline:none;border-color:var(--primary) !important;box-shadow:0 0 0 3px var(--primary)20 !important}","input[type=date]{color-scheme:light}","input[type=date]::-webkit-calendar-picker-indicator{opacity:0.4;cursor:pointer}","button{cursor:pointer;font-family:'Nunito',sans-serif !important}button:active{transform:scale(0.97)}",".weavr-widget-wrap:hover .weavr-widget-ctrl{opacity:1 !important}",".weavr-widget-scroll>div>*{flex:1;min-height:0}",".weavr-widget-scroll::-webkit-scrollbar{width:3px}",".weavr-widget-scroll::-webkit-scrollbar-track{background:transparent;margin:8px}",".weavr-widget-scroll::-webkit-scrollbar-thumb{background:rgba(0,0,0,0.08);border-radius:8px}",".weavr-widget-scroll::-webkit-scrollbar-thumb:hover{background:rgba(0,0,0,0.15)}",".weavr-widget-scroll{scrollbar-width:thin;scrollbar-color:rgba(0,0,0,0.08) transparent}",".weavr-ov-grid{transition:all 0.3s ease}","@media(max-width:768px){.weavr-sidebar{display:none !important}.weavr-collapse-btn{display:none !important}.weavr-mobile-nav{display:flex !important}.weavr-main{margin-left:0 !important}.weavr-main-header{padding:12px 14px !important}.weavr-main-header>div:first-child{display:none !important}.weavr-main-content{padding:0 14px 100px !important}.weavr-pipeline-hero{padding:20px 16px !important;border-radius:8px !important;margin-bottom:16px !important}.weavr-pipeline-grid{grid-template-columns:repeat(3,1fr) !important;gap:6px !important}.weavr-pipeline-grid>div{padding:12px 6px !important}.weavr-pipeline-grid>div>div:first-child{font-size:20px !important}.weavr-kpi-grid{grid-template-columns:repeat(2,1fr) !important}.weavr-kpi-grid>div{padding:16px !important}.weavr-kpi-grid>div>div:last-child{font-size:24px !important}.weavr-card-grid{grid-template-columns:1fr !important}.weavr-panel{width:100vw !important;max-width:100vw !important;border-radius:0 !important}.weavr-panel-header{padding:16px 16px 14px !important}.weavr-panel-actions{padding:10px 16px !important}.weavr-panel-actions>div{gap:4px !important}.weavr-panel-body{padding:14px 16px !important}.weavr-qe-layout{flex-direction:column !important}.weavr-qe-recent{width:100% !important}.weavr-next-up{flex-direction:column !important}.weavr-next-up>div:first-child{padding:16px !important;gap:12px !important}.weavr-next-up>div:first-child>div:last-child{display:none !important}.weavr-ws-grid{grid-template-columns:repeat(2,1fr) !important}.weavr-ws-input{grid-template-columns:repeat(2,1fr) !important}.weavr-widget-picker{grid-template-columns:repeat(2,1fr) !important}.weavr-sort-wrap{overflow-x:auto;-webkit-overflow-scrolling:touch;flex-wrap:nowrap !important}.weavr-ac-header{flex-direction:column !important;align-items:flex-start !important;gap:10px !important}.weavr-ac-header>div:last-child{width:100% !important}.weavr-ac-teams{overflow-x:auto !important;-webkit-overflow-scrolling:touch !important;flex-wrap:nowrap !important;padding-bottom:4px}.weavr-fc-header{flex-direction:column !important;align-items:flex-start !important;gap:8px !important}table{display:block;overflow-x:auto;white-space:nowrap;-webkit-overflow-scrolling:touch}.weavr-stats-compare{flex-direction:column !important;align-items:flex-start !important;gap:6px !important}.weavr-ov-grid{grid-template-columns:1fr !important;gap:14px !important;grid-auto-rows:auto !important}.weavr-login-split{flex-direction:column !important}.weavr-login-left{display:none !important}.weavr-login-right{width:100% !important;min-height:100vh !important;justify-content:center !important;border-left:none !important;padding:40px 24px !important}.weavr-login-right h2{font-size:24px !important}.weavr-login-mobile-logo{display:flex !important}.weavr-portal-grid{grid-template-columns:repeat(2,1fr) !important;gap:12px !important}.weavr-portal-header{padding:20px 0 !important}.weavr-portal-hero{padding:32px 0 40px !important}.weavr-portal-hero h1{font-size:26px !important}.weavr-portal-card{padding:20px 16px !important}.weavr-modal-inner{width:95vw !important;max-width:95vw !important;padding:20px !important;margin:0 !important;border-radius:8px !important}.weavr-modal-inner h3{font-size:17px !important}}","@media(max-width:480px){.weavr-pipeline-grid{grid-template-columns:repeat(2,1fr) !important}.weavr-kpi-grid{grid-template-columns:1fr !important}.weavr-main-header{flex-wrap:wrap !important;gap:8px !important}.weavr-ws-grid{grid-template-columns:1fr 1fr !important}.weavr-ws-input{grid-template-columns:1fr 1fr !important}.weavr-portal-grid{grid-template-columns:1fr !important}.weavr-weekly-summary-grid{grid-template-columns:1fr !important}.weavr-login-right{padding:32px 20px !important}}"].join("\n")}</style>
      ...
      <div style={{padding:sbCollapsed?"12px 0":"12px 10px",borderBottom:"1px solid var(--sb-divider)",display:"flex",alignItems:"center",justifyContent:sbCollapsed?"center":"flex-start",gap:sbCollapsed?0:10,position:"relative",zIndex:80}}>
        <div style={{position:"relative",width:sbCollapsed?"auto":"100%"}}>
          <button onClick={function(){setToolDrop(!toolDrop)}} style={{display:"flex",alignItems:"center",gap:sbCollapsed?0:10,background:"none",border:"none",cursor:"pointer",padding:0,width:sbCollapsed?"auto":"100%"}}>
            <div style={{width:32,height:32,borderRadius:8,background:"var(--logo)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><WeavrLogo sz={20}/></div>
            {!sbCollapsed&&<div style={{flex:1,display:"flex",alignItems:"center",gap:5}}><div style={{fontSize:13,fontWeight:800,color:"var(--sb-text)",letterSpacing:"0.12em"}}>WEAVR</div><span style={{fontSize:7,fontWeight:600,color:"var(--sb-muted)",background:"var(--sb-active)",padding:"2px 4px",borderRadius:8,letterSpacing:"0.04em"}}>Connect</span></div>}
          ...
          {toolDrop&&<div style={{position:"absolute",top:"100%",left:sbCollapsed?-12:-10,right:sbCollapsed?-12:-10,marginTop:0,background:isDark?"#1A1744":"#2E2A8A",borderRadius:0,padding:"4px 6px",zIndex:200,boxShadow:"0 8px 20px rgba(0,0,0,0.35)",borderTop:"1px solid rgba(255,255,255,0.06)"}}>
            {TOOLS.map(function(t){var isCurrent=t.key==="connect";return <button key={t.key} onClick={t.ready?function(){setToolDrop(false);if(!isCurrent)alert("Coming soon: Weavr "+t.name)}:undefined} style={{display:"flex",alignItems:"center",justifyContent:sbCollapsed?"center":"flex-start",gap:sbCollapsed?0:7,width:"100%",padding:sbCollapsed?"5px 0":"4px 6px",borderRadius:6,border:"none",background:isCurrent?"rgba(255,255,255,0.08)":"transparent",cursor:t.ready?"pointer":"default",opacity:t.ready?1:0.6,textAlign:"left",transition:"all 0.15s"}} onMouseEnter={t.ready?function(e){if(!isCurrent)e.currentTarget.style.background="rgba(255,255,255,0.06)"}:undefined} onMouseLeave={t.ready?function(e){if(!isCurrent)e.currentTarget.style.background="transparent"}:undefined}>
              <div style={{width:20,height:20,borderRadius:5,background:isCurrent?t.color:t.color+"25",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><I n={t.icon} sz={9} c={isCurrent?"#fff":t.color}/></div>
            ...
            <button onClick={function(){setToolDrop(false);p.onBackToPortal()}} style={{display:"flex",alignItems:"center",justifyContent:sbCollapsed?"center":"flex-start",gap:sbCollapsed?0:7,width:"100%",padding:sbCollapsed?"5px 0":"4px 6px",borderRadius:6,border:"none",background:"transparent",cursor:"pointer",textAlign:"left",marginTop:2,borderTop:"1px solid rgba(255,255,255,0.06)",paddingTop:6}}><I n="home" sz={9} c="rgba(255,255,255,0.35)"/>{!sbCollapsed&&<span style={{fontSize:9,color:"rgba(255,255,255,0.35)"}}>Portal</span>}</button>
          </div>}
        </div>
      </div>
      <nav style={{flex:1,padding:sbCollapsed?"12px 6px 6px":"14px 8px 6px",overflowY:"auto",overflowX:"hidden"}}>{sideItems.map(function(item,i){if(item.type==="divider")return <div key={"d"+i} style={{height:1,background:"var(--sb-divider)",margin:sbCollapsed?"5px 4px":"6px 8px"}}/>;if(item.group){var isOpen=sbExpand[item.group];var childActive=item.children.some(function(c){return activeKey===c.key});return <div key={item.key}><button onClick={function(){var ne={...sbExpand};ne[item.group]=!ne[item.group];setSbExpand(ne)}} onMouseEnter={function(e){if(!childActive)e.currentTarget.style.background="var(--sb-active)"}} onMouseLeave={function(e){if(!childActive)e.currentTarget.style.background="transparent"}} style={{display:"flex",alignItems:"center",justifyContent:sbCollapsed?"center":"flex-start",gap:sbCollapsed?0:10,width:"100%",padding:sbCollapsed?"8px 0":"8px 10px",borderRadius:6,border:"none",fontSize:11,fontWeight:childActive?600:500,background:childActive&&!isOpen?"var(--sb-active)":"transparent",color:childActive?"var(--sb-text)":"var(--sb-muted)",textAlign:"left",marginBottom:1,transition:"all 0.15s"}}><I n={item.icon} sz={16} c={item.key==="_connected"?"#F97316":childActive?"var(--sb-text)":"var(--sb-muted)"}/>{!sbCollapsed&&<span style={{flex:1}}>{item.label}</span>}{!sbCollapsed&&<span style={{fontSize:8,color:"var(--sb-muted)",opacity:0.5,transform:isOpen?"rotate(180deg)":"rotate(0)",transition:"transform 0.2s",display:"inline-flex"}}>{"▾"}</span>}</button>{isOpen&&!sbCollapsed&&<div style={{paddingLeft:16,marginBottom:2}}>{item.children.map(function(c){var cActive=activeKey===c.key;return <button key={c.key} onClick={function(){nav(c.vw,c.sf)}} onMouseEnter={function(e){if(!cActive)e.currentTarget.style.background="var(--sb-active)"}} onMouseLeave={function(e){if(!cActive)e.currentTarget.style.background="transparent"}} style={{display:"flex",alignItems:"center",gap:8,width:"100%",padding:"6px 10px",borderRadius:6,border:"none",fontSize:10,fontWeight:cActive?600:500,background:cActive?"var(--sb-active)":"transparent",color:cActive?"var(--sb-text)":"var(--sb-muted)",textAlign:"left",marginBottom:1,transition:"all 0.15s"}}>{c.icon?<I n={c.icon} sz={13} c={c.key==="fully-connected"?"#F97316":cActive?"var(--sb-text)":"var(--sb-muted)"}/>:<span style={{width:6,height:6,borderRadius:"50%",background:c.color,display:"inline-block"}}/>}<span style={{flex:1}}>{c.label}</span>{c.color&&<span style={{fontSize:9,fontWeight:600,color:"var(--sb-muted)",opacity:0.7}}>{countForStage(c.key)}</span>}</button>})}</div>}</div>}var active=activeKey===item.key;return <button key={item.key} onClick={function(){nav(item.vw,item.sf)}} onMouseEnter={function(e){if(!active)e.currentTarget.style.background="var(--sb-active)"}} onMouseLeave={function(e){if(!active)e.currentTarget.style.background="transparent"}} style={{display:"flex",alignItems:"center",justifyContent:sbCollapsed?"center":"flex-start",gap:sbCollapsed?0:10,width:"100%",padding:sbCollapsed?"8px 0":"8px 10px",borderRadius:6,border:"none",fontSize:11,fontWeight:active?600:500,background:active?"var(--sb-active)":"transparent",color:active?"var(--sb-text)":"var(--sb-muted)",textAlign:"left",marginBottom:1,transition:"all 0.15s"}}>{item.icon?<I n={item.icon} sz={16} c={active?"var(--sb-text)":"var(--sb-muted)"}/>:<span style={{width:8,height:8,borderRadius:"50%",background:item.color,display:"inline-block"}}/>}{!sbCollapsed&&<span style={{flex:1}}>{item.label}</span>}{!sbCollapsed&&item.color&&<span style={{fontSize:9,fontWeight:600,color:"var(--sb-muted)",opacity:0.7}}>{countForStage(item.key)}</span>}</button>})}</nav>
      <div style={{padding:sbCollapsed?"6px 6px 10px":"8px 10px 12px",borderTop:"1px solid var(--sb-divider)"}}>
        {!sbCollapsed&&<button style={{width:"100%",padding:"8px 11px",borderRadius:8,border:"none",background:"#3898FF",color:"#fff",fontSize:11,fontWeight:600,display:"flex",alignItems:"center",justifyContent:"center",gap:5,transition:"all 0.15s",boxShadow:"0 2px 8px rgba(56,152,255,0.3)"}} onClick={function(){setAdd(true)}} onMouseEnter={function(e){e.currentTarget.style.opacity="0.9";e.currentTarget.style.transform="translateY(-1px)"}} onMouseLeave={function(e){e.currentTarget.style.opacity="1";e.currentTarget.style.transform="translateY(0)"}}><I n="plus" sz={13} c="#fff"/>Add Person</button>}
        {sbCollapsed&&<button style={{width:"100%",padding:"7px 0",borderRadius:8,border:"none",background:"#3898FF",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 2px 8px rgba(56,152,255,0.3)"}} onClick={function(){setAdd(true)}} title="Add Person"><I n="plus" sz={14} c="#fff"/></button>}
        {!sbCollapsed&&<div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:3,marginTop:12}}>
          ...
          <div style={{textAlign:"center",padding:"4px 0",background:"#F9731608",borderRadius:6}}><div style={{fontSize:12,fontWeight:700,color:"#F97316"}}>{people.filter(function(x){return x.fullyConnected}).length}</div><div style={{fontSize:7,color:"var(--sb-muted)",fontWeight:700,textTransform:"uppercase",letterSpacing:"0.05em",marginTop:1}}>Connected</div></div>
    ...
    <main style={{flex:1,overflowY:"auto",minHeight:"100vh"}}>
      <div className="weavr-main-header" style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 32px",position:"sticky",top:0,background:isDark?cw.darkSolid:isMono?"#F8F9FA":"#fff",zIndex:50,boxShadow:"0 1px 0 var(--divider)"}}>
        <div style={{display:"flex",alignItems:"center",gap:6,fontSize:11,fontFamily:"'Nunito',sans-serif"}}>
          <span style={{fontWeight:800,textTransform:"uppercase",letterSpacing:"0.06em",color:"var(--text)"}}>{new Date().toLocaleDateString("en-US",{weekday:"long"})}</span>
          <span style={{fontWeight:400,color:"var(--text-sub)"}}>{new Date().toLocaleDateString("en-US",{month:"long",day:"numeric",year:"numeric"})}</span>
        </div>
        <div style={{display:"flex",alignItems:"center",gap:5}}>
          <div style={{display:"flex",alignItems:"center",overflow:"hidden",borderRadius:8,border:search?"1px solid var(--inp-border)":"1px solid transparent",background:search?"var(--inp)":"transparent",transition:"all 0.3s cubic-bezier(0.22,1,0.36,1)"}}>
            {search&&<input value={search} onChange={function(e){setSearch(e.target.value)}} placeholder="Search..." autoFocus style={{width:160,padding:"6px 10px",border:"none",background:"transparent",color:"var(--text)",fontSize:11,outline:"none",boxSizing:"border-box"}}/>}
            {search&&<button onClick={function(){setSearch("")}} style={{background:"none",border:"none",cursor:"pointer",padding:"0 6px 0 0",display:"flex"}}><I n="x" sz={11} c="var(--text-muted)"/></button>}
          </div>
          {[{icon:search?"search":"search",title:"Search",onClick:function(){if(!search)setSearch(" ");else setSearch("")},active:!!search},{icon:config.theme==="mono"?"eye":"sun",title:config.theme==="light"?"Dark Mode":config.theme==="dark"?"Simple Mode":"Light Mode",onClick:function(){var next=config.theme==="light"?"dark":config.theme==="dark"?"mono":"light";setConfig({...config,theme:next})}},{icon:"upload",title:"Import",onClick:function(){setImp(true)}}].map(function(btn,bi){return <button key={bi} onClick={btn.onClick} style={{display:"flex",alignItems:"center",justifyContent:"center",width:30,height:30,borderRadius:6,border:"none",background:btn.active?"var(--primary)":"transparent",cursor:"pointer",transition:"all 0.15s"}} title={btn.title} onMouseEnter={function(e){if(!btn.active)e.currentTarget.style.background="var(--hover)"}} onMouseLeave={function(e){if(!btn.active)e.currentTarget.style.background="transparent"}}><I n={btn.icon} sz={14} c={btn.active?"#fff":"var(--text-muted)"}/></button>})}
        </div>
      </div>
      <div className="weavr-main-content" style={{padding:"19px 32px 32px",animation:"fadeUp 0.5s cubic-bezier(0.22,1,0.36,1)"}}>
        ...
        :search.trim()&&view!=="settings"?<PeopleView people={fp} teams={teams} stageFilter={null} search={search} setSearch={setSearch} onPerson={setSel} onImport={function(){setImp(true)}} onDelete={function(id){setPeople(function(prev){return prev.filter(function(x){return x.id!==id})})}} onUpdate={function(u){setPeople(function(prev){return prev.map(function(x){return x.id===u.id?u:x})})}} showToast={showToast}/>
        :view==="overview"?<Overview people={fp} teams={teams} onPerson={setSel} navTo={nav} onAdd={function(){setAdd(true)}} onAddPerson={function(per){setPeople(function(prev){return prev.concat([per])})}} onImport={function(){setImp(true)}} templates={tpl} config={config} setConfig={setConfig} onContact={setContactTarget} showToast={showToast} onUpdatePerson={function(u){setPeople(function(prev){return prev.map(function(x){return x.id===u.id?u:x})})}}/>
        ...
        :view==="assigned"?<AssignedCards people={fp} teams={teams} onPerson={setSel} templates={tpl} config={config} onContact={setContactTarget} acView={acViewMode} setAcView={setAcViewMode} onUpdate={function(u){setPeople(function(prev){return prev.map(function(x){return x.id===u.id?u:x})})}}/>
        :view==="connected"?<FullyConnected people={fp} onPerson={setSel}/>
        :<PeopleView people={fp} teams={teams} stageFilter={sf} search={search} setSearch={setSearch} onPerson={setSel} onImport={function(){setImp(true)}} onDelete={function(id){setPeople(function(prev){return prev.filter(function(x){return x.id!==id})})}} onUpdate={function(u){setPeople(function(prev){return prev.map(function(x){return x.id===u.id?u:x})})}} showToast={showToast}/>})()}
      </div>
    </main>
    ...
    {contactTarget&&contactTarget.mode==="text"&&<TextModal person={contactTarget.person} config={config} fuTemplate={contactTarget.fuTemplate} onLog={function(type,via){var per=contactTarget.person;var now=new Date().toISOString();var fuCfg=(config.followUpConfig||{})[per.currentStage]||{};var fuDelay=fuCfg.delay||FOLLOW_UP_DELAYS[per.currentStage]||5;var fuDate=new Date();fuDate.setDate(fuDate.getDate()+fuDelay);var fuLabel=fuCfg.label||FOLLOW_UP_LABELS[per.currentStage]||"Follow-up";setPeople(function(prev){return prev.map(function(x){if(x.id!==per.id)return x;return{...x,lastContactDate:now,checkIns:[...(x.checkIns||[]),{type:type,note:"Sent via "+via,date:now}],scheduledFollowUps:[...(x.scheduledFollowUps||[]).filter(function(fu){return new Date(fu.date)>new Date()}),{date:fuDate.toISOString(),label:fuLabel,stage:per.currentStage,type:"text"}]}})});if(contactTarget.onLogDone)contactTarget.onLogDone(type,via);showToast(per.firstName+" — text logged")}} onClose={function(){setContactTarget(null)}}/>}
    {contactTarget&&contactTarget.mode==="email"&&<EmailModal person={contactTarget.person} config={config} fuTemplate={contactTarget.fuTemplate} onLog={function(type,via){var per=contactTarget.person;var now=new Date().toISOString();var fuCfg=(config.followUpConfig||{})[per.currentStage]||{};var fuDelay=fuCfg.delay||FOLLOW_UP_DELAYS[per.currentStage]||5;var fuDate=new Date();fuDate.setDate(fuDate.getDate()+fuDelay);var fuLabel=fuCfg.label||FOLLOW_UP_LABELS[per.currentStage]||"Follow-up";setPeople(function(prev){return prev.map(function(x){if(x.id!==per.id)return x;return{...x,lastContactDate:now,checkIns:[...(x.checkIns||[]),{type:type,note:"Sent via "+via,date:now}],scheduledFollowUps:[...(x.scheduledFollowUps||[]).filter(function(fu){return new Date(fu.date)>new Date()}),{date:fuDate.toISOString(),label:fuLabel,stage:per.currentStage,type:"email"}]}})});if(contactTarget.onLogDone)contactTarget.onLogDone(type,via);showToast(per.firstName+" — email logged")}} onClose={function(){setContactTarget(null)}}/>}
    {contactTarget&&contactTarget.mode==="call"&&<CallModal person={contactTarget.person} config={config} onLog={function(type,via){var per=contactTarget.person;var now=new Date().toISOString();setPeople(function(prev){return prev.map(function(x){if(x.id!==per.id)return x;return{...x,lastContactDate:now,checkIns:[...(x.checkIns||[]),{type:type,note:"Called via "+via,date:now}]}})});if(contactTarget.onLogDone)contactTarget.onLogDone(type,via);showToast(per.firstName+" — call logged")}} onClose={function(){setContactTarget(null)}}/>}
    {toast&&<div style={{position:"fixed",bottom:24,left:"50%",transform:"translateX(-50%)",background:"var(--text)",color:"var(--bg)",padding:"10px 20px",borderRadius:8,fontSize:11,fontWeight:600,boxShadow:"0 8px 32px rgba(0,0,0,0.2)",zIndex:9999,animation:"fadeUp 0.3s ease",display:"flex",alignItems:"center",gap:7}}><I n="check" sz={12} c="var(--bg)"/>{toast}</div>}
  </div>;
}
...
var TOOLS=[
  ...
  {key:"sections",name:"Sections",desc:"Section pastor dashboard",icon:"flag",color:"#3898FF",grad:"linear-gradient(135deg,#67E8F9,#0891B2)",ready:false},
    ...
    <div style={{maxWidth:1000,margin:"0 auto",padding:"0 22px",width:"100%",display:"flex",flexDirection:"column",flex:1}}>
      <div className="weavr-portal-header" style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"16px 0",animation:"loginSlideUp 0.5s cubic-bezier(0.22,1,0.36,1) 0.1s both",flexShrink:0}}>
        <div style={{display:"flex",alignItems:"center",gap:11}}>
          <div style={{width:40,height:40,borderRadius:8,background:"linear-gradient(135deg,#7C3AED,#06B6D4)",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 4px 20px rgba(124,58,237,0.3)"}}><WeavrLogo sz={22}/></div>
          <div style={{fontSize:17,fontWeight:800,color:"#fff",letterSpacing:"0.12em"}}>WEAVR</div>
        </div>
        <button onClick={p.onLogout} style={{padding:"7px 16px",borderRadius:8,border:"1px solid rgba(255,255,255,0.12)",background:"rgba(255,255,255,0.06)",color:"rgba(255,255,255,0.6)",fontSize:10,fontWeight:600,cursor:"pointer",backdropFilter:"blur(8px)",transition:"all 0.2s"}} onMouseEnter={function(e){e.currentTarget.style.background="rgba(255,255,255,0.12)"}} onMouseLeave={function(e){e.currentTarget.style.background="rgba(255,255,255,0.06)"}}>Sign Out</button>
      </div>
      ...
      <div style={{textAlign:"center",padding:"16px 0",flexShrink:0}}>
        <div style={{display:"inline-flex",alignItems:"center",gap:7,padding:"6px 14px",borderRadius:8,background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.06)"}}>
    ...
    {!authed&&<div style={{position:"fixed",inset:0,zIndex:300,display:"flex"}} className="weavr-login-split">
      <div className="weavr-login-left" style={{flex:1,background:"linear-gradient(145deg,#3730A3 0%,#4338CA 25%,#3B35B0 50%,#2E2A8A 75%,#252262 100%)",display:"flex",flexDirection:"column",justifyContent:"center",padding:"48px 45px",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{position:"relative",zIndex:1}}>
          <div style={{display:"flex",alignItems:"center",gap:11,marginBottom:45,animation:"loginSlideUp 0.7s cubic-bezier(0.22,1,0.36,1) both"}}>
            <div style={{width:48,height:48,borderRadius:8,background:"linear-gradient(135deg,#7C3AED,#06B6D4)",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 4px 20px rgba(124,58,237,0.3)"}}><WeavrLogo sz={26}/></div>
            <div style={{fontSize:17,fontWeight:800,color:"#fff",letterSpacing:"0.12em"}}>WEAVR</div>
          </div>
      ...
      <div className="weavr-login-right" style={{width:400,flexShrink:0,background:"linear-gradient(180deg,#252262 0%,#1E1B4B 100%)",display:"flex",flexDirection:"column",justifyContent:"center",padding:"48px 42px"}}>
        <div style={{animation:authAnim?"authFadeOut 0.5s ease forwards":"authFadeIn 0.6s ease 0.3s both"}}>
          <div className="weavr-login-mobile-logo" style={{display:"none",alignItems:"center",gap:10,marginBottom:26}}><div style={{width:40,height:40,borderRadius:8,background:"linear-gradient(135deg,#7C3AED,#06B6D4)",display:"flex",alignItems:"center",justifyContent:"center"}}><WeavrLogo sz={22}/></div><div style={{fontSize:16,fontWeight:800,color:"#fff",letterSpacing:"0.12em"}}>WEAVR</div></div>
          ...
          <div style={{display:"flex",gap:0,marginBottom:26,background:"rgba(255,255,255,0.06)",borderRadius:8,padding:2}}>
          ...
          <div style={{marginBottom:18}}>
            ...
            <input style={{width:"100%",padding:"11px 14px",borderRadius:8,border:"1px solid rgba(255,255,255,0.08)",background:"rgba(255,255,255,0.04)",color:"#fff",fontSize:12,boxSizing:"border-box",outline:"none",transition:"border-color 0.2s"}} value={user} onChange={function(e){setUser(e.target.value)}} onKeyDown={function(e){if(e.key==="Enter"){if(authTab==="signin")doLogin();else doSignup()}}} placeholder="Enter your username" autoFocus onFocus={function(e){e.currentTarget.style.borderColor="rgba(139,92,246,0.5)"}} onBlur={function(e){e.currentTarget.style.borderColor="rgba(255,255,255,0.08)"}}/>
          </div>
          ...
          <div style={{marginBottom:authTab==="signup"?22:32}}>
            ...
            <input type="password" style={{width:"100%",padding:"11px 14px",borderRadius:8,border:"1px solid rgba(255,255,255,0.08)",background:"rgba(255,255,255,0.04)",color:"#fff",fontSize:12,boxSizing:"border-box",outline:"none",transition:"border-color 0.2s"}} value={pass} onChange={function(e){setPass(e.target.value)}} onKeyDown={function(e){if(e.key==="Enter"&&authTab==="signin")doLogin()}} placeholder="Enter password" onFocus={function(e){e.currentTarget.style.borderColor="rgba(139,92,246,0.5)"}} onBlur={function(e){e.currentTarget.style.borderColor="rgba(255,255,255,0.08)"}}/>
          </div>
          ...
          {authTab==="signup"&&<div style={{marginBottom:26}}>
            ...
            <input type="password" style={{width:"100%",padding:"11px 14px",borderRadius:8,border:"1px solid rgba(255,255,255,0.08)",background:"rgba(255,255,255,0.04)",color:"#fff",fontSize:12,boxSizing:"border-box",outline:"none",transition:"border-color 0.2s"}} value={confirmPass} onChange={function(e){setConfirmPass(e.target.value)}} onKeyDown={function(e){if(e.key==="Enter")doSignup()}} placeholder="Confirm password" onFocus={function(e){e.currentTarget.style.borderColor="rgba(139,92,246,0.5)"}} onBlur={function(e){e.currentTarget.style.borderColor="rgba(255,255,255,0.08)"}}/>
          </div>}

          {err&&<div style={{fontSize:10,color:"#FCA5A5",marginBottom:13,textAlign:"center",background:"rgba(239,68,68,0.08)",padding:"8px 11px",borderRadius:8,border:"1px solid rgba(239,68,68,0.15)"}}>{err}</div>}

          <button onClick={authTab==="signin"?doLogin:doSignup} style={{width:"100%",padding:"13px",borderRadius:8,border:"none",background:"linear-gradient(135deg,#8B5CF6,#7C3AED)",color:"#fff",fontSize:12,fontWeight:700,cursor:"pointer",transition:"all 0.2s",boxShadow:"0 4px 20px rgba(124,58,237,0.3)"}} onMouseEnter={function(e){e.currentTarget.style.transform="translateY(-2px)";e.currentTarget.style.boxShadow="0 8px 32px rgba(124,58,237,0.5)"}} onMouseLeave={function(e){e.currentTarget.style.transform="translateY(0)";e.currentTarget.style.boxShadow="0 4px 20px rgba(124,58,237,0.3)"}}>{authTab==="signin"?"Sign In":"Create Account"}</button>
