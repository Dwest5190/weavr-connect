    ...
    <aside className="weavr-sidebar" style={{width:sbCollapsed?60:220,background:"var(--sidebar)",display:"flex",flexDirection:"column",flexShrink:0,height:"100vh",position:"sticky",top:0,transition:"width 0.25s cubic-bezier(0.22,1,0.36,1)",overflow:"visible",zIndex:60}}>
      <div style={{padding:sbCollapsed?"12px 0":"12px 10px",borderBottom:"1px solid var(--sb-divider)",display:"flex",alignItems:"center",justifyContent:sbCollapsed?"center":"flex-start",gap:sbCollapsed?0:10,position:"relative",zIndex:80}}>
        <div style={{position:"relative",width:sbCollapsed?"auto":"100%"}}>
          ...
          {toolDrop&&<div style={{position:"absolute",top:"100%",left:sbCollapsed?-12:-10,right:sbCollapsed?-12:-10,marginTop:1,background:isDark?"#1A1744":"#2E2A8A",borderRadius:"0 0 10px 10px",padding:"4px 6px",zIndex:200,boxShadow:"0 8px 20px rgba(0,0,0,0.35)",borderTop:"1px solid rgba(255,255,255,0.06)"}}>
      ...
      <nav style={{flex:1,padding:sbCollapsed?"6px 6px":"6px 8px",overflowY:"auto",overflowX:"hidden"}}>{sideItems.map(function(item,i){if(item.type==="divider")return <div key={"d"+i} style={{height:1,background:"var(--sb-divider)",margin:sbCollapsed?"5px 4px":"6px 8px"}}/>;var active=activeKey===item.key;return <button key={item.key} onClick={function(){nav(item.vw,item.sf)}} onMouseEnter={function(e){if(!active)e.currentTarget.style.background="var(--sb-active)"}} onMouseLeave={function(e){if(!active)e.currentTarget.style.background="transparent"}} style={{display:"flex",alignItems:"center",justifyContent:sbCollapsed?"center":"flex-start",gap:sbCollapsed?0:10,width:"100%",padding:sbCollapsed?"8px 0":"8px 10px",borderRadius:7,border:"none",fontSize:11,fontWeight:active?600:500,background:active?"var(--sb-active)":"transparent",color:active?"var(--sb-text)":"var(--sb-muted)",textAlign:"left",marginBottom:1,transition:"all 0.15s"}}>{item.icon?<I n={item.icon} sz={16} c={active?"var(--sb-text)":"var(--sb-muted)"}/>:<span style={{width:8,height:8,borderRadius:"50%",background:item.color,display:"inline-block"}}/>}{!sbCollapsed&&<span style={{flex:1}}>{item.label}</span>}{!sbCollapsed&&item.color&&<span style={{fontSize:9,fontWeight:600,color:"var(--sb-muted)",opacity:0.7}}>{countForStage(item.key)}</span>}</button>})}</nav>
