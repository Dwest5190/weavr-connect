...
function AppMain(p){
  ...
  return <div style={{display:"flex",height:"100vh",fontFamily:"'Nunito',sans-serif",background:"var(--bg)",color:"var(--text)",...cssVars}}>
    ...
    <aside className="weavr-sidebar" style={{width:sbCollapsed?60:220,background:"var(--sidebar)",display:"flex",flexDirection:"column",flexShrink:0,height:"100vh",position:"sticky",top:0,transition:"width 0.25s cubic-bezier(0.22,1,0.36,1)",overflow:"visible",zIndex:60}}>
      ...
      <nav style={{flex:1,padding:sbCollapsed?"14px 8px":"14px 14px",overflowY:"auto",overflowX:"hidden"}}>{sideItems.map(function(item,i){if(item.type==="divider")return <div key={"d"+i} style={{height:1,background:"var(--sb-divider)",margin:sbCollapsed?"8px 4px":"10px 14px"}}/>;var active=activeKey===item.key;return <button key={item.key} onClick={function(){nav(item.vw,item.sf)}} onMouseEnter={function(e){if(!active)e.currentTarget.style.background="var(--sb-active)"}} onMouseLeave={function(e){if(!active)e.currentTarget.style.background="transparent"}} style={{display:"flex",alignItems:"center",justifyContent:sbCollapsed?"center":"flex-start",gap:sbCollapsed?0:12,width:"100%",padding:sbCollapsed?"11px 0":"11px 14px",borderRadius:10,border:"none",fontSize:11,fontWeight:active?600:500,background:active?"var(--sb-active)":"transparent",color:active?"var(--sb-text)":"var(--sb-muted)",textAlign:"left",marginBottom:2,transition:"all 0.15s"}}>{item.icon?<I n={item.icon} sz={16} c={active?"var(--sb-text)":"var(--sb-muted)"}/>:<span style={{width:8,height:8,borderRadius:"50%",background:item.color,display:"inline-block"}}/>}{!sbCollapsed&&<span style={{flex:1}}>{item.label}</span>}{!sbCollapsed&&item.color&&<span style={{fontSize:9,fontWeight:600,color:"var(--sb-muted)",opacity:0.7}}>{countForStage(item.key)}</span>}</button>})}</nav>
      <div style={{padding:sbCollapsed?"10px 8px 14px":"14px 14px 18px",borderTop:"1px solid var(--sb-divider)"}}>
        {!sbCollapsed&&<button style={{width:"100%",padding:"10px 11px",borderRadius:10,border:"none",background:"var(--sb-active)",color:"var(--sb-text)",fontSize:11,fontWeight:600,display:"flex",alignItems:"center",justifyContent:"center",gap:5,transition:"all 0.15s"}} onClick={function(){setAdd(true)}}><I n="plus" sz={14} c="var(--sb-text)"/>Add Person</button>}
        {sbCollapsed&&<button style={{width:"100%",padding:"8px 0",borderRadius:10,border:"none",background:"var(--sb-active)",display:"flex",alignItems:"center",justifyContent:"center"}} onClick={function(){setAdd(true)}} title="Add Person"><I n="plus" sz={16} c="var(--sb-text)"/></button>}
        {!sbCollapsed&&<div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:3,marginTop:10}}>
          <div style={{textAlign:"center",padding:"6px 0",background:"var(--sb-active)",borderRadius:8}}><div style={{fontSize:13,fontWeight:700,color:"var(--sb-text)"}}>{people.length}</div><div style={{fontSize:8,color:"var(--sb-muted)",fontWeight:700,textTransform:"uppercase",letterSpacing:"0.05em",marginTop:1}}>Tracked</div></div>
          <div style={{textAlign:"center",padding:"6px 0",background:"#F59E0B08",borderRadius:8}}><div style={{fontSize:13,fontWeight:700,color:"#F59E0B"}}>{people.filter(function(x){return x.fullyConnected}).length}</div><div style={{fontSize:8,color:"var(--sb-muted)",fontWeight:700,textTransform:"uppercase",letterSpacing:"0.05em",marginTop:1}}>Connected</div></div>
          <div style={{textAlign:"center",padding:"6px 0",background:"#EF444408",borderRadius:8}}><div style={{fontSize:13,fontWeight:700,color:"#EF4444"}}>{people.filter(function(x){return!x.fullyConnected&&(ago(x.lastContactDate)===null||ago(x.lastContactDate)>3)}).length}</div><div style={{fontSize:8,color:"var(--sb-muted)",fontWeight:700,textTransform:"uppercase",letterSpacing:"0.05em",marginTop:1}}>Overdue</div></div>
        </div>}
      </div>
...
function Portal(p){
  return <div style={{minHeight:"100vh",background:"linear-gradient(145deg,#3730A3 0%,#4338CA 25%,#3B35B0 50%,#2E2A8A 75%,#252262 100%)",fontFamily:"'Nunito',sans-serif",position:"relative",overflow:"hidden",animation:"portalFadeIn 0.4s ease"}}>
    ...
    <div style={{maxWidth:1000,margin:"0 auto",padding:"0 22px"}}>
      <div className="weavr-portal-header" style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"22px 0",animation:"loginSlideUp 0.5s cubic-bezier(0.22,1,0.36,1) 0.1s both"}}>
        ...
        <button onClick={p.onLogout} style={{padding:"8px 19px",borderRadius:10,border:"1px solid rgba(255,255,255,0.12)",background:"rgba(255,255,255,0.06)",color:"rgba(255,255,255,0.6)",fontSize:10,fontWeight:600,cursor:"pointer",backdropFilter:"blur(8px)",transition:"all 0.2s"}} onMouseEnter={function(e){e.currentTarget.style.background="rgba(255,255,255,0.12)"}} onMouseLeave={function(e){e.currentTarget.style.background="rgba(255,255,255,0.06)"}}>Sign Out</button>
      </div>

      <div className="weavr-portal-hero" style={{textAlign:"center",padding:"38px 0 45px"}}>
        <h1 style={{fontSize:28,fontWeight:800,color:"#fff",lineHeight:1.25,marginBottom:11,letterSpacing:"-0.01em",animation:"loginSlideUp 0.6s cubic-bezier(0.22,1,0.36,1) 0.2s both"}}>Ministry Tools That Keep<br/>Your Church Connected</h1>
        <p style={{fontSize:12,color:"rgba(255,255,255,0.4)",maxWidth:1000,margin:"0 auto",lineHeight:1.7,animation:"loginSlideUp 0.6s cubic-bezier(0.22,1,0.36,1) 0.3s both"}}>Everything your team needs to track, connect, and care for every person who walks through your doors.</p>
      </div>

      <div className="weavr-portal-grid" style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:13,paddingBottom:64}}>
        {TOOLS.map(function(t,idx){var isActive=t.ready;var delay=(0.4+idx*0.08)+"s";return <div key={t.key} className="weavr-portal-card" onClick={isActive?function(){p.onTool(t.key)}:undefined} style={{background:isActive?t.grad:"rgba(255,255,255,0.04)",borderRadius:17,padding:"22px 19px",cursor:isActive?"pointer":"default",border:isActive?"none":"1px solid rgba(255,255,255,0.06)",transition:"all 0.3s ease",opacity:isActive?1:0.55,position:"relative",overflow:"hidden",animation:"portalCardDrop 0.5s cubic-bezier(0.22,1,0.36,1) "+delay+" both"}} onMouseEnter={isActive?function(e){e.currentTarget.style.transform="translateY(-6px)";e.currentTarget.style.boxShadow="0 16px 48px "+t.color+"50"}:function(e){e.currentTarget.style.background="rgba(255,255,255,0.07)"}} onMouseLeave={isActive?function(e){e.currentTarget.style.transform="translateY(0)";e.currentTarget.style.boxShadow="none"}:function(e){e.currentTarget.style.background="rgba(255,255,255,0.04)"}}>
            <div style={{width:48,height:48,borderRadius:12,background:isActive?"rgba(255,255,255,0.2)":"rgba(255,255,255,0.06)",display:"flex",alignItems:"center",justifyContent:"center",marginBottom:16,backdropFilter:"blur(4px)"}}><I n={t.icon} sz={22} c="#fff"/></div>
            <div style={{fontSize:13,fontWeight:700,color:"#fff",marginBottom:5}}>Weavr {t.name}</div>
            <div style={{fontSize:10,color:isActive?"rgba(255,255,255,0.7)":"rgba(255,255,255,0.3)",lineHeight:1.6,marginBottom:13}}>{t.desc}</div>
            {isActive?<div style={{display:"flex",alignItems:"center",gap:5,fontSize:11,fontWeight:700,color:"rgba(255,255,255,0.9)"}}><span>Launch</span><span style={{fontSize:13}}>{"→"}</span></div>:<div style={{fontSize:9,fontWeight:700,color:"rgba(255,255,255,0.2)",textTransform:"uppercase",letterSpacing:"0.12em"}}>Coming Soon</div>}
          </div>})}
      </div>

      <div style={{textAlign:"center",paddingBottom:40}}>
        <div style={{display:"inline-flex",alignItems:"center",gap:7,padding:"8px 16px",borderRadius:12,background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.06)"}}>
