...
function TextModal(p){
  ...
  var tpls=getMultiTpl(person,config,"text");
  ...
  return <Modal title={"Text "+person.firstName} onClose={p.onClose}>
    <div style={{display:"flex",alignItems:"center",gap:7,background:"var(--inp)",borderRadius:10,padding:"10px 13px",marginBottom:10}}>
      <I n="phone" sz={16} c="#06B6D4"/>
      <span style={{flex:1,fontSize:13,fontWeight:600,color:"var(--text)",letterSpacing:"0.02em"}}>{fmtPhone(person.phone)}</span>
      <button onClick={function(){doCopy(person.phone,"phone")}} style={{padding:"5px 11px",borderRadius:7,background:copied==="phone"?"#10B981":"#06B6D4",color:"#fff",border:"none",fontSize:9,fontWeight:600,cursor:"pointer"}}>{copied==="phone"?"Copied!":"Copy #"}</button>
    </div>
    ...
    <div style={{display:"flex",gap:6}}>
      <a href={smsUrl(person.phone,msg)} target="_top" style={{flex:1,textDecoration:"none",padding:"10px",borderRadius:9,background:"linear-gradient(135deg,#34D399,#10B981)",textAlign:"center",fontSize:11,fontWeight:700,color:"#fff",display:"block"}}>iMessage</a>
      <a href={"https://voice.google.com/u/0/messages"} target="_blank" rel="noopener" style={{flex:1,textDecoration:"none",padding:"10px",borderRadius:9,background:"linear-gradient(135deg,#60A5FA,#3B82F6)",textAlign:"center",fontSize:11,fontWeight:700,color:"#fff",display:"block"}}>Google Voice</a>
    </div>
  </Modal>;
}
...
function EmailModal(p){
  ...
  var [showPicker,setShowPicker]=useState(false);
  var person=p.person;var config=p.config||{};
  var tpls=getMultiTpl(person,config,"email");
  ...
  return <Modal title={"Email "+person.firstName} onClose={p.onClose}>
    <div style={{display:"flex",alignItems:"center",gap:7,background:"var(--inp)",borderRadius:10,padding:"10px 13px",marginBottom:10}}>
      ...
      <button onClick={function(){doCopy(person.email,"email")}} style={{padding:"5px 11px",borderRadius:7,background:copied==="email"?"#10B981":"#10B981",color:"#fff",border:"none",fontSize:9,fontWeight:600,cursor:"pointer"}}>{copied==="email"?"Copied!":"Copy"}</button>
    </div>
    <div style={{marginBottom:10}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:5}}>
        <div style={{fontSize:9,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.08em",color:"var(--text-muted)"}}>Template: {tpls[selIdx]?tpls[selIdx].name:""}</div>
      ...
      {showPicker&&<div style={{display:"flex",flexWrap:"wrap",gap:5,marginBottom:8}}>{tpls.map(function(t,i){return <button key={i} onClick={function(){setSelIdx(i);setShowPicker(false)}} style={{padding:"5px 12px",borderRadius:7,border:i===selIdx?"1px solid #10B981":"1px solid var(--inp-border)",background:i===selIdx?"#10B98108":"var(--inp)",fontSize:10,fontWeight:i===selIdx?700:500,color:i===selIdx?"#10B981":"var(--text-sub)",cursor:"pointer"}}>{t.name}</button>})}</div>}
    ...
    <a href={mailUrl(person.email,em.subj||"",em.body||"")} target="_top" style={{display:"block",textDecoration:"none",padding:"10px",borderRadius:9,background:"linear-gradient(135deg,#34D399,#10B981)",textAlign:"center",fontSize:11,fontWeight:700,color:"#fff"}}>Open in Mail</a>
  </Modal>;
}
...
function CallModal(p){
  ...
  return <Modal title={"Call "+person.firstName} onClose={p.onClose}>
    <div style={{display:"flex",alignItems:"center",gap:7,background:"var(--inp)",borderRadius:10,padding:"10px 13px",marginBottom:13}}>
    ...
    <div style={{display:"flex",gap:8}}>
      <a href={"facetime-audio://"+person.phone} target="_top" style={{flex:1,textDecoration:"none",padding:"12px",borderRadius:10,background:"linear-gradient(135deg,#A78BFA,#7C3AED)",textAlign:"center",display:"block"}}><div style={{fontSize:12,fontWeight:700,color:"#fff",marginBottom:2}}>FaceTime Audio</div><div style={{fontSize:9,color:"rgba(255,255,255,0.6)"}}>Apple devices</div></a>
      <a href={"https://voice.google.com/u/0/calls"} target="_blank" rel="noopener" style={{flex:1,textDecoration:"none",padding:"12px",borderRadius:10,background:"linear-gradient(135deg,#60A5FA,#3B82F6)",textAlign:"center",display:"block"}}><div style={{fontSize:12,fontWeight:700,color:"#fff",marginBottom:2}}>Google Voice</div><div style={{fontSize:9,color:"rgba(255,255,255,0.6)"}}>Any device</div></a>
    </div>
  </Modal>;
}
...
function WeeklyStats(p){
  ...
  return <div style={{background:"var(--card)",borderRadius:17,overflow:"hidden",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
    <div style={{background:"var(--hero)",padding:"16px 22px"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div><h3 style={{fontSize:13,fontWeight:700,color:"#fff"}}>Weekly Church Stats</h3><p style={{fontSize:9,color:"rgba(255,255,255,0.45)",marginTop:2}}>{data.length} weeks tracked</p></div>
        <div style={{display:"flex",gap:4}}>
          {["week","trends","data"].map(function(t){return <button key={t} onClick={function(){setTab(t)}} style={{padding:"5px 10px",borderRadius:7,border:"none",fontSize:9,fontWeight:tab===t?700:500,color:tab===t?"#fff":"rgba(255,255,255,0.5)",background:tab===t?"rgba(255,255,255,0.15)":"transparent",cursor:"pointer"}}>{t==="week"?"This Week":t==="trends"?"Trends":"Data"}</button>})}
          <button onClick={function(){setShowAdd(!showAdd)}} style={{padding:"5px 10px",borderRadius:7,border:"1px solid rgba(255,255,255,0.2)",background:"rgba(255,255,255,0.08)",fontSize:9,fontWeight:600,color:"#fff",cursor:"pointer"}}>{showAdd?"Cancel":"+ Log Week"}</button>
        </div>
      </div>
    </div>
    ...
    <div style={{padding:"16px 22px 19px"}}>
    {tab==="week"&&<div>
      {recent.length>0?<div className="weavr-ws-grid" style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8}}>
        {cats.map(function(c){var val=latest[c.key]||0;var delta=getDelta(c.key);var avg=getAvg(c.key);return <div key={c.key} style={{background:c.color+"08",borderRadius:14,padding:"14px 13px",position:"relative",overflow:"hidden",border:"1px solid "+c.color+"12"}}>
          ...
          <div style={{fontSize:22,fontWeight:800,color:"var(--text)",lineHeight:1}}>{val}</div>
...
function Overview(p){
  ...
  var renderWidget=function(key){
    if(key==="next-up"){var top1=mml[0];if(!top1)return null;var stg=STAGES.find(function(s){return s.key===top1.currentStage});var d=ago(top1.lastContactDate);var tm=teams.find(function(t){return t.id===top1.assignedTo});return <div className="weavr-next-up" style={{display:"flex",gap:13,height:"100%",minHeight:0}}>
      <div style={{flex:"1 1 0",minWidth:0,background:"var(--card)",borderRadius:19,padding:"19px 22px",boxShadow:"var(--card-shadow)",display:"flex",alignItems:"center",gap:15,cursor:"pointer",transition:"all 0.2s",border:"1px solid var(--divider)",overflow:"hidden"}} onClick={function(){p.onPerson(top1)}} onMouseEnter={function(e){e.currentTarget.style.transform="translateY(-2px)";e.currentTarget.style.boxShadow="0 4px 20px rgba(0,0,0,0.08)"}} onMouseLeave={function(e){e.currentTarget.style.transform="translateY(0)";e.currentTarget.style.boxShadow="var(--card-shadow)"}}>
        <div style={{width:44,height:44,borderRadius:12,background:stg?stg.grad:"var(--divider)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><I n="zap" sz={18} c="#fff"/></div>
        ...
        <div style={{display:"flex",gap:6,flexShrink:0}}>
          {top1.phone&&<button onClick={function(e){e.stopPropagation();p.onContact({person:top1,mode:"text"})}} style={{width:34,height:34,borderRadius:9,background:"#06B6D408",border:"1px solid #06B6D415",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}><I n="msg" sz={14} c="#06B6D4"/></button>}
          {top1.phone&&<button onClick={function(e){e.stopPropagation();p.onContact({person:top1,mode:"call"})}} style={{width:34,height:34,borderRadius:9,background:"var(--primary)08",border:"1px solid var(--primary)15",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}><I n="phone" sz={14} c="var(--primary)"/></button>}
          {top1.email&&<button onClick={function(e){e.stopPropagation();p.onContact({person:top1,mode:"email"})}} style={{width:34,height:34,borderRadius:9,background:"#10B98108",border:"1px solid #10B98115",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}><I n="mail" sz={14} c="#10B981"/></button>}
        </div>
      </div>
      <div style={{display:"flex",flexDirection:"column",gap:6,width:160,flexShrink:0}}>
        {[{l:"New This Week",v:people.filter(function(x){return ago(x.createdAt)<=7}).length,c:"#10B981"},{l:"Need Follow-Up",v:people.filter(function(x){return!x.fullyConnected&&(ago(x.lastContactDate)===null||ago(x.lastContactDate)>3)}).length,c:"#EF4444"},{l:"Fully Connected",v:people.filter(function(x){return x.fullyConnected}).length,c:"#F97316"}].map(function(k){return <div key={k.l} style={{flex:1,background:k.c+"08",borderRadius:10,padding:"8px 12px",display:"flex",alignItems:"center",gap:7,border:"1px solid "+k.c+"12",minHeight:0}}><div style={{fontSize:15,fontWeight:800,color:k.c}}>{k.v}</div><div style={{fontSize:8,fontWeight:600,color:"var(--text-muted)",lineHeight:1.2}}>{k.l}</div></div>})}
      </div>
    </div>}
  ...
  return <div>
    <div className="weavr-pipeline-hero" style={{background:"var(--hero)",borderRadius:20,padding:"28px 30px 24px",marginBottom:22,position:"relative",overflow:"hidden"}}>
    ...
    workMode&&workPerson?<div style={{background:"var(--card)",borderRadius:19,overflow:"hidden",boxShadow:"0 4px 24px rgba(0,0,0,0.06)",animation:"gentleFade 0.4s cubic-bezier(0.22,1,0.36,1)"}}>
      ...
      <div style={{padding:"16px 26px"}}>
        <div style={{display:"flex",alignItems:"center",gap:7,marginBottom:13,flexWrap:"wrap"}}>
          {workPerson.phone&&<button onClick={function(){p.onContact({person:workPerson,mode:"text"})}} style={{padding:"6px 13px",borderRadius:8,background:"#06B6D410",fontSize:10,fontWeight:600,color:"#06B6D4",display:"flex",alignItems:"center",gap:4,border:"1px solid #06B6D425",cursor:"pointer"}}><I n="msg" sz={13} c="#06B6D4"/>Text</button>}
          {workPerson.phone&&<button onClick={function(){p.onContact({person:workPerson,mode:"call"})}} style={{padding:"6px 13px",borderRadius:8,background:"var(--primary)10",fontSize:10,fontWeight:600,color:"var(--primary)",display:"flex",alignItems:"center",gap:4,border:"1px solid var(--primary)25",cursor:"pointer"}}><I n="phone" sz={13} c="var(--primary)"/>Call</button>}
          {workPerson.email&&<button onClick={function(){p.onContact({person:workPerson,mode:"email"})}} style={{padding:"6px 13px",borderRadius:8,background:"#10B98110",fontSize:10,fontWeight:600,color:"#10B981",display:"flex",alignItems:"center",gap:4,border:"1px solid #10B98125",cursor:"pointer"}}><I n="mail" sz={13} c="#10B981"/>Email</button>}
          <div style={{flex:1}}/>
  ...
  return <div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(320px,1fr))",gap:15}}>
      {connected.map(function(x){
        ...
        return <div key={x.id} style={{background:"var(--card)",borderRadius:19,padding:"19px",cursor:"pointer",boxShadow:"var(--card-shadow)",border:"1px solid #F9731620",transition:"all 0.2s"}} onClick={function(){p.onPerson(x)}} onMouseEnter={function(e){e.currentTarget.style.transform="translateY(-2px)";e.currentTarget.style.borderColor="#F9731640"}} onMouseLeave={function(e){e.currentTarget.style.transform="translateY(0)";e.currentTarget.style.borderColor="#F9731620"}}>
          <div style={{display:"flex",alignItems:"center",gap:11,marginBottom:13}}>
            <div style={{width:44,height:44,borderRadius:12,background:"#F9731610",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:700,color:"#F97316"}}>{(x.firstName||"?").charAt(0)}{(x.lastName||"").charAt(0)}</div>
...
function Settings(p){
  ...
  return <div>
    ...
    {tab==="templates"&&<div style={{background:"var(--card)",borderRadius:19,padding:"21px 22px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
      ...
      <div style={{display:"flex",gap:16}}><div style={{width:180,flexShrink:0}}>{STAGES.map(function(s){return <button key={s.key} onClick={function(){setEk(s.key);setSaved(false)}} style={{display:"flex",alignItems:"center",gap:7,width:"100%",padding:"8px 11px",borderRadius:10,border:"none",fontSize:11,fontWeight:ek===s.key?600:500,background:ek===s.key?"var(--primary)08":"transparent",color:ek===s.key?"var(--primary)":"var(--text-sub)",textAlign:"left",marginBottom:2,cursor:"pointer"}}><Dot color={s.color} sz={7}/>{s.label}</button>})}</div>
      <div style={{flex:1}}>{ek?(function(){var stg=STAGES.find(function(s){return s.key===ek});var multiTpls=(config.multiTextTpl||DEFAULT_MULTI_TPL)[ek]||DEFAULT_MULTI_TPL[ek]||[];
        ...
        return <div>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
            ...
            <button onClick={addTpl} style={{padding:"4px 12px",borderRadius:7,background:"var(--primary)10",border:"1px solid var(--primary)25",fontSize:9,fontWeight:600,color:"var(--primary)",cursor:"pointer"}}>+ Add Template</button>
          </div>
          {multiTpls.map(function(t,i){return <div key={i} style={{background:"var(--inp)",borderRadius:12,padding:"12px",marginBottom:8,border:"1px solid var(--inp-border)"}}>
    ...
    {tab==="email-tpl"&&<div style={{background:"var(--card)",borderRadius:19,padding:"21px 22px",boxShadow:"var(--card-shadow)",border:"1px solid var(--divider)"}}>
      ...
      <div style={{display:"flex",gap:16}}>
        <div style={{width:180,flexShrink:0}}>{STAGES.map(function(s){return <button key={s.key} onClick={function(){setEmailEk(s.key);setEmailSaved(false)}} style={{display:"flex",alignItems:"center",gap:7,width:"100%",padding:"8px 11px",borderRadius:10,border:"none",fontSize:11,fontWeight:emailEk===s.key?600:500,background:emailEk===s.key?"var(--primary)08":"transparent",color:emailEk===s.key?"var(--primary)":"var(--text-sub)",textAlign:"left",marginBottom:2,cursor:"pointer"}}><Dot color={s.color} sz={7}/>{s.label}</button>})}</div>
        <div style={{flex:1}}>{emailEk?(function(){var stg=STAGES.find(function(s){return s.key===emailEk});var multiEmail=(config.multiEmailTpl||DEFAULT_EMAIL_TPL)[emailEk]||DEFAULT_EMAIL_TPL[emailEk]||[];
          ...
          return <div>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
              ...
              <button onClick={addETpl} style={{padding:"4px 12px",borderRadius:7,background:"var(--primary)10",border:"1px solid var(--primary)25",fontSize:9,fontWeight:600,color:"var(--primary)",cursor:"pointer"}}>+ Add Template</button>
            </div>
            {multiEmail.map(function(t,i){return <div key={i} style={{background:"var(--inp)",borderRadius:12,padding:"12px",marginBottom:8,border:"1px solid var(--inp-border)"}}>
              <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:6}}>
                <div style={{fontSize:9,fontWeight:700,color:"var(--text-muted)",textTransform:"uppercase",letterSpacing:"0.06em",flexShrink:0}}>Name</div>
...
function Panel(p){
  ...
  return <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,0.5)",zIndex:200,display:"flex",justifyContent:"flex-end"}} onClick={p.onClose}><div className="weavr-panel" style={{width:560,maxWidth:"100vw",height:"100vh",background:"var(--card-solid)",display:"flex",flexDirection:"column",animation:"panelSlide 0.4s cubic-bezier(0.22,1,0.36,1)",overflowY:"auto",boxShadow:"-12px 0 48px rgba(0,0,0,0.12)"}} onClick={function(e){e.stopPropagation()}}>
    ...
    <div className="weavr-panel-body" style={{padding:"16px 19px",overflowY:"auto",flex:1}}>
      ...
      {contactOpen==="text"&&<TextModal person={person} config={p.config} onClose={function(){setContactOpen(null)}}/>}
      {contactOpen==="email"&&<EmailModal person={person} config={p.config} onClose={function(){setContactOpen(null)}}/>}
      {contactOpen==="call"&&<CallModal person={person} config={p.config} onClose={function(){setContactOpen(null)}}/>}
...
function AppMain(p){
  ...
  var sideItems=[{key:"overview",icon:"home",label:"Overview",vw:"overview"},{key:"all",icon:"users",label:"All People",vw:"people"},{type:"divider"}].concat(STAGES.filter(function(s){return s.key!=="bgroup"&&s.key!=="ateam"}).map(function(s){return{key:s.key,label:s.label,color:s.color,vw:"people",sf:s.key}})).concat([{type:"divider"}]).concat(STAGES.filter(function(s){return s.key==="bgroup"||s.key==="ateam"}).map(function(s){return{key:s.key,label:s.label,color:s.color,vw:"people",sf:s.key}})).concat([{key:"fully-connected",label:"Fully Connected",color:"#F97316",icon:"target",vw:"connected"},{type:"divider"},{key:"assigned",icon:"card",label:"Assigned Cards",vw:"assigned"},{key:"quick",icon:"plus",label:"Quick Entry",vw:"quick"},{key:"bulk",icon:"send",label:"Bulk Message",vw:"bulk"},{key:"reports",icon:"chart",label:"Reports",vw:"reports"},{type:"divider"},{key:"settings",icon:"gear",label:"Settings",vw:"settings"}]);
  ...
  return <div style={{display:"flex",height:"100vh",fontFamily:"'Nunito',sans-serif",background:"var(--bg)",color:"var(--text)",...cssVars}}>
    ...
    <aside className="weavr-sidebar" style={{width:sbCollapsed?60:220,background:"var(--sidebar)",display:"flex",flexDirection:"column",flexShrink:0,height:"100vh",position:"sticky",top:0,transition:"width 0.25s cubic-bezier(0.22,1,0.36,1)",overflow:"visible",zIndex:60}}>
      <div style={{padding:sbCollapsed?"12px 0":"12px 10px",borderBottom:"1px solid var(--sb-divider)",display:"flex",alignItems:"center",justifyContent:sbCollapsed?"center":"flex-start",gap:sbCollapsed?0:10,position:"relative",zIndex:80}}>
        <div style={{position:"relative",width:sbCollapsed?"auto":"100%"}}>
          <button onClick={function(){setToolDrop(!toolDrop)}} style={{display:"flex",alignItems:"center",gap:sbCollapsed?0:10,background:"none",border:"none",cursor:"pointer",padding:0,width:sbCollapsed?"auto":"100%"}}>
            <div style={{width:32,height:32,borderRadius:9,background:"var(--logo)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><WeavrLogo sz={18}/></div>
            {!sbCollapsed&&<div style={{flex:1,display:"flex",alignItems:"center",gap:5}}><div style={{fontSize:13,fontWeight:800,color:"var(--sb-text)",letterSpacing:"0.12em"}}>WEAVR</div><span style={{fontSize:7,fontWeight:600,color:"var(--sb-muted)",background:"var(--sb-active)",padding:"2px 4px",borderRadius:3,letterSpacing:"0.04em"}}>Connect</span></div>}
      ...
      <nav style={{flex:1,padding:sbCollapsed?"12px 6px 6px":"14px 8px 6px",overflowY:"auto",overflowX:"hidden"}}>{sideItems.map(function(item,i){if(item.type==="divider")return <div key={"d"+i} style={{height:1,background:"var(--sb-divider)",margin:sbCollapsed?"5px 4px":"6px 8px"}}/>;var active=activeKey===item.key;return <button key={item.key} onClick={function(){nav(item.vw,item.sf)}} onMouseEnter={function(e){if(!active)e.currentTarget.style.background="var(--sb-active)"}} onMouseLeave={function(e){if(!active)e.currentTarget.style.background="transparent"}} style={{display:"flex",alignItems:"center",justifyContent:sbCollapsed?"center":"flex-start",gap:sbCollapsed?0:10,width:"100%",padding:sbCollapsed?"8px 0":"8px 10px",borderRadius:7,border:"none",fontSize:11,fontWeight:active?600:500,background:active?"var(--sb-active)":"transparent",color:active?"var(--sb-text)":"var(--sb-muted)",textAlign:"left",marginBottom:1,transition:"all 0.15s"}}>{item.icon?<I n={item.icon} sz={16} c={item.key==="fully-connected"?item.color:active?"var(--sb-text)":"var(--sb-muted)"}/>:<span style={{width:8,height:8,borderRadius:"50%",background:item.color,display:"inline-block"}}/>}{!sbCollapsed&&<span style={{flex:1}}>{item.label}</span>}{!sbCollapsed&&item.color&&<span style={{fontSize:9,fontWeight:600,color:"var(--sb-muted)",opacity:0.7}}>{countForStage(item.key)}</span>}</button>})}</nav>
      <div style={{padding:sbCollapsed?"6px 6px 10px":"8px 10px 12px",borderTop:"1px solid var(--sb-divider)"}}>
        {!sbCollapsed&&<button style={{width:"100%",padding:"8px 11px",borderRadius:8,border:"none",background:"var(--logo)",color:"#fff",fontSize:11,fontWeight:600,display:"flex",alignItems:"center",justifyContent:"center",gap:5,transition:"all 0.15s",boxShadow:"0 2px 8px rgba(0,0,0,0.15)"}} onClick={function(){setAdd(true)}} onMouseEnter={function(e){e.currentTarget.style.opacity="0.9";e.currentTarget.style.transform="translateY(-1px)"}} onMouseLeave={function(e){e.currentTarget.style.opacity="1";e.currentTarget.style.transform="translateY(0)"}}><I n="plus" sz={13} c="#fff"/>Add Person</button>}
        {sbCollapsed&&<button style={{width:"100%",padding:"7px 0",borderRadius:8,border:"none",background:"var(--logo)",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 2px 8px rgba(0,0,0,0.15)"}} onClick={function(){setAdd(true)}} title="Add Person"><I n="plus" sz={14} c="#fff"/></button>}
    ...
    <main style={{flex:1,overflowY:"auto",minHeight:"100vh"}}>
      <div className="weavr-main-header" style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 32px",position:"sticky",top:0,background:isDark?cw.darkSolid:isMono?"#F8F9FA":"#fff",zIndex:50,borderBottom:"1px solid var(--divider)",boxShadow:"0 2px 8px rgba(0,0,0,0.03)"}}>
        <span style={{fontSize:12,fontWeight:600,color:"var(--text)"}}>{new Date().toLocaleDateString("en-US",{weekday:"long",month:"long",day:"numeric",year:"numeric"})}</span>
        <div style={{display:"flex",alignItems:"center",gap:6}}>
          ...
          <button onClick={function(){if(!search)setSearch(" ");else setSearch("")}} style={{display:"flex",alignItems:"center",justifyContent:"center",width:32,height:32,borderRadius:8,border:"1px solid var(--inp-border)",background:search?"var(--primary)":"var(--inp)",cursor:"pointer",transition:"all 0.15s"}} title="Search" onMouseEnter={function(e){if(!search)e.currentTarget.style.background="var(--divider)"}} onMouseLeave={function(e){if(!search)e.currentTarget.style.background="var(--inp)"}}><I n="search" sz={13} c={search?"#fff":"var(--text-muted)"}/></button>
          <button onClick={function(){var next=config.theme==="light"?"dark":config.theme==="dark"?"mono":"light";setConfig({...config,theme:next})}} style={{display:"flex",alignItems:"center",justifyContent:"center",width:32,height:32,borderRadius:8,border:"1px solid var(--inp-border)",background:"var(--inp)",cursor:"pointer",transition:"all 0.15s"}} title={config.theme==="light"?"Dark Mode":config.theme==="dark"?"Simple Mode":"Light Mode"} onMouseEnter={function(e){e.currentTarget.style.background="var(--divider)"}} onMouseLeave={function(e){e.currentTarget.style.background="var(--inp)"}}><I n={config.theme==="mono"?"eye":"sun"} sz={13} c="var(--text-muted)"/></button>
          <button onClick={function(){setImp(true)}} style={{display:"flex",alignItems:"center",justifyContent:"center",width:32,height:32,borderRadius:8,border:"1px solid var(--inp-border)",background:"var(--inp)",cursor:"pointer",transition:"all 0.15s"}} title="Import" onMouseEnter={function(e){e.currentTarget.style.background="var(--divider)"}} onMouseLeave={function(e){e.currentTarget.style.background="var(--inp)"}}><I n="upload" sz={13} c="var(--text-muted)"/></button>
        </div>
      </div>
    ...
    {contactTarget&&contactTarget.mode==="text"&&<TextModal person={contactTarget.person} config={config} onClose={function(){setContactTarget(null)}}/>}
    {contactTarget&&contactTarget.mode==="email"&&<EmailModal person={contactTarget.person} config={config} onClose={function(){setContactTarget(null)}}/>}
    {contactTarget&&contactTarget.mode==="call"&&<CallModal person={contactTarget.person} config={config} onClose={function(){setContactTarget(null)}}/>}
  </div>;
}
